...
class AlphaSiftOpportunitiesApiTestCase(unittest.TestCase):
    ...
    def test_cross_market_board_theme_maps_ai_compute_gaming_and_fiber(self) -> None:
        ...
        self.assertEqual(classify("食品饮料"), "consumer")
        self.assertEqual(classify("银行"), "bank")
        self.assertEqual(classify("公用事业"), "utilities")
        self.assertEqual(classify("煤炭"), "energy")
        self.assertEqual(classify("机械设备"), "industrials")
        self.assertEqual(classify("消费电子"), "other")
        self.assertEqual(classify("新能源"), "other")
        self.assertEqual(classify("电力设备"), "other")
        self.assertEqual(classify("工业金属"), "other")
