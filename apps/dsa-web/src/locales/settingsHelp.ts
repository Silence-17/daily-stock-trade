...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.VNPY_RUNTIME_ENABLED': {
    title: 'vn.py runtime 启动',
    summary: '控制 Web/API 启动时是否创建 vn.py EventEngine 和 MainEngine。',
    usage: '默认关闭；关闭时仍可使用 DSA 本地模拟账本。开启后需要当前 Python 环境已安装 vn.py 及目标 gateway。',
    valueNotes: ['这是启动期配置，保存后需要重启后端或桌面端进程才会创建或重建 vn.py runtime。'],
    impact: ['影响模拟交易页面能否桥接真实 vn.py MainEngine、EventEngine、gateway 连接和事件订阅。'],
    notes: ['交易账号、密码、broker 等敏感连接参数应放在 VNPY_CONNECT_SETTINGS_PATH 指向的外部 JSON 文件里，不要写入 .env。'],
  },
  'settings.data_source.VNPY_GATEWAY_CLASS': {
    title: 'vn.py gateway 类',
    summary: '配置启动期加载的 vn.py gateway 类导入路径。',
    usage: '支持 module:Class 或 module.Class 格式；留空时只创建 MainEngine/EventEngine，不自动添加 gateway。',
    valueNotes: ['保存后需要重启后端或桌面端进程才会重新导入 gateway。'],
    impact: ['影响 runtime bootstrap 是否能调用 MainEngine.add_gateway，并决定后续连接和下单使用的 gateway 类型。'],
    notes: ['请确保该类来自已安装且可信的 vn.py gateway 包。'],
  },
  'settings.data_source.VNPY_GATEWAY_NAME': {
    title: 'vn.py gateway 名称',
    summary: '配置连接和下单时使用的 gateway name。',
    usage: '通常与 gateway 类或 vn.py gateway 实现的默认名称一致；留空时由适配层使用默认值或诊断为未配置。',
    valueNotes: ['保存后需要重启进程，或重新创建 paper trading service 后才会影响 runtime 桥接。'],
    impact: ['影响 MainEngine.connect 和 MainEngine.send_order 的 gateway_name 参数。'],
    notes: ['名称不等于账号凭证；账号凭证仍应放在外部连接 JSON。'],
  },
  'settings.data_source.VNPY_CONNECT_SETTINGS_PATH': {
    title: 'vn.py 连接配置文件',
    summary: '指向外部 JSON 文件，供启动期读取 gateway 连接参数。',
    usage: '文件内容会传给 MainEngine.connect；路径留空时不会自动连接。',
    valueNotes: ['该字段只保存路径，不保存账号、密码或 broker 参数本身。'],
    impact: ['配合 VNPY_CONNECT_ON_START=true 时影响启动期是否能自动连接 gateway。'],
    notes: ['请把连接 JSON 放在部署环境安全位置，并避免提交到仓库。'],
  },
  'settings.data_source.VNPY_CONNECT_ON_START': {
    title: '启动时连接 vn.py',
    summary: '控制 runtime bootstrap 是否在启动时调用 MainEngine.connect。',
    usage: '只有在已配置 gateway 和连接 JSON 时才建议开启；否则保持 false，由人工或外部流程连接。',
    valueNotes: ['保存后需要重启进程才会触发下一次启动期连接。'],
  ...
  'settings.data_source.VNPY_AUTO_ATTACH_EVENTS': {
    ...
    usage: '默认开启；当 EventEngine 可用时自动订阅，缺失时诊断为未配置。',
    valueNotes: ['保存后需要重启进程才会重新注册事件订阅。'],
    impact: ['影响 vn.py 回报是否自动同步到 DSA 模拟交易账本和页面状态。'],
    notes: ['关闭后仍可通过事件回写 API 手动同步订单、成交、账户和持仓。'],
  },
  'settings.data_source.VNPY_AUTO_RECONNECT_ENABLED': {
    title: 'vn.py 自动重连',
    summary: '控制 runtime 是否后台监测并恢复明确失败或断开的 gateway。',
    usage: '默认关闭；仅在已开启启动连接且 gateway 能报告连接状态时启用。连接状态无法确认时不会自动重连。',
    valueNotes: ['保存后需要重启进程才会启动或停止监控线程。'],
    impact: ['断线后按冷却间隔重新读取外部连接文件并调用 MainEngine.connect。'],
    notes: ['账号凭据仍只保存在 VNPY_CONNECT_SETTINGS_PATH 指向的外部文件中；诊断只记录次数、时间和结果。'],
  },
  'settings.data_source.VNPY_AUTO_RECONNECT_INTERVAL_SECONDS': {
    title: 'vn.py 自动重连间隔',
    summary: '设置连接检查与重连尝试的冷却秒数。',
    usage: '允许 5 至 3600 秒，默认 60 秒；连接正常时只检查状态，不重复调用 connect。',
    valueNotes: ['过短的间隔可能触发 gateway 或券商侧连接频率限制。'],
  ...
  'settings.data_source.VNPY_AUTO_RECONNECT_CONFIRMATION_GRACE_SECONDS': {
    ...
    usage: '允许 5 至 600 秒，默认 30 秒；宽限期内即使 gateway 暂时报告未连接也不会重复登录。',
    valueNotes: ['应覆盖目标 gateway 正常登录和行情/交易通道初始化耗时。'],
    impact: ['影响启动或重连后最早可再次尝试连接的时间。'],
    notes: ['保存后需要重启进程生效。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.VNPY_RUNTIME_ENABLED': {
    ...
    summary: 'Controls whether Web/API startup creates a vn.py EventEngine and MainEngine.',
    usage: 'Disabled by default. DSA local paper ledger remains available while disabled. Enabling it requires vn.py and the target gateway package in the backend Python environment.',
    valueNotes: ['This is a startup-time setting; restart the backend or desktop process after saving before expecting the vn.py runtime to be rebuilt.'],
    impact: ['Affects whether the paper trading page can bridge to a real vn.py MainEngine, EventEngine, gateway connection, and event subscriptions.'],
    notes: ['Keep account, password, broker, and other sensitive connection values in the external JSON referenced by VNPY_CONNECT_SETTINGS_PATH, not in .env.'],
  },
  'settings.data_source.VNPY_GATEWAY_CLASS': {
    title: 'vn.py Gateway Class',
    summary: 'Configures the vn.py gateway class import path loaded during startup.',
    usage: 'Supports module:Class or module.Class syntax. Leave empty to create only MainEngine/EventEngine without automatically adding a gateway.',
    valueNotes: ['Restart the backend or desktop process after saving before the gateway is imported again.'],
    impact: ['Affects MainEngine.add_gateway and determines the gateway type used later for connect and order routing.'],
    notes: ['Use an installed and trusted vn.py gateway package.'],
  },
  'settings.data_source.VNPY_GATEWAY_NAME': {
    title: 'vn.py Gateway Name',
    summary: 'Configures the gateway name used for connect and order routing.',
    usage: 'Usually matches the gateway class or the gateway implementation default. Leave empty to let the adapter use defaults or report the missing gateway in diagnostics.',
    valueNotes: ['Restart the process, or recreate the paper trading service, before expecting runtime bridge changes to apply.'],
    impact: ['Affects the gateway_name argument passed to MainEngine.connect and MainEngine.send_order.'],
    notes: ['This name is not a credential; credentials should still stay in the external connection JSON.'],
  },
  'settings.data_source.VNPY_CONNECT_SETTINGS_PATH': {
    title: 'vn.py Connect Settings File',
    summary: 'Points to an external JSON file containing gateway connection settings.',
    usage: 'The JSON object is passed to MainEngine.connect. Leave empty to skip automatic gateway connection.',
    valueNotes: ['This field stores only the file path, not account, password, or broker values.'],
    impact: ['When paired with VNPY_CONNECT_ON_START=true, affects whether startup can auto-connect the gateway.'],
    notes: ['Store the JSON in a secure deployment location and do not commit it to the repository.'],
  },
  'settings.data_source.VNPY_CONNECT_ON_START': {
    title: 'Connect vn.py On Start',
    summary: 'Controls whether runtime bootstrap calls MainEngine.connect during startup.',
    usage: 'Enable only when the gateway and external connection JSON are configured. Otherwise keep false and connect manually or through an external process.',
  ...
  'settings.data_source.VNPY_AUTO_ATTACH_EVENTS': {
    ...
    summary: 'Controls whether startup attaches the paper-trading sync bridge to vn.py order, trade, account, and position events.',
    usage: 'Enabled by default. When EventEngine is available, DSA subscribes automatically; otherwise diagnostics reports it as not configured.',
    valueNotes: ['Restart the process after saving before event subscriptions are registered again.'],
    impact: ['Affects whether vn.py callbacks automatically sync into the DSA paper ledger and page state.'],
    notes: ['When disabled, order, trade, account, and position data can still be synced through the event callback APIs.'],
  },
  'settings.data_source.VNPY_AUTO_RECONNECT_ENABLED': {
    title: 'vn.py Auto Reconnect',
    summary: 'Controls whether the runtime monitors and recovers a gateway that explicitly reports failed or disconnected state.',
    usage: 'Disabled by default. Enable only with connect-on-start and a gateway that reports connection state. An unconfirmed state is never reconnected automatically.',
    valueNotes: ['Restart the process after saving to start or stop the monitor thread.'],
    impact: ['After a disconnect, rereads the external connection file and calls MainEngine.connect at the configured interval.'],
    notes: ['Credentials remain only in the file referenced by VNPY_CONNECT_SETTINGS_PATH; diagnostics record counts, timestamps, and outcomes only.'],
  },
  'settings.data_source.VNPY_AUTO_RECONNECT_INTERVAL_SECONDS': {
    title: 'vn.py Auto Reconnect Interval',
    summary: 'Sets the cooldown in seconds between connection checks and reconnect attempts.',
    usage: 'Accepts 5 to 3600 seconds and defaults to 60. A healthy connection is checked without another connect call.',
  ...
  'settings.data_source.VNPY_AUTO_RECONNECT_CONFIRMATION_GRACE_SECONDS': {
    ...
    summary: 'Sets how long an asynchronous connect request may take to receive gateway confirmation.',
    usage: 'Accepts 5 to 600 seconds and defaults to 30. A gateway that temporarily reports disconnected is not logged in again during this grace period.',
    valueNotes: ['Allow enough time for normal login and market-data/trading-channel initialization.'],
    impact: ['Affects the earliest retry after startup or a reconnect request.'],
    notes: ['Restart the process after saving for the new grace period to take effect.'],
  },
  'settings.data_source.REALTIME_SOURCE_PRIORITY': {
