...
- [修复] A 股涨跌家数取证优先使用带供应商时间的东方财富完整分页快照，并采用独立 35 秒预算与全市场数据源的 30 秒请求上限对齐，避免主源空 JSON、慢速回退或通用 15 秒上限使 13:30/14:30 尾盘轮次误判数据不可用
- [改进] 跨市场 V1.4 恢复 13:30/14:30 独立入场轮次，按同日 A 股涨跌家数识别强势后分歧、跌超 4000 家的极端下跌与后续修复；尾盘放行后仍执行候选、技术位、收益成本、仓位与风控门槛，并以降级仓位上限下单
- [修复] 跨市场 Windows 长跑任务安装脚本恢复 13:28/14:28 唤醒，优先使用系统 Windows PowerShell、当前 PowerShell 作为备用，并保持每组 `powercfg` 参数边界，避免解释器路径缺失或数组展开导致安装失败
- [修复] 跨市场板块历史数据在 AkShare 东财接口返回非 JSON 时，银行与交通运输板块改用精确匹配的同花顺行业代理，避免候选因 `JSONDecodeError` 丢失技术证据
- [修复] 收敛三个活动策略的首次入场契约：跨市场 vn.py 模式由 09:30-09:35 入场盯盘独占正式执行并删除旧恢复别名，热点双策略删除绕过实时信号复核的强制兜底买入；旧 09:35 跨市场轮次仅保留历史验收兼容。
- [改进] 三个活动策略的首次入场统一提前到 A 股连续竞价 `[09:30,09:35)`：热点双策略在 09:28 预先冻结候选，跨市场日级调度直接投影到 09:30；旧 `09:35` 跨市场正式基线继续按原时窗计入既有活动进度。
- [修复] 跨市场板块技术日线新增东方财富有限重试、同花顺独立板块代理与同日内存缓存三层降级，审计披露代理板块来源；三层均不可用时仍失败关闭，不放宽入场阈值。
- [修复] 跨市场 10:40 受限复核合并读取当日同账户全部正式轮次的候选审计，不再因后续轮次覆盖基准而丢失早盘可恢复候选；已形成买单的股票继续排除，避免重复下单。
- [修复] 候选映射源板块的技术日线不可用时失败关闭并留待受限复核，不再把“昨日高振幅”等次级风格板块提升为主技术板块。
- [修复] 跨市场 60 秒只卖监控在策略仓位全部受 T+1 限制时直接轻量跳过，不创建 Agent 运行、不同步公司行动或请求卖出信号。
- [改进] 跨市场国内轮动候选扩展至医药、消费、银行、公用事业、能源和工业板块；海外方向不作硬否决且默认只占 10% 入场分，国内板块/个股/盘中/技术位占 90%，各权重及未站回开盘价/VWAP 的 25% 仓位上限均可在账户因子看板配置。
- [改进] 跨市场外盘方向门槛改为按主题相关性执行：美股、纳指期货及韩日明显走弱继续硬否决半导体、存储、CPO/CCL/MLCC、AI、算力、游戏等科技权重主题；医药候选可作为国内轮动后备继续接受本地板块、量价、技术位、评分、成本和账户风控判断，黄金仍使用独立信号。
- [测试] 补充科技主题外盘下跌继续关闭、医药绕过外盘方向否决及弱美股环境下国内医药候选保留的回归覆盖。
- [新功能] 模拟交易新增可选策略账户看板，统一展示当前跨市场执行账户与两个热点策略账户的权益、持仓和 30 日进度，并完整列出账户级运行因子，支持按账户校验、持久化配置和恢复默认且不切换实际执行账户。
- [修复] 模拟交易改为单端口复用主 API，历史风险回算按账户补齐且不覆盖其他账户或当前持仓缓存，缺失/过期价格不再持久化为有效净值，跨市场因子在下一次决策前重新加载，账户重置保留策略归属，热点策略单票目标由 50% 降至 25%，并停止记录窗口外或无工作空转事件及其重复信息日志。
- [改进] 三个策略账户在 A 股交易日 09:30-09:35 连续盯盘：热点双策略每 15 秒刷新候选并在条件首次满足时用第二份新鲜行情成交，跨市场 V1.4 同窗按候选恢复，并仅在 10:40 对早盘可恢复候选进行一次受限复核；其他时段禁止新买入及买单自动重试，保留 10:38 唤醒并移除 13:28/14:28 入场唤醒。
- [修复] 跨市场板块日线证据在同一交易日接口临时失败时复用已成功加载的内存缓存并披露缓存来源；没有合格缓存时继续失败关闭，10:40 只重试早盘已审计的可恢复候选。
- [改进] 跨市场亚洲方向门控由“韩日指数必须全部上涨”改为“明显走弱才否决”，轻微分化作为中性评分证据；MLCC/CCL 供应链强度仍是硬门槛，并新增可配置的亚洲市场明显走弱阈值。
- [改进] 自动买入继续受可用现金、手续费、整手、单票及总仓位上限约束；两个热点策略默认单票目标为当前账户权益 25%、最多两只，跨市场策略沿用自身分档仓位规则。
- [修复] 防止 Windows 混合睡眠和零委托观察任务抢锁导致跨市场 09:30 正式轮次错过执行窗口
- [改进] 09:45 双策略卖出逻辑改为 09:27 判断开盘集合竞价涨幅：低于 5% 在 09:30 模拟卖出，达到或超过 5% 则盯盘开盘 15 分钟，触及行情源涨停价时当日持有，否则分别启动 2%/3% 移动止盈并持续监控至收盘；停用原硬止损、成本保护、强弱确认和最长持仓期条件，并新增独立 Windows 盯盘任务与失败关闭审计。
- [修复] 内置 `DSA_SIM` 在进程启动时从绑定的本地模拟账户恢复现金与持仓，并在事件桥接后重新发布完整账户快照；单条持仓事件改为合并更新且零数量会清除旧持仓，避免重启后资金重置或卖出被误记为负持仓。

- [改进] V1.4 跨市场策略将沪深 300 高开改为开盘不追、日内回落承接：09:30 继续禁止追高，10:40 仅对早盘可恢复候选在回落、反弹并站稳开盘价或 VWAP 后允许分档买入；13:30/14:30 不再新开仓。
- [修复] V1.4 硬止损卖单改用当日跌停价作为保护性限价并继续按下一分钟 VWAP 撮合；活动卖单因 `limit_not_touched` 挂住时自动撤单重定价，避免快速下跌中止损委托长期失效。

- [修复] 美股盘前主题读取改为同一会话内优先最新可用证据，避免最后一分钟局部报价缺失覆盖此前合格主题快照，同时保留最后批次的原始缺失审计。
- [改进] 跨市场策略升级为 V1.4 分档执行：四个时点独立分析、按候选恢复、100 分分级入场、单票 25%+25%、主板块技术位降权、同主题分散及固定卖出优先级；支持在归属可证明时原账户接管现有策略仓位。
- [修复] 空的 09:45 候选池不再被记为已完成交易日；vn.py 模拟设置允许显式清空分配预算与连续亏损限制，Web 调度开关在保存并刷新运行状态后回收临时覆盖值。
- [chore] 删除项目内无引用的旧 Portfolio 非原子快照接口、已禁用的 AkShare 美股历史实现及孤立辅助代码。
- [改进] 根据 2026-08-04 至 2026-08-12 七个前向交易日的正式审计校准跨市场入场：纯 5/10 日短周期压力可由严格核心领涨、平开日内承接首档 25%，20/30/60 日压力、开盘追入、已有持仓和未确认伪突破仍失败关闭。
- [修复] 板块突破确认改为使用当前实际测试的压力位，不再以无关的 60 日高点统一代替；同价位重叠周期、各周期压力价位与偏离幅度完整写入 Agent 审计。
- [测试] 补充纯短压吸收、长短周期重叠、短压突破基准及候选审计字段边界回归。
- [改进] 跨市场平开日内抄底新增严格的核心领涨背离首档：仅允许经主题验证的异动龙头在高成交额、高实时量比、显著跑赢非下跌板块且完成回撤承接时建立 25% 仓位；压力位、外盘时序、亚洲联动、成本和风控仍失败关闭，板块未恢复时不得补仓。
- [修复] 跨市场人工模拟委托支持显式策略归属和主题，成交后按万 0.8 最低 5 元佣金、过户费及卖出印花税记账，并纳入策略卖出监控与当天防重复入场；所有 A 股自动选股入口统一执行当前账户仅限沪深主板的交易权限。
- [改进] 重构 09:45 双策略退出规则：取消固定止盈即全卖，超跌反转采用 3% 硬止损、4% 启动的 2% 移动止盈和双基准反弹失效，趋势再加速采用 3% 硬止损、5% 成本保护、7% 启动的 3% 移动止盈及两次趋势失效确认；峰值使用买入后证据，最长持有期按 A 股交易日历计算
- [修复] 09:45 双策略模拟改用强制供应商时间戳的行情回退链路，按请求完成时刻校验新鲜度并记录候选级拒绝原因，避免首选数据源无时间戳导致连续零决策
- [修复] A 股交易日恰逢日本法定休市时将日股门控记录为可审计的中性休市证据，避免因不存在的日股实时行情错误降级正式模拟日；日本正常交易日行情缺失或日历未知仍失败关闭。
- [改进] 平开跨市场入场不再强制候选板块靠近 5/10/20/30/60 日支撑或达到最低支撑分；支撑分改为最多 5 分加分，技术位可用、未贴近未突破压力位、板块强度和其他证据仍为入场条件。
- [修复] 将板块技术位证据暂不可用纳入跨市场正式轮次的盘中可恢复阻断，避免零计划零提交轮次错误停止后续自动复核。
- [修复] 韩股实时采集在供应商时钟仅领先本机 1 至 3 秒时短暂等待本机时钟追平，保留原始供应商时间戳；超过上限及显式时间路径仍失败关闭，避免正常实时行情被误判为未来证据。
- [新功能] 新增热点超跌反转与趋势再加速两个隔离的 09:45 前向模拟账户，按真实交易日自动运行 30 日，使用带供应商时间戳的信号/下一分钟成交证据并计入 A 股费用、滑点、T+1 与收盘估值
- [改进] 按当前账户交易权限将跨市场策略新买入限定为沪深主板，候选预筛和最终逐票决策双重排除创业板、科创板及北交所股票，同时保留已有非主板仓位的卖出与风险退出能力
- [改进] 在跨市场 `cn_open` 状态顶层披露 A 股 `session_date`，使四时点门控和纯前向交易日审计无需依赖嵌套原始快照即可核对证据归属日期
- [改进] 按最新策略确认将沪深 300 全局高开禁买阈值由 `+0.5%` 调整为 `+0.19%`；不足 `+0.19%` 的小幅高开继续进入平开/震荡及其他证据分析，边界回归固定 `0.189999%` 放行、`0.19%` 拦截；开盘原始证据保持冻结但每次读取按当前阈值重分类并披露旧分类差异，使盘中调整立即作用于当天
- [改进] 跨市场 V1.3 的 MLCC/CCL 逐票复核复用每 60 秒后台任务已采集且仍符合时间戳、覆盖率和阶段门槛的亚洲供应链快照，仅在缺失或不可用时现场重采整组；韩日市场仍逐票刷新并先于候选 A 股报价，避免同轮候选重复等待供应链网络超时
- [改进] 跨市场 V1.3 将沪深 300 高开与极端低开的全局买入硬门槛前置到候选筛选前；每轮先完成持仓卖出检查，再以 `completed` 零候选结果跳过 AlphaSift 和逐票取证，保留正式证据、卖单与门槛审计且 CPO 不可绕过
- [修复] 黄金利率新闻改为每轮无状态采集美联储两条官方 RSS 与金十/华尔街见闻补充源，统一输出 UTC 带时区发布时间并审计来源覆盖；空 Feed、缺源或窗口不完整时失败关闭，并保留同一前向窗口早先实时匹配但已滚出快讯列表的新闻，避免空响应或固定长度列表把有效利率信号误降为零分
- [修复] 09:35 跨市场正式轮次仅因沪深 300 开盘快照尚未落库而零计划零提交时，允许在证据补齐后由五分钟正式恢复任务重新评估，避免采集与下单同秒竞态锁死全天机会
- [修复] 跨市场逐票买入与持仓卖出刷新韩股、日股及亚洲供应链时改用各网络请求的真实完成时钟；买入在门控后再获取候选 A 股报价，卖出把韩股完成时刻传给后续风险信号，避免后返回的供应商时间戳被误判为未来数据
- [修复] 黄金与沪深 300 开盘采集保留请求开始时的窗口判定，并按网络完成时刻校验供应商时间、记录观察时刻与采集耗时，避免刚返回的秒级报价被误判为未来数据
- [修复] 黄金 09:15 前采集只作为盘前预览，正式与盘中决策仅接受利率新闻窗口已完整覆盖到 09:15 的快照，避免后续采集失败时误用更早的不完整新闻证据
- [修复] 后台任务健康摘要识别已完成亚洲组合采集中的 `component_status=degraded`，以 warning 和 `degraded_components` 披露局部供应商故障，不再误报 13 项全部健康

- [修复] 北交所 `920` 股票不再误映射为上海交易所，vn.py 适配器与 DSA_SIM 新增 `BSE` 路由，并按最低 100 股、之后 1 股递增及不足 100 股余量一次卖出的申报规则处理。
- [修复] 跨市场逐票决策池改为按活跃主题分排序后逐主题轮转取样，所有有候选的活跃主题先获得一次判断机会再补同主题后备，避免算力和人工智能候选挤掉 CPO、CCL、游戏等独立行情。
- [修复] 东方财富跨市场板块异动龙头新增源主题与个股实时所属板块交叉验证，错配、空板块或查询失败在预筛与逐票决策两层失败关闭，并披露验证、拒绝和不可用计数。
- [修复] 跨市场主题英文缩写改用字母数字边界匹配，阻止 `MicroLED` 因内部包含 `cro` 被误归医药，并补齐 MicroLED/MiniLED、微控制器与 AIDC 的明确映射。
- [修复] `EFINANCE_PRIORITY` 改为在 `.env` 加载后解析，`TUSHARE_PRIORITY` 显式配置不再被 Token 自动提权覆盖；不稳定或权限不足的数据源可保留为后备，并允许腾讯日线优先以缩短跨市场候选技术数据耗时。
- [修复] 收紧跨市场 A 股半导体主题映射：光刻胶不再误归设备，光伏“硅料硅片”不再误入半导体材料候选池，同时保留明确的半导体硅片与光刻设备映射。
- [修复] `NQ00Y` 分钟采集节流为 60 秒任务保留 5 秒调度余量，避免网络完成时间晚于调度起点后实际退化为每两分钟一条，提高 09:35 前三样本连续趋势的容错
- [修复] 跨市场 V1.3 正式观察升级为 schema 6，将 91 只全池采集审计与逐主题可交易覆盖分离；薄盘前成交不再否决合法开盘日，未覆盖主题仍在筛选和下单层失败关闭，且美股盘前与收盘必须同会话配对
- [修复] 跨市场 V1.3 将正式交易启动限定在 09:35 后两分钟内，错过时直接安排下一交易日；盘中正式恢复必须关联当天开盘基线并持久化恢复标记，迟到轮次与无基线恢复不再推进 30 日验收
- [修复] 跨市场 V1.3 在当天完全缺失 09:35 开盘基线时将后续时点路由到独立日内扫描，避免失败的正式恢复吞掉 10:40、13:30、14:30 抄底机会
- [修复] 跨市场 V1.3 日内扫描的窗外诊断只返回该任务实际负责的 10:40、13:30、14:30 时点，不再把独立正式轮次 09:35 混入配置列表
- [修复] 跨市场 V1.3 正式调度状态新增账户号并在重启恢复时校验策略、市场、账户、执行模式与来源，其他账户、`dry_run` 和独立日内轮次不再可能占用当前活动的 09:35
- [修复] 无 Agent run UID 的重复正式调用被 cadence guard 拒绝时不再覆盖 `last_formal_auto_run`，保留真实正式轮次的证据摘要供 readiness 与调度恢复使用
- [修复] 跨市场 V1.3 将最多两笔买入与只检查前两只候选解耦：每轮在两只目标之外依次复核 4 只后备，逐票硬门槛跳过不再抢占买入名额，形成两笔计划或提交后立即停止
- [修复] 美股 91 只盘前主题池在美东 09:14 实际启动并保留 Yahoo WebSocket 且不落策略证据；正式采集把陈旧或无效流式成交重新送入 Chart HTTP 补漏，并审计流式原始数、新鲜通过数、拒绝数和补漏数
- [修复] MLCC/CCL 亚洲供应链补足从运行状态贯通到主题筛选与逐票决策，仅在同一美股会话同阶段快照明确记录 OTC 成分覆盖不足且亚洲信号达到强势阈值时生效，缺少会话、阶段、主题记录或普通弱势信号继续失败关闭
- [修复] 串行同步同一 vn.py 委托的订单与成交回报，禁止迟到订单状态覆盖 `fill_sync`、下一分钟 VWAP、滑点和费用审计证据
- [修复] MLCC/CCL 亚洲供应链补足仅接受目标美股会话同阶段快照中明确的成分覆盖不足，禁止用亚洲行情掩盖美股会话、日历、schema、主题记录或阶段缺失
- [修复] 沪深 300 开盘冻结证据改为只接受当日交易日历 09:30 后的供应商时间，09:25-09:29 集合竞价快照不再可能被沿用为正式高低开信号
- [修复] 跨市场四时点与五分钟正式恢复新增当前账户当日计划/提交总闸，正式、日内入场或只卖来源任一产生订单活动后停止后续买入下单，同时将时点防重限定为当前账户的 `vnpy_paper` 轮次
- [修复] 黄金降息新闻窗口对“上一已完成 A 股交易日”的二次解析改为严格日历模式，日历缺失或异常时失败关闭，不再回退自然日生成未来或错误区间
- [修复] 美股收盘全主题池新增零主题部分采集持久化和 120 秒内逐成分滚动补齐；科技五项已成功时仅重试缺失主题池，预期覆盖不足记录为可重试证据警告而非任务崩溃，仍不生成合格快照
- [修复] 美股盘前零主题覆盖轮次改为独立持久化部分采集尝试，并在下一分钟仅滚动复用同一会话内、供应商时间仍不超过 120 秒的成分；尝试记录不进入合格快照、交易门槛或 30 日计数
- [修复] 美股盘前尚无主题达到最低覆盖时将 `us_premarket_evidence_unavailable` 记录为可重试证据跳过而非后台任务异常，同时继续拒绝持久化合格快照和交易门槛，其他采集异常仍失败报警
- [改进] 跨市场四时点后台审计持久化 `analysis_slot`、`formal_recovery`、`trigger_source` 与 `execution_mode`，`/task-events` 新增 `started_at` 下界并支持最多 5000 条按需查询，晚间可完整复核当日盘中扫描
- [修复] 美股收盘五项科技与 91 只全主题快照改为独立完成和重试，主题失败或部分覆盖可在收盘后 3 分钟内继续采集，并按同会话可用主题并集合并，避免科技快照先成功后永久阻断主题恢复
- [修复] Yahoo 盘前批量流在每分钟调度之间保持连接并缓存期间真实成交，连续 90 秒无调用后自动回收；缓存沿用供应商成交时间并继续执行 120 秒与正式开盘硬边界，避免短连接只监听数秒而漏掉轮询间成交
- [改进] 跨市场状态接口新增 `us_premarket.latest_capture`，直接披露最新真实盘前会话的 91 只成分采集数量、11 个映射主题覆盖与缺失清单，且与下一 A 股交易日使用的上一美股会话决策证据明确分离
- [修复] 美股 91 只盘前池改用 Yahoo 实时 WebSocket 单连接批量取秒级成交时间，Chart HTTP 仅补漏；不完整快照每分钟重试并滚动合并同一会话内仍不超过 120 秒的真实成分，避免 HTTP `429` 和五分钟冷却耗尽 09:15-09:30 盘前窗口
- [修复] 跨市场 V1.3 正式轮次在完成持仓卖出检查后、逐票实时分析前执行账户买入熔断，回撤、连亏或现金门槛生效时不再发起无效的跨市场候选调用，并保留真实账户风控原因为审计结论
- [修复] 跨市场 V1.3 手工批准、失败重试和活动买单复核统一重跑账户回撤、连续亏损冷却及现金门槛，风险熔断后将允许数量归零并撤销未成交余量，同时保持卖出计划不受买入门槛阻断
- [修复] 跨市场 V1.3 平开补仓资格改按策略成本占当前总权益的比例判断，净边际门槛按本次目标仓位对应的动态订单金额估算最低佣金、税费和滑点，避免账户盈亏后继续沿用初始 10 万/5 万元静态基准
- [修复] 跨市场 V1.3 次日高开回落卖出改按每个策略分档的真实下一 A 股交易日和对应数量执行，跳过周末休市并阻止过期持仓在随后任意高开日误触发
- [修复] 跨市场 V1.3 待成交买单按当前权益、剩余容量、执行价和费用重算数量，部分成交标的不再重复占用持股名额，同一 vn.py 委托的多笔成交仅计一档；卖单在真实平仓前保留移动止损峰值，买卖复核行情统一按网络完成时刻校验新鲜度
- [修复] 跨市场 V1.3 买入分档改按当前账户总权益乘以信号仓位百分比计算，并移除初始 5 万元单笔/单股及 10 万元日预算的绝对金额上限，避免账户盈利后两只各 50% 的满仓合同被静态初始资金截断
- [修复] 美股 91 只盘前与收盘观察池为无共享状态的 Yfinance 跨市场 HTTP 路由启用方法级并发白名单，避免 16 线程采集被 Fetcher 实例锁重新串行化，同时保留其他方法和数据源的串行保护
- [修复] 跨市场 V1.3 固定预设清空通用最大持有期和无进展退出，避免历史保存值进入审计或在后续执行重构中引入策略未定义的卖出条件
- [修复] 跨市场回放 API Schema 补齐 V1.3 全部 12 个可交易主题及收盘、盘前、`NQ00Y`、亚洲和技术位证据字段，策略选项名称同步更新为全板块轮动 V1.3
- [修复] MLCC/CCL 美股 ADR 或映射篮子缺少合格时间戳时，盘前与收盘覆盖可由同主题的新鲜亚洲供应链证据显式补足；其他主题及过期证据仍严格失败关闭
- [修复] 亚洲供应链读取按成分重新校验并选择最新仍有效的完整快照，避免临时报价失败覆盖尚在午休或同日官方收盘有效期内的 MLCC/CCL 证据，同时禁止过期或跨日沿用
- [修复] 跨市场 V1.3 自动买入按目标现金额度预留万 0.8 最低佣金和买入过户费后反解委托金额，避免两只 50% 委托同时提交时第二笔成交回写现金不足；审计同时披露限价与 50bp 动态滑点边界
- [修复] 跨市场 V1.3 运行预设强制关闭评分加权分配、旧目标权重、再平衡、通用市场门控及动态 LLM 策略切换，防止未确认的通用能力改变两只等额仓位和固定策略合同
- [测试] 新增真实 vn.py EventEngine 与 `DSA_SIM next_minute_vwap` 双买单系统回归，验证两只近满仓委托均成交、费用完整入账且账户现金不为负

- [修复] 跨市场正式观察契约升级为 schema 5，美股盘前与收盘不再以“任一主题可用”冒充全映射覆盖；两者都要求 11 个主题全部可验证，且仅允许 MLCC/CCL 由合格亚洲供应链证据补足
- [修复] 亚洲后台采集隔离韩国、日本及 MLCC/CCL 供应链路由，单一市场行情异常只降级对应证据并保留其他两路采集，不再令整项后台任务抛错中断
- [修复] vn.py 状态与生产就绪诊断区分最近任意观察轮次和最近正式 `vnpy_paper_auto` 轮次；V1.3 健康审计不再被后续 dry-run 覆盖，并显式报告正式轮次证据降级项
- [修复] 跨市场运行状态补齐半导体设备与材料主题，盘前/收盘覆盖统计现与实际全主题映射一致
- [修复] 跨市场 `10:40` 日股午休与 `14:30` 日韩台收盘证据改为交易日历约束的停盘延续：仅接受接近午间停盘或正式收盘的同日供应商时间，A 股 15:00 后及跨日立即失效，韩股连续五分钟链、日经/TOPIX 和 MLCC/CCL 逐成分重验
- [修复] TWSE MIS 台股行情改用响应明示的交易日期 `d` 与交易时间 `t` 构造台北成交时刻，不再用相对实际成交时间偏移一小时的 `tlong` 误判未来或伪实时行情
- [改进] 亚洲 MLCC/CCL 后台采集覆盖完整 A 股交易时段，日韩台收盘后至 15:00 继续重试合格正式收盘快照，避免 `14:30` 第四次分析因采集任务提前停止而永久缺证
- [测试] 补充日股午休、日韩台同日收盘、A 股收盘失效、跨日拒绝、韩股连续链和 `14:30` 组合亚洲门控回归
- [修复] MLCC 美股观察池移除京瓷于 2026 年 6 月 5 日终止的 `KYOCY` ADR 并同步 Web 股票索引，剩余 `TTDKY`/`MRAAY` 直接证据按 2/2 校验，盘前或收盘稀疏时仅使用合格亚洲供应链补充
- [修复] 美股存储观察池将 Everpure 的失效旧代码 `PSTG` 更新为 2026 年 4 月启用的纽交所代码 `P`，并同步 Web 股票索引，避免固定 404 降低主题覆盖率
- [修复] 跨市场活动主题候选层与最终逐票层统一重验美股收盘、盘前和亚洲供应链数值阈值，并取消缺少阶段证据时的 CPO 默认占位，避免无效主题抢占两只候选名额
- [修复] 美股 91 只盘前与收盘主题池改用最多 16 个有界并发任务，并将跨市场专用腾讯/Yahoo 单主机请求限制为 5 秒，避免串行回退耗尽 120 秒证据新鲜度窗口且不改变普通行情超时
- [修复] 美股科技与 CPO 收盘证据改按交易所日历的实际收盘时间采集，并增加夏冬令提前收市唤醒点，避免提前收市日因 16:00 固定窗口导致证据过期
- [修复] 美股 91 只盘前观察池新增交易所开盘时间戳硬边界与 09:14 夏冬令预唤醒，防止长耗时采集把 09:30 后正式盘分钟误标为盘前证据
- [修复] 半导体、存储、设备和材料在主题激活、逐票买入及震荡低吸路径统一执行 `US_TECH_SCORE>=40`，避免已采集的首小时/收盘总科技信号仅写审计却不参与交易门槛
- [修复] 全局主题震荡低吸与加仓重新执行韩日组合买入门控，CPO 保持独立豁免且震荡卖出不受买入门槛影响
- [文档] 修正跨市场观察调度频率：美股科技与 CPO 每 60 秒采集，黄金与零委托观察每 5 分钟执行
- [修复] 修复 vn.py 模拟交易页在移动端被长策略标识和诊断文本撑宽并产生页面级横向滚动的问题，宽表仍保留局部横向滚动。

- [修复] 日股严格证据新增日经 225 东方财富现金指数、明确标记的新浪 `hf_NK` 期货方向代理与 TOPIX Yahoo Japan 零延迟专用路由，拒绝 `delayMinutes>0`、不可解析及超过 120 秒的行情，避免 Yahoo 限流或延迟令韩日采集任务持续失败
- [修复] 跨市场日级自动任务每次结束后重新投影下一 A 股交易日 09:35，避免电脑休眠后迟到执行把固定 24 小时间隔永久漂移到后续交易日
- [修复] Windows 长跑任务新增 10:38、13:28、14:28 唤醒触发，覆盖三次盘中入场分析窗口且继续以 `IgnoreNew` 防止重复实例
- [修复] 盘中入场两分钟窗口在硬门槛阻断后仅审计一次，后续分钟轮询不再重复筛选
- [修复] MLCC/CCL 亚洲观察池新增 Yahoo Japan 零延迟个股、Naver 六位韩股与 TWSE MIS 明示成交时刻路由，并将台燿修正为上柜代码 `6274.TWO`
- [修复] MLCC/CCL 并发行情篮子改用整组网络完成时刻校验供应商时间，避免后返回的 Naver 行情被误判为未来证据
- [修复] Yahoo Japan 只有日内时刻的 TOPIX 与日本个股新增东京交易日校验，防止休市日把前一交易日收盘价误判为新鲜证据
- [测试] 新增日股双指数解析、延迟失败关闭、管理器路由优先级和后台任务动态重排回归
- [修复] 跨市场 V1.3 将观察池来源主题锁定到候选、成交与持仓卖出链路，避免同一股票兼具银行、黄金等标签时被重分类并套用错误的韩股或 `NQ00Y` 规则
- [修复] 沪深 300 高开仅阻止新买入；持仓卖出改为同时要求个股次日开盘涨幅不低于 1% 且相对盘中高点回落至少 0.8%，并在正式、盘中和待成交复核中统一执行
- [修复] 预期收益不再由 `ATR20%` 代替，改用显式预测、布林上轨、确认后的日内高点或最近板块压力空间；ATR 仅用于波动率和滑点诊断
- [改进] 跨市场 V1.3 候选新增当日成交额不低于 1 亿元门槛、低位因子正负 5 分修正、平滑板块强度和黄金价格单证据 25% 仓位上限
- [改进] 跨市场震荡高抛低吸收敛为最多两档，匹配两只股票各 50% 与总仓 100% 的激进仓位合同
- [新功能] 新增跨市场旧活动到干净 V1.3 账户的显式迁移 API，阻止活动计划、策略持仓或已有成交账户混入，并将前向模拟安全重置为 0/30
- [改进] 30 日报告新增至少 8 笔买入成交和 5 笔已平仓交易的统计门槛，样本不足时返回 `final_insufficient_sample` 而不宣称策略通过
- [测试] 补充 V1.3 迁移、主题持久化、高开双条件卖出、`NQ00Y` 持仓减仓、黄金仓位、低位修正、两档震荡、恢复防重、流动性和样本门槛回归
- [改进] 跨市场 V1.3 科技策略新增 `NQ00Y` 小型纳指连续走势：A 股时段连续采集，确认急跌时阻止买入或减半可卖仓位，正常方向仅作最多正负 5 分辅助修正，并纳入正式 30 日证据资格
- [修复] 跨市场正式观察契约升级为 schema 4，要求 `NQ00Y` 至少三个独立供应商时间戳覆盖 120 秒，旧 ready 记录不得在缺少期货证据时继续计入合格日
- [测试] 补充 `NQ00Y` 新浪路由、连续趋势、过期失败关闭、买入阻断、严重下跌减仓和验收降级回归
- [改进] 跨市场 V1.3 平开强信号改为板块分不低于 60、有效支撑分不低于 85 后首笔 25%，仅在后续日内回撤反弹确认后补至单票 50%；每日买单上限相应调整为 4 笔且仍受两只持仓、单票 50% 和总仓 100% 限制
- [修复] 板块支撑改为只接受当前点位下方且非下降均线的有效支撑；压力突破新增板块量比不低于 1.5 与连续两个已完成 5 分钟收盘确认，缺失量价证据时继续禁止追高
- [测试] 补充平开 25% 分档及日内加仓、支撑方向与均线斜率、突破量比和连续 5 分钟收盘回归
- [新功能] 跨市场策略升级为 `cross_market_global_sector_rotation_v1.3_aggressive`，新增医药、MLCC、CCL/PCB、日股大盘及亚洲供应链观察，并以统一主题词典覆盖观察池对应的 A 股完整子板块映射
- [新功能] 跨市场逐票决策新增板块 5/10/20/30/60 日均线、区间支撑、压力位和多周期共振提醒，任一周期接近压力且未突破时禁止追高，白酒/银行仅在目标板块已确认支撑时提供有限轮动加分
- [改进] 跨市场模拟成本按用户账户调整为买卖双边万 0.8 佣金并保留每笔最低 5 元，股票卖出印花税 0.05%、双边过户费 0.001% 和动态滑点继续计入净收益
- [改进] 黄金主题同时识别降息、暂停加息、宽松周期以及重新加息、降息推迟和实际利率上行新闻，分别提高或压低买入信号
- [修复] 跨市场 `09:35` 开盘判断改用刚完成的美股收盘主题，`10:40`、`13:30`、`14:30` 日内抄底改用上一盘前与同会话收盘配对并拒绝盘前强势后的收盘反转
- [修复] 新代码读取 V1.1/V1.2 活动账户时在选股或委托前失败关闭，禁止服务重启把旧 30 日进度静默混入 V1.3 或普通选股策略
- [测试] 补充全主题映射、韩日及亚洲产业链、盘前与收盘会话配对、5/10/20/30/60 日支撑压力、轮动门禁、手续费和 Web 技术位提醒回归
- [新功能] 跨市场激进策略扩展美股盘前观察池至半导体、存储、光通信、人工智能、算力服务和游戏龙头，按主题归一化后映射实时 A 股板块及板块龙头股票
- [新功能] 跨市场策略在 09:35、10:40、13:30、14:30 四个时点重新选板块和股票，后三次使用独立防重审计来源且不重复推进30日合格日
- [改进] 跨市场激进策略调整为最多两只股票、单票目标50%和总仓位100%，新增及加仓金额按目标仓位剩余缺口计算并继续扣除手续费、税费和滑点
- [测试] 补充美股盘前观察池全局去重、主题覆盖率、A股主题映射、同主题双股票、四时点防重及韩股差异化门控回归
- [修复] 跨市场证据完整但零活动的正式轮次新增五分钟盘中入场复核，仅重试低开承接、震荡买点、板块强度和供应商时间戳等可恢复条件，并在任一计划或提交后停止
- [修复] CPO 美股证据新增开盘首小时五分钟节流快照用于当夜新鲜度审计，次日 A 股信号仍只读取同会话正式收盘快照

- [改进] 跨市场 30 日模拟绩效指标在桌面端改为每行五项，避免权益、收益和成本金额被截断。

- [修复] 为跨市场三十日模拟活动增加无人值守的 A 股交易日 14:55 后账户快照任务和 14:55 系统唤醒点，避免页面未打开或电脑午后休眠时缺失最终净值证据，且不回填历史日期
- [修复] 任务健康接口按跨市场策略与零委托观察的实际运行合同标记必需任务，任一采集、恢复、复核或收盘快照任务缺失时失败告警
- [修复] 跨市场活动收盘快照通过严格行情路由持久化价格与 provider timestamp，最终报告拒绝历史收盘价、时间戳过期、陈旧汇率或受限估值，并持续重试降级快照
- [修复] 跨市场模拟状态读取改为只读 Portfolio 快照，避免页面刷新覆盖活动的严格收盘估值证据
- [修复] 跨市场盘中监控、委托复核、公司行动与风险读取统一禁用 Portfolio 快照写入，保证收盘任务是活动日净值证据的唯一写入口
- [修复] 模拟交易设置在跨市场策略下将连续亏损冷却准确展示为只读的 3 个 A 股交易日，避免 4320 分钟字段误导为自然日配置
- [修复] 模拟交易页按 vn.py 安装和 gateway 连接状态展示真实执行提示，DSA_SIM 已连接时不再误显示未安装降级文案
- [修复] 模拟交易绩效统计中的长策略与跳过原因标识可在卡片内换行，避免页面出现横向溢出
- [修复] 跨市场正式运行在最终服务入口按 A 股交易日去重，页面/API 误触不得重复下单，仅允许零活动降级轮次在证据恢复后补跑一次
- [修复] 正式恢复证据助手与主验收统一仅接受 `completed`，`partial` 运行保留活动审计但不得冒充完整正式日
- [测试] 锁定跨市场韩股、美股、CPO、黄金和 A 股开盘运行快照按信号类型独立有界保留，避免 30 日长跑状态无限增长
- [修复] 30 日跨市场验收仅接受 `completed` 正式运行，旧 `partial` 状态失败关闭且不得推进日级进度
- [测试] 锁定美股夏令时与冬令时唤醒触发均覆盖开盘、首小时和收盘采集窗口
- [修复] 30 日跨市场验收将正式计数模式与零委托观察模式分开披露，`dry_run` 不再出现在合格执行模式中
- [修复] 30 日跨市场报告明确标记后续观察补齐的证据不计入正式模拟交易日，避免警告文案误示为已完成计数
- [改进] 30 日跨市场模拟报告新增逐笔下一分钟 VWAP、实际/模型滑点、价差、分钟波动和参与率证据，并汇总滑点成本、全口径交易成本及证据覆盖率
- [改进] 30 日跨市场模拟报告改用持久化账户日快照生成逐日净值、收益和最大回撤，保留零成交日；缺少任一观察日快照时最终报告失败关闭
- [改进] 30 日活动只将 A 股交易日 14:55 后更新的账户快照认定为收盘净值证据，午间快照不再误计为完整日终绩效
- [改进] 30 日跨市场报告新增正式模拟运行的逐股紧凑决策审计与动作、状态、主题、阻断原因汇总，模拟交易页展示主要阻断原因
- [修复] 30 日跨市场决策审计以交易前实时板块复核主题为准，并保留候选预筛主题与重分类标记，避免被阻断标的仍按旧主题计数
- [修复] 模拟交易页兼容 API camelCase 决策原因码并补齐实时行情时间戳与主题复核阻断中文说明
- [修复] 30 日前向模拟只允许证据完整的正式 `vnpy_paper_auto + vnpy_paper` 日期推进完成门槛，后续 dry-run 补证仅保留为观察记录，页面区分有效正式模拟日与证据完整观察日
- [测试] 韩股连续门控新增5样本仅覆盖240秒仍失败关闭的回归，明确连续5分钟必须由首尾至少300秒的样本窗口证明
- [新功能] 跨市场正式轮次因证据未就绪且零计划零提交时，新增五分钟一次的受限正式恢复任务；证据完整后仅补跑正常 `vnpy_paper`，已有活动或合格正式轮次时严格跳过
- [修复] 任务健康检查识别跨市场正式恢复的盘外、未到正式轮次、证据未就绪、已有活动和已完成等预期跳过状态，避免正常等待被误报为 warning
- [修复] 跨市场震荡指标排除当日未完成日线并将实时高低价与现价作为新会话追加，避免覆盖上一交易日真实收盘而污染 ADX、RSI 和布林带
- [修复] CPO 核心股门槛要求 EastMoney 板块异动龙头来源、核心股标记和 CPO 来源主题同时成立，移除全局前三高分候选冒充核心股的宽松后门

- [改进] 跨市场 30 日活动逐日保留正式 `vnpy_paper` 轮次自身的运行状态及候选、计划、提交、跳过数量，避免后续零委托观察成为代表记录时覆盖正式执行审计
- [改进] 跨市场 30 日活动将证据完整日与正式 `vnpy_paper` 执行覆盖分开披露，逐日保留正式运行当时的证据状态，并在后续零委托观察补证或完整日缺少正式运行时给出报告警告

- [修复] 内置 `DSA_SIM` 启动和重连时从 paper 设置同步初始资金，并丢弃误传入的外部券商凭据字段，避免网关默认 100 万元与 10 万元跨市场活动账本错配。
- [修复] 跨市场 30 日活动绩效报告为初始权益和只读账户快照补充活动启动日与当前/完成交易日，避免零成交阶段权益曲线端点日期为空
- [修复] 跨市场 30 日活动以第 30 个完整证据日固定报告截止日并只读重放截止净值，完成日后自动交易正常跳过；遗留委托先对账再撤销剩余量，失败计划禁止重提，避免第 31 日行情或成交继续改写最终结果
- [改进] 跨市场 30 日活动验收与报告新增逐交易日 `session_results`，同等证据完整度下优先固化正式 `vnpy_paper` 运行，并记录缺失要求、执行模式和决策数量，且不生成或回填历史行情
- [改进] 跨市场零委托观察在同账户当日已有完整证据后轻量跳过，避免每五分钟重复筛选候选，同时保留缺证据日的恢复重试
- [改进] 移除重复且已由选股覆盖的行业板块页面、侧栏入口和公开 API，保留选股与个股分析内部复用的板块数据能力
- [修复] 模拟交易权益曲线为响应式容器提供合法初始尺寸，避免页面首次布局时产生负宽高警告或短暂空白
- [修复] 韩股连续 5 分钟门控除按 45–90 秒有效采样间隔选择确认链外，强制首尾覆盖至少 300 秒；正常一分钟轮询需第 6 个样本，拒绝用近重复刷新、突发采样或仅跨四分钟的 5 个样本冒充持续观察
- [修复] A 股委托申报单位与撮合成交单位分层校验：普通沪深合法整手及科创板最低 200 股后按 1 股递增申报，进入撮合后允许整数股部分成交，避免 216 股科创板委托成交 200 股后剩余 16 股永久无法完成
- [修复] 在线严格撮合从腾讯/新浪统一实时行情读取买一与卖一并计算实际价差进入动态滑点，仅在盘口缺失时回退 4bp；真实 0bp 价差不再被误判为缺失，仍受最低 2bp 滑点约束
- [修复] 严格下一分钟 VWAP 撮合要求两次供应商累计行情时间跨度处于 45–90 秒，跨度过短或因休眠跨越多根分钟线时撤单失败关闭，并在 vn.py 脱敏诊断中披露门禁范围
- [修复] 跨市场 A 股委托使用决策时已验证实时价并按板块执行申报数量规则，vn.py 状态诊断同步披露普通沪深与科创板数量契约；普通沪深部分卖出按 100 股、零股仅清仓，科创板买入最低 200 股且之后按 1 股递增，资金不足最小申报量时失败关闭
- [修复] 震荡高抛由固定卖出当前仓位三分之一改为按当前一/二/三档分别卖出全部、二分之一、三分之一，确保盘中监控和未成交复核每次恰好减少一整档且仍受 T+1 限制
- [修复] 沪深 300 开盘快照不再将成分股总成交额除以总股数误作指数点位 VWAP，并明确标记不可用；低开候选继续使用个股自身 VWAP，缺失时仅允许真实站回个股开盘价
- [修复] 跨市场 30 日 Windows 长跑为 08:50 黄金盘前、09:23 A 股开盘/正式决策和 23:25 美股冬令时首小时补充独立唤醒触发，并在修复已运行任务时跳过重复启动，避免再次休眠漏采关键证据或留下“已有实例”的假故障结果
- [测试] 锁定跨市场 Windows 长跑的开机启动、八个夏冬令关键窗口唤醒点及运行中幂等安装行为
- [修复] 跨市场纯前向模拟仅以 30 个证据完整的 A 股交易日完成验收，缺失外部信号的降级观察日继续披露但不再冲抵目标天数
- [测试] 补充跨市场运行时风控设置回归，锁定最多 3 只、总仓 60%、板块 25%、单股 10%、首次三分之一及 5%/8%/4% 止损止盈参数
- [修复] 跨市场账户总回撤达到 8% 时改为暂停新买入而非强制清仓，已有仓位继续执行硬止损、高开、韩股转弱、止盈和震荡卖出
- [修复] vn.py 成交回调与自动任务并发写配置时统一使用进程内锁，并对 Windows 原子替换的瞬时文件占用有限重试，避免真实成交后正式任务偶发失败
- [测试] 补充 CPO 盘中卖出独立性回归，确保韩股转弱不会触发 CPO 卖出，通用风控与 CPO 自身行情规则保持不变

- [测试] 新增跨市场 ETF 免股票印花税与过户费，以及动态滑点严格限制在 2–50bp 的确定性回归
- [修复] 后台任务健康检查将跨市场采集器的非窗口、正常节流、已采集、非交易日及空仓跳过识别为健康节奏，并为各采集任务补充中文标签；日历或行情异常等非白名单跳过仍保持告警
- [文档] 修正 KIS 配置示例中的韩股行情回退顺序为 Naver Finance 后接 YFinance，并明确所有来源统一执行 120 秒新鲜度门禁
- [改进] 模拟交易页新增账户绑定的 30 日跨市场活动进度与净绩效面板，展示证据完整度、各类缺失证据累计天数、费用和最大回撤，并支持导出活动报告 JSON
- [修复] A 股收盘后重启且当日正式模拟已执行时，下一次正式任务改从当前盘后时刻投影到下一交易日 09:35，避免固定增加一天后落到明日收盘并误跳至后天
- [改进] 30 日跨市场模拟活动绑定启动时的模拟账户和起始权益，拒绝混入账户重置后的 Agent 运行，并新增活动期净绩效、风险指标、费用税费和逐笔成交报告接口
- [修复] 跨市场候选买入、加仓与待成交复核改用严格供应商时间戳行情路由，避免无供应商时间的普通 Tushare 快照遮蔽腾讯实时数据并导致所有候选误报 `candidate_provider_timestamp_required`
- [修复] 跨市场正式和盘中卖出、震荡卖出 VWAP 及待成交卖单复核统一使用严格供应商时间戳行情路由，避免普通 Tushare 无时间快照使高开、止损或韩股转弱卖出被静默跳过
- [改进] 沪深 300 开盘冻结快照新增 `quote_source`，可审计 TickFlow 或回退指数源的实际命中结果
- [修复] EastMoney 目标板块龙头在候选预筛阶段复用最终决策的实时所属板块分类，过滤仅有黄金概念但主营零售等不支持标的，并按主题族渐进递补下一名且只为潜在占席候选计算 ATR
- [修复] 跨市场候选严格行情改用单候选报价请求完成时刻校验供应商时间并记录耗时，避免实时板块与报价串行请求期间后返回的腾讯秒级行情误报 `candidate_realtime_quote_stale`
- [修复] 盘中重启后五分钟零委托观察与 60 秒卖出监控同相位时改为等待共享 Agent 锁最多 60 秒，避免每轮固定碰撞而永久漏掉下午观察
- [修复] 订单型 Agent run 预先绑定模拟账户，自动重试、活动委托扫描及手工批准/重试按账户 ID 与创建边界双重隔离，阻止账户重置前的旧计划重新提交到当前账户
- [修复] 韩股四项串行行情改用整组网络请求完成时刻校验供应商时间，并记录采集开始与耗时，避免后返回的 Naver 秒级时间相对请求开始晚数秒而误报 `korea_evidence_from_future`，真实未来和超过 120 秒证据仍失败关闭
- [修复] 美股科技五项与 CPO 三项串行行情改用整组网络请求完成时刻校验供应商时间并记录采集耗时，避免后返回的真实秒级报价相对请求开始晚数秒而误报未来数据
- [修复] vn.py paper 绩效归因排除不操作模拟账户的 `agent_calibration_shadow` 运行，避免校准用的其他策略样本混入 30 日模拟报告
- [修复] 跨市场 30 日验收在同一交易日存在多次观察时按就绪、缺失项更少、时间更新的顺序选择代表快照，避免日级缺失要求统计随轮询顺序抖动
- [修复] vn.py paper 绩效摘要默认按当前模拟账户创建时间过滤 Agent 运行，避免账户重置前的旧策略归因混入 30 日纯前向模拟；显式时间窗口仍优先并披露窗口来源
- [修复] 盘中启动时校准 shadow 与首次正式自动交易落入同一锁窗口会自动错后至少 5 分钟，证据监控再错后 5 分钟，避免长校准任务耗尽正式轮次的 120 秒锁等待
- [修复] 最近任意 Agent 运行与最近正式 `vnpy_paper_auto` 轮次分开持久化，旧状态从 Agent 审计回补正式轮次，避免 5 分钟零委托观察覆盖日级调度基准并在盘中重启后重复正式执行
- [修复] 正式 vn.py paper 日级任务遇到同秒盘中卖出监控、观察或校准任务短暂占用 Agent 运行锁时改为最多等待 120 秒，超时单独记录 `auto_agent_run_lock_timeout`，避免毫秒级竞争直接漏掉当日正式模拟
- [改进] 跨市场 30 日 Windows 长跑新增可重复安装的 `SYSTEM` 计划任务与前台运行脚本，固化开机启动、异常每分钟重试、单实例和韩股/A 股及美股夏冬令关键窗口唤醒配置
- [修复] vn.py paper 配置改为同目录临时文件落盘后原子替换，启动读取遇到瞬时空文件或替换竞争时有限重试，避免服务重启时回落默认配置并漏注册自动交易及跨市场采集任务
- [修复] 跨市场零委托观察任务不再被正式交易的连续失败熔断锁存，数据源恢复后下一轮可继续形成完整或降级观察；正式 `vnpy_paper` 交易仍保留熔断与冷却恢复探测
- [改进] vn.py paper 绩效摘要新增累计手续费、税费、总交易成本、扣费后盈利/亏损金额和盈利因子，并明确动态滑点已包含在下一分钟 VWAP 冲击后的成交价中
- [修复] A 股开盘与黄金盘前采集器在时钟窗口内先校验交易所交易日，周末和法定休市不再发起行情请求或误报陈旧数据，交易日历不可用时继续失败关闭
- [改进] 跨市场 30 日长跑补充 Windows NTP 时间同步与交流电唤醒定时器运行门禁，避免主机退回 CMOS 时钟后误判 120 秒行情新鲜度或睡眠时漏采关键窗口
- [修复] 美股科技与 CPO 收盘采集按市场阶段枚举值识别 `postmarket`，避免真实交易日历返回 `MarketPhase.POSTMARKET` 时错误跳过正式收盘快照
- [测试] 新增真实美股交易日历的夏令时、冬令时与圣诞休市收盘采集回归，覆盖科技和 CPO 两条链路
- [修复] 纽约金主连 `GC1` 新增腾讯实时、故障时新浪实时，以及新浪全球期货日线的专用路由，保留秒级供应商时间并在黄金快照记录实时/日线来源；Yahoo 降为最终备用，避免约 20 分钟延迟和日线限流使黄金信号永久无法通过 120 秒门槛
- [修复] 未配置 KIS 时韩股连续门控改为先读取 Naver Finance 的 KOSPI、KOSDAQ、三星电子和 SK 海力士实时轮询数据并保留 `localTradedAt`，Yahoo 降为第二备用源；约 30 分钟延迟的 Yahoo 分钟数据不再让韩股门控在 120 秒门槛下永久不可用
- [修复] 跨市场美股科技与 CPO 严格证据改用腾讯实时行情优先、Yahoo Chart 备用的独立路由，保留供应商时间戳与 120 秒新鲜度门禁，避免同轮第 5 个 Yahoo 请求受限导致整组信号失败关闭；其他美股分析路由不变
- [修复] vn.py 适配器烟测、拒单/重复成交故障矩阵和重连循环改用临时 vn.py 工作目录及隔离的固定延时撮合，不再与在线进程共用 `.vntrader`，也不会读取生产 `DSA_SIM_MATCHING_MODE=next_minute_vwap` 后因缺少真实分钟基线误报失败；严格撮合仍由专用网关测试覆盖
- [改进] 跨市场运行状态为美股科技与 CPO 增加采集会话、阶段计数、最后时间戳和首小时/收盘阶段完整度，支持在下一 A 股交易日前直接审计真实采集进度
- [改进] 30 日纯前向模拟将已运行 A 股交易日与证据完整日拆分统计；缺失或过期外部证据继续失败关闭，当天保留为降级观察日并单独披露完整率、降级日期和缺失项，但不冲抵 30 个完整日目标，盘后或时间戳错位运行仍不计数
- [修复] 启动或续接 30 个交易日纯前向模拟活动时清除旧回测证据，仅保留既有活动开始时间和未来交易日计数，避免不参与验收的历史夹具继续出现在状态中
- [修复] 跨市场目标主题候选范围在内部 EastMoney 板块源校验通过后直接运行，不再受 AlphaSift 启用状态或适配器可用性阻断，自动交易就绪度与系统健康检查同步识别该内部候选源；其他候选范围仍保持原 AlphaSift 检查
- [修复] 跨市场候选源改为一次读取 EastMoney 目标板块异动龙头，并按半导体链、CPO、黄金均衡排列前三名；移除该策略前置的通用新闻/资金流选股，避免非目标行业挤占候选和分钟级扫描，同时以已完成日线 ATR20 补齐龙头候选的成本边际证据

- [修复] 跨市场低开买入不再把 AlphaSift 个股综合分冒充板块强度；改用 EastMoney 点时所属板块涨跌幅聚合主题相关板块并在证据缺失时失败关闭，高分个股所在板块转弱仍禁止买入
- [修复] 美股科技与 CPO 收盘证据改为每 60 秒检查并仅在交易所进入 `postmarket` 后 3 分钟内冻结一次，首小时仍按 5 分钟节流，不再把 15:50 起的盘中快照冒充正式收盘
- [修复] 跨市场黄金降息新闻读取按上一 A 股收盘至 09:15 的实际窗口动态扩展并分页，覆盖国庆等超过 5 天的休市，避免固定 5 日/100 条查询漏掉有效新闻
- [修复] 跨市场黄金 5/20 日趋势在 A 股盘前纳入纽约前一日晚间已完成的最新结算日线，并记录 `trend_through_date`，避免周二至周五趋势证据固定慢一天
- [修复] 跨市场策略在持有归属仓位时于正式运行、盘中只卖和待成交重校验前自动同步最近 45 天已实施分红与拆股，按 Tushare 到免费公告源回退并去重；全部来源失败时失败关闭，避免用未复权持仓错误交易
- [修复] 跨市场策略单日 1.5% 净亏损熔断改用持久化的上一 A 股交易日收盘权益，计入浮动盈亏和交易成本，避免把隔夜累计浮亏误算为当日亏损；熔断只阻止新买单，不影响风险卖出
- [修复] 跨市场实时候选不再因 AlphaSift 未提供顶层预期收益字段而把边际默认为零；按显式预测、震荡布林上轨空间、点时 ATR20% 依次估算毛边际并记录来源，全部缺失时失败关闭
- [修复] 跨市场震荡入场在候选缺少预计算指标时按需加载日线计算 ADX、MA20 斜率、RSI 与布林位置，避免平开震荡策略永久无信号

- [新功能] 内置 `DSA_SIM` 新增严格 `next_minute_vwap` 撮合模式，以两次供应商时间戳实时快照的累计成交量和成交额差值构造下一分钟 VWAP，并应用动态滑点、限价、5% 分钟参与率、部分成交和继续等待
- [修复] 跨市场 `vnpy_paper` 委托仅允许处于严格下一分钟模式的 `DSA_SIM`，行情陈旧、时间戳未推进、累计值回退或量额增量不一致时撤单失败关闭，不再用固定延迟请求价冒充真实成交
- [修复] 跨市场 vn.py 部分成交回写按整张委托累计最低佣金，并逐笔记入佣金增量、过户费和卖出印花税，避免模拟账本漏计成本或重复收取最低佣金
- [修复] 严格撮合直接采用腾讯供应商涨跌停价覆盖 ST 等特殊限幅；A 股同时缺少供应商限价和昨收时拒绝委托，且两次快照切换供应商时撤单失败关闭
- [改进] vn.py 只读状态诊断新增脱敏 `matching` 摘要，展示 DSA_SIM 撮合模式、等待秒数和待撮合基线数
- [测试] 新增下一分钟 VWAP、动态滑点、分钟参与率部分成交、异常累计行情撤单及跨市场撮合模式门禁回归
- [修复] 美股科技与 CPO 信号按交易日历精确绑定 A 股交易前最近一个已完成美股会话，缺少目标会话或交易日历不可用时失败关闭，避免跨交易日复用旧信号
- [修复] 跨市场策略新增 A 股盘中 60 秒只卖监控，持续根据韩股转弱、高开、止损/止盈、移动止损和震荡信号处理策略可卖仓位，不运行选股或生成买单

- [修复] 跨市场零委托观察任务在启用自动模拟交易时错后 2 分钟启动，避免与 09:35 `vnpy_paper` 任务争抢共享运行锁而跳过当天真实模拟交易
- [修复] `DataFetcherManager(fetchers=[])` 现在保留调用方显式空数据源语义，仅在 `fetchers=None` 时加载默认数据源，避免测试或隔离流程意外访问已配置的 Tushare
- [测试] 新增跨市场自动交易与只读观察启动时间错峰，以及周五美股首小时/收盘信号供周一 A 股取用的跨周末回归
- [改进] `cross_market_semiconductor_gold_v1.1` 验收改为从用户启动时刻起累计 30 个 A 股交易日的前向模拟，不再依赖历史数据导入或 500 日回放资格
- [新功能] 新增跨市场 30 日模拟活动开始/重置 API，按交易时段、执行模式和就绪跨市场快照去重计日，并保留历史回放为可选诊断
- [测试] 新增无历史数据启动、30 个交易日计数、重复启动、重置与 API 回归
- [修复] 切换 vn.py 网关时清理旧网关账户、持仓和委托同步快照，并在新快照中记录网关归属，避免模拟与实盘诊断串仓
- [修复] Tushare 初始化日志移除 Windows GBK 控制台无法编码的装饰符号，避免后台服务持续输出日志编码异常
- [改进] 本机 30 交易日跨市场模拟服务改由 Windows `SYSTEM` 计划任务在系统启动时托管，异常退出每分钟重试、仅监听回环地址，并在韩股/A股与美股关键窗口唤醒睡眠中的电脑；同步补充中英文停用说明
- [修复] Windows 非 ASCII 工作区为 curl/yfinance 配置保持证书校验的 ASCII CA 包，Yahoo 限流时通过双域 Chart 分钟接口回退并保留真实供应商时间戳
- [修复] 跨市场沪深 300 开盘信号改用严格指数路由，跳过缺少供应商时间的 Tushare 等快照并继续回退 TickFlow/EastMoney，避免活动因错误股票路由永久无法计日
- [测试] 新增 CA 包复制复用、Yahoo Chart 时间戳与双域回退、严格指数逐源选择及 A 股开盘快照回归

- [新功能] 跨市场韩股信号新增可选韩国投资证券 KIS Open API 首选源，按官方 KOSPI/KOSDAQ 分钟指数与三星电子/SK 海力士分时成交时间构造报价，未配置或请求失败时回退 YFinance
- [改进] 跨市场策略状态新增脱敏韩股行情路由诊断，展示 KIS/YFinance 可用性而不暴露凭据或接入地址
- [测试] 新增 KIS 认证令牌复用、韩股指数与个股映射、供应商时间戳、120 秒过期失败关闭、数据源优先级和 YFinance 回退回归
- [修复] 跨市场真实历史验收升级为原始供应商 artifact、frames artifact 和逐帧来源记录三层校验；仅声明 manifest/frames 哈希的数据不再具备真实历史资格
- [测试] 新增原始 artifact 哈希与大小、frames 路径/哈希/条数/覆盖期、逐帧 source role 和 record key 验证回归
- [新功能] 新增不覆盖现有自动策略的跨市场零委托观察开关，启用后持续采集韩股、美股科技、CPO、黄金和 A 股开盘信号，并仅在 A 股交易时段运行独立 `dry_run` 以累计活动启动后的 30 个 A 股交易日证据
- [测试] 新增跨市场独立观察任务零委托、调度注册、API 设置持久化和专用观察来源计数回归
- [修复] 跨市场连续 3 次亏损后的冷却由 4320 自然分钟改为 A 股交易日历的后续 3 个完整交易日，第 4 个交易日恢复，周末和休市日不再提前消耗冷却期
- [测试] 新增跨周末的跨市场连续亏损交易日冷却与恢复回归
- [修复] 跨市场黄金主题按显式证券类型或 A 股 ETF 代码区分股票与 ETF，不再让黄金矿业股票错误免除卖出印花税和双边过户费；预期成本门槛与本地账本复用同一品种判断
- [测试] 新增黄金矿业股票与黄金 ETF 的跨市场费用分类回归
- [修复] 跨市场 `vnpy_paper` 活动委托新增每 60 秒网关对账与当前信号重校验；信号失效或允许数量低于未成交数量时主动撤单并持久化证据，部分成交买单保留风险占用但不因已有部分仓位被误取消
- [测试] 新增跨市场活动委托继续等待、信号失效撤单、数量收缩撤单和 60 秒后台任务回归
- [修复] 跨市场 30 日前向观察不再把活动启动前、盘后演练、非交易日、错误执行模式或时间戳错位运行计为有效日期；每轮交易时段内的 `dry_run`/`vnpy_paper` 冻结 A 股开盘、美股首小时与收盘、连续韩股门控及黄金信号，四项就绪记为完整日，关键证据缺失则失败关闭并记为降级观察日
- [测试] 新增跨市场观察快照必需证据、盘后运行拒绝和缺失证据不计日回归
- [修复] 跨市场待审批与重试卖单按当前账户、策略仓位和新鲜报价重新计算熔断、硬止损、移动止损、高开、韩股、止盈及震荡退出，按当前触发比例收缩原计划、使用重校验报价并复核 T+1；待审批阶段不再提前清除移动止损峰值
- [测试] 新增跨市场硬止损卖单审批重校验回归，覆盖止损持续时放行和价格恢复后信号失效
...
- [改进] 跨市场模拟策略新增独立 CPO 美股证据、策略仓位归属、撤单与分板涨跌停撮合、严格下一分钟和证据时效校验，以及可选的带文件哈希历史回放与 30 交易日前向模拟验收

- [新功能] 新增 `cross_market_semiconductor_gold_v1.1` 跨市场模拟交易策略、韩美股与纽约金冻结信号、A股开盘和T+1风控、分钟级成本撮合、可选历史回放消融 API 及中英文执行说明
- [改进] 跨市场策略以同一美股交易日首小时/收盘 `40%/60%` 合成科技信号，震荡低吸支持最多三档逐档加仓，并按每笔策略 lot 独立执行拆股成本与 A 股 T+1
- [修复] 跨市场历史验收要求实际覆盖科技链、CPO、黄金和震荡场景，并验证四项消融均相对完整策略产生差异，避免未触发对应逻辑的空消融被误判通过
- [修复] CPO 独立美股与新闻证据现实际进入板块强度决策，并在单项缺失时按剩余来源归一化；全部外部证据缺失时保留 A 股自身路径且继续绕过韩股门控
- [修复] 黄金降息新闻窗口改用 A 股交易日历定位最近已完成收盘，避免法定长假后仅按工作日回溯而截断有效新闻
