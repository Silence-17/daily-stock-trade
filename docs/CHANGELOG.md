...
- [新功能] 已实施公司行动新增 Tushare -> AkShare EastMoney -> 巨潮资讯 fallback；免费源只接受有效除权日的实施方案，按每十股口径标准化现金分红与送转倍数，缓存 24 小时并持久化实际 provider，已验证覆盖 SSE/SZSE，BSE 不生成未验证事件
- [测试] 新增免费公司行动实施状态、日期范围、每股换算、送转合并、巨潮降级、缓存、无效 Schema、Tushare 优先/权限失败降级和 BSE 显式空覆盖回归；真实默认路由为 `600519` 持久化 3 条现金分红，重复采集为 0 新增/3 更新，EastMoney 与巨潮 34 个共同日期/类型事件数值全部一致；重启后的 API 后台任务完成 1 条完整因子与 3 条公司行动更新，零错误且 DSA_SIM 保持连接
