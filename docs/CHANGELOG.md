...
- [修复] 自动卖出持仓天数改为按 100 条合法分页读取完整成交历史并 FIFO 重放当前未平仓批次，清仓后重新建仓会重置持仓时钟；数据质量或账户/市场买入门禁触发前已成交的止损卖单会保留并写入一致的 Agent run 提交/跳过计数
- [测试] 扩展 vn.py paper 风控组合回归，覆盖超过 100 条成交、清仓后重建仓、数据质量门禁和账户/市场门禁下卖出优先执行及审计一致性
