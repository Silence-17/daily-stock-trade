...
- [测试] 外部 XTP 定时成交只读观察器新增受限风险合同，要求单次/每日预算、订单数、持仓暴露、最低现金和失败熔断均处于验收边界；配置即使保持不变，过宽也会失败
- [修复] vn.py 状态诊断不再返回外部柜台账户标识和原始账户回报，仅保留余额、可用/冻结资金、盈亏、币种与更新时间；内部事件关联和订单恢复状态不变
- [测试] XTP 定时成交观察器强制候选、决策、计划、提交、成交严格各为一且零跳过，防止额外终态订单被误判为验收通过
- [修复] A 股自动交易会从扩展候选池中优先选择单笔额度可覆盖一手的股票，避免最高分候选超出最小手数预算时整轮零下单
- [改进] vn.py 部署长跑新增指定 build id 与公共账户诊断脱敏门禁，可只读拒绝旧进程或暴露 `account_id`/`raw` 的运行时
- [改进] Windows 计划内 API 切换可严格校验并优雅停止只读成交观察器进程组，避免重启连接间隙污染最终成交验收

- [修复] vn.py Gateway 长跑验收的零连接预检改用自动清理的专用 JSON 结果文件，XTP Windows 账户 workflow 显式启用 Python UTF-8 输出，不再把插件启动信息或中文字段报告误判为预检失败
- [改进] XTP A 股账户长跑在连续 60 秒确认交易/行情双通道连接后才开始稳定性计量，过滤登录收敛期抖动但仍保持 99% 连接率门禁
- [测试] 中泰 XTP 股票类型测试账号完成首轮 15 分钟零下单验收：180/180 次双通道连接确认、账户/持仓事件 240/4320、订单/成交事件 0，并保留 14 天脱敏 artifact
...
- [测试] 新增免费公司行动实施状态、日期范围、每股换算、送转合并、巨潮降级、缓存、无效 Schema、Tushare 优先/权限失败降级，以及北交所公告标题/代码/日期/方案校验和新旧代码查询回归；真实默认路由为 `600519` 持久化 3 条现金分红，EastMoney 与巨潮 34 个共同日期/类型事件数值全部一致；北交所最新 20 份实施公告解析 20/20，`920976` 持久化 4 条事件且重复采集为 0 新增/4 更新；重启后的 API 任务 `ff2c2bb67d194596811efd53ace3ad42` 完成 1 条完整因子与 4 条公司行动更新，零错误且 DSA_SIM 保持连接
- [新功能] 历史全 A 股成员新增 Tushare 优先、BaoStock 沪深生命周期与北交所官方当前名单/代码映射/最终摘牌公告的完整 fallback；北交所任何未知退市或未解释缺口均 fail-closed，组合结果按进程缓存 24 小时
- [改进] 全市场历史因子日线改为新浪、EastMoney、腾讯、BaoStock 多源降级并逐行记录真实 provider；作业冻结真实 `universe_source`，不再把免费组合源误记为 Tushare
- [修复] 全市场验收覆盖率改用完整因子行而非单纯写入行数，并累计完整/部分、精确日线和估值完整计数，防止缺日线的空壳快照通过长任务门禁
- [测试] 新增 BaoStock 生命周期、北交所退市闭环/开市日钳制/未知缺口失败、Tushare 优先与免费源 fallback、日线来源降级和部分因子验收拒绝回归；在线 2026-07-17 股票池返回 5,529 条，干净长任务完成 111 批，完整因子 5,494（99.367%）、精确日线 5,522（99.8734%）、估值完整 5,502（99.5117%）、源错误 27（0.4883%），最终验收 `ok=true`
...
- [测试] 新增活跃租约拒绝、重启 orphaned 自动接管和全市场验收器回归；真实 Python 3.13 API 重启态验证返回 `recovery_state=orphaned`、`resume_allowed=true`，Agent 控制台在 1440px 与 390px 均无横向溢出。早期 Tushare `stock_basic` 权限不足的 424 已由后续 BaoStock+BSE 官方 fallback 与成功全市场长任务证据取代
...
- [修复] 盘中市场门禁新增默认开启的 provider 行情时间校验：指数和 A 股宽度必须携带完整且 15 分钟内的 provider 时间戳，否则 fail-closed；Web 可显式关闭该严格要求以接受仅有抓取时刻的降级行情，诊断会保留开关、覆盖率和拒绝原因
...
- [改进] 盘中与跨市场取证调用新增 15 秒单次预算和 4 个进程级滞留线程上限；provider 超时、异常或工作池耗尽均快速 fail-closed，并审计稳定错误类型、耗时和预算，避免单一行情源无限拖住自动交易调度
- [修复] 主指数与市场宽度 fallback 结果新增 provider、抓取时刻和数据粒度元数据；TickFlow 保留真实 provider 时间，YFinance 标记 session bar，Tushare `index_daily` 标记收盘日线。盘中本市场门禁不再把 Tushare 最近日线当作实时指数，并拒绝日期不属于当前市场日或 provider 时间超过 15 分钟的明确陈旧证据
...
- [测试] 扩展 vn.py paper 风控组合回归，覆盖超过 100 条成交、清仓后重建仓、同日拆股后部分卖出、公司行动读取失败、数据质量门禁和账户/市场门禁下卖出优先执行及审计一致性
- [新功能] 模拟交易完整状态按 15 分钟桶持久化持仓估值价格源健康并保留 120 天；系统健康和 Web 页面新增 7/30/90 天有效/总观测数、时间与持仓加权覆盖/新鲜率、降级占比及 provider 份额，空仓不再计作 100% 健康，轻量状态只读历史，存储失败返回脱敏稳定标记且不阻断当前估值和状态接口
- [新功能] AlphaSift 现金账本组合回测新增版本化成本档位：A 股常规参考、A 股低佣参考和零成本对照会由后端原子应用佣金、最低佣金、历史税制与滑点；`custom` 保留显式参数，结果与 Agent 控制台审计请求值、实际生效值和档位版本
...
- [新功能] 自动模拟交易新增默认关闭的市场宽度与热点退潮门禁，基于最近两次持久化 `MarketLightSnapshot` 的 breadth/limit 维度阻断新增买入，缺少已启用规则所需快照时 fail-closed，并把当前值、前值、阈值和原因写入 Agent 诊断与时间线
...
- [新功能] 新增 Agent 收益/风险长期校准趋势 API 与 Web 视图，支持 7/30/90 天及市场、策略、状态筛选，展示持久化滚动快照的观测率、目标状态、风险效用和版本分组，并明确不把重叠窗口计作独立样本
- [新功能] Agent 跨运行评价新增版本化逐日收益/风险目标函数，汇总日均收益、波动率、下行偏差、20% 尾部收益和最大不利波动，并以风险调整效用自动收紧候选数量与模拟交易预算或触发可选买入门禁
- [新功能] 跨运行质量门禁新增“LLM 最终复核优先、规则 Agent 兜底”的成熟样本校准，低通过精度或低阻断避损率可阻断新增买入并驱动跨市场目标只收紧
- [新功能] Agent 前瞻评价新增规则复核与 LLM 模型/提示词/评估器版本的 1/5/10/20 日质量矩阵，展示通过精度、阻断避损率、收益差和成熟样本覆盖率
- [修复] Agent 前瞻评价在包含跳过候选时纳入真实 `action=skip` 风控阻断记录，避免阻断质量统计漏样本
- [新功能] 自动选股 Agent 新增跨市场动态目标，根据人工反馈、连续失败和跨运行质量只收紧候选数量与模拟交易预算，并在 Agent 控制台展示生效前后数值和原因

- [新功能] Agent 控制台新增 run 级人工验收，可保存通过、需修改或驳回结论以及审阅人和备注，并在运行详情、列表、时间线和今日总结中统一展示。
- [改进] 最近同策略/市场 Agent 上下文新增人工验收分布与逐 run 意见，可供后续规则计划审计和可选 LLM 动态计划参考，但不会绕过数据质量、仓位或交易风控。
- [测试] 新增人工反馈幂等更新、API 契约、日汇总、时间线、后续上下文、Web 保存交互和 snake_case 映射回归。

- [改进] vn.py 内置模拟网关在重连时保留资金、持仓和活动订单并恢复在途延迟成交；安装验收新增三轮断线循环，校验订单缓存、恰好一次成交和编号去重。

- [新功能] 自动模拟交易新增独立的连续已平仓亏损门禁，复用本地 paper 账本 FIFO 已实现盈亏；达到可选亏损笔数上限后仅暂停新买入，卖出继续执行。
- [改进] 连续亏损门禁支持持久化开启状态和冷却自动恢复，盈利/持平平仓也会立即清零连续亏损；系统健康、Agent run 和告警中心记录当前连亏、恢复时间及 `resolved` 恢复事件。
- [测试] 新增连续亏损 FIFO 统计、买入阻断、卖出后重置、冷却恢复、API 健康组件及 Web 设置映射回归。
...
- [新功能] AlphaSift 现金账本组合回测支持请求内显式现金分红和拆并股事件，按每个标的的交易/估值时钟执行，并固定同日“公司行动先于交易”。
- [改进] 现金分红按事件日前已有股数增加现金，拆并股调整持仓数量；逐期审计 `applied`/`not_held` 状态、前后股数和现金影响，未处理事件在方法论中披露，产生零碎股时 fail-closed。
- [改进] Agent 控制台新增公司行动 JSON 输入和事件执行审计；当前使用未复权价格账本，不自动从外部源抓取事件或推断零碎股现金补偿。
- [测试] 新增持仓期分红、拆股后退出、买入同日不追溯分红、零碎股阻断及 API/Web 事件映射回归。
- [新功能] AlphaSift 现金账本组合回测新增每笔最低佣金和卖出税率参数，目标整手数量与可用现金使用同一绝对费用函数计算，避免最低佣金导致现金透支。
- [改进] 组合回测逐笔成交新增执行价、佣金、税费、滑点成本和净现金影响，逐期及汇总指标分别披露佣金、税费和滑点成本；Agent 控制台支持填写并展示总成本。
- [测试] 新增最低佣金限制可买整手、卖出单边税费、净现金对账、非法近似模式及 API/Web 字段映射回归。
- [新功能] AlphaSift 历史组合回测支持按股票代码配置任意目标权益权重，未配置比例保留为现金，只有当期策略 Top K 已选中的配置标的才参与调仓。
- [改进] 组合回测逐期返回实际目标权重和“已配置但未入选”标的审计；任意权重仅允许使用现金账本模式，非法单票权重或总和超过 100% 时明确拒绝。
- [测试] 新增任意目标权重、现金留存、未入选标的审计、非法权重/API 转发及 Agent 控制台输入展示回归。
...
- [测试] 新增高相关候选排除、负相关候选保留、历史不足阻断、配置映射和 Agent 相关性审计展示回归。
- [新功能] 自动选股 Agent 新增跨运行前瞻质量状态机，按持久化候选的严格样本外结果生成 `insufficient_evidence`、`healthy`、`guarded`、`blocked` 或 `unavailable` 状态并记录状态迁移。
- [改进] 新增默认关闭的 `auto_cross_run_quality_gate_enabled` 买入门禁；成熟样本胜率低于阈值或评价不可用时阻断新增买入，但保留自动卖出风险处置，证据不足只审计不阻断。
- [改进] 模拟交易页可配置前瞻周期、成熟样本数、最低胜率和扫描上限，Agent 控制台展示成熟样本、胜率、原因及恢复/恶化迁移。
- [测试] 新增跨运行证据不足、恶化、受控、恢复、评价失败关闭、卖出保留和 API/Web 配置映射回归。
- [新功能] 自动选股 Agent 新增确定性统一数据质量评分，按筛选完整性、候选字段覆盖和 AlphaSift snapshot/daily 来源健康汇总 0~100 分及等级。
- [改进] 新增可选 `auto_min_data_quality_score` 门禁；配置后低于阈值的整轮候选会以 `data_quality_score_below_threshold` fail-closed，并记录分数组成和告警阈值。
- [改进] 候选规则复核新增候选质量分，模拟交易页可配置最低质量分，Agent 控制台在运行详情中同时展示质量状态和分数。
- [测试] 新增健康来源满分、部分候选/来源失败降分、阈值阻断、告警参数、API/Web 映射和 Agent 质量分展示回归。
- [新功能] 自动模拟交易的组合分配新增 `score_inverse_volatility_20d` 方法，按候选评分除以同一决策时点的 20 日年化波动率生成风险调整权重。
- [改进] 逆波动率分配对缺失或非法风险字段逐票 fail-closed，并审计波动率来源、原始值、下限调整、有效波动率、风险调整权重和排除原因。
- [改进] 模拟交易页新增组合分配方法和波动率下限设置，Agent 控制台同步展示候选 20 日波动率、有效波动率和风险权重。
- [测试] 新增低波动优先、风险字段缺失阻断、零波动率下限、单票上限再分配、API/Web 配置映射和 Agent 风险审计展示回归。

- [新功能] 自动模拟交易新增默认关闭的评分加权组合分配，可通过 `auto_score_weighted_allocation_enabled` 和 `auto_allocation_budget` 将每轮组合预算按候选分数分配到实际买入计划。
- [改进] 组合分配同时受现金保留、每日预算、每日订单槽位、最大持仓数、单票上限、总仓位上限、行业共享上限和显式股票/行业目标约束，候选触顶后会把剩余预算重新分配给仍有空间的候选。
- [改进] Agent run 与候选仓位计划新增 `portfolio_allocation` 审计，记录评分权重、候选上限、分配金额、约束、排除原因、A 股整手后的可执行预算和残差；Agent 控制台同步展示组合分配摘要与候选权重。
- [测试] 新增评分比例、单票上限再分配、行业共享空间、每日预算、每日订单槽位和 A 股整手残差回归，并覆盖 API/Web 配置映射与 Agent 控制台展示。
- [改进] 自动买入在配置股票或行业目标权重时按剩余目标缺口缩减单笔预算，已有持仓即使达到最大持仓数或启用跳过已有持仓，也可继续补足显式股票目标。
- [改进] 目标缺口预算统一经过账户基准币种换算和交易市场整手校验；不足最小整手时以 `target_weight_below_min_lot` 跳过，并在订单原始载荷记录目标、当前值、剩余缺口和可执行金额。
- [测试] 新增股票目标补仓、目标已达、行业剩余缺口不足整手以及跨币种目标缺口与每日预算联动回归。
- [改进] 模拟交易完整状态新增持仓行业归属覆盖诊断，返回已解析/缺失持仓数、覆盖率、缺失代码和行业市值分布；配置行业上限或目标权重时，覆盖不完整会在系统健康矩阵中明确标为阻断。
- [测试] 新增行业归属部分覆盖、轻量状态未评估、首次账户快照不可用及 Web 可用性诊断展示回归。
- [修复] 模拟交易账户最大回撤改为按账户持久化的已观测权益峰值计算，不再只比较初始资金；账户先盈利后回撤且仍高于初始资金时也能正确阻断新买入。
- [改进] Agent run 审计和完整状态新增账户权益峰值、当前权益、当前回撤、阈值与计算口径，Web 可用性诊断同步展示账户回撤组件。
- [测试] 新增权益峰值跨 service 重建持久化、峰值回撤阻断和系统健康/Web 展示回归。
- [新功能] Agent 跨 run 数据质量趋势新增来源级健康观测，按 `group/source` 汇总观测数、降级次数/比例、最新状态、最新/最大失败计数和最后观测时间。
- [改进] Agent 控制台新增“来源健康观测”表，按降级率优先展示 snapshot/daily 具体来源；旧 run 没有 `source_health` 时显示无快照，不推断为健康。
- [测试] 新增来源健康快照 API 聚合、snake/camel 映射及 Agent 控制台来源趋势展示回归。
- [新功能] 每轮自动选股 Agent 计划新增确定性 `recent_run_context`，汇总同策略/市场最近 5 次 run 的状态、数据质量、候选/计划/成交/跳过、成交率和连续失败，并写入审计。
- [改进] LLM 动态计划提示词升级为 `vnpy_paper_dynamic_agent_plan_v2`，复用与规则 Agent 相同的最近运行上下文；Agent 控制台计划卡展示最近运行数、历史成交率和最近状态。
- [测试] 新增跨 service 历史 run 汇总、连续失败、LLM 提示词上下文和 Web 计划展示回归。
- [新功能] 新增时间点选股因子快照表及 `POST /api/v1/alphasift/replay/snapshots`、`GET /api/v1/alphasift/replay/compatibility`、`POST /api/v1/alphasift/replay/run`，支持按指定交易日执行 AlphaSift 硬过滤与因子评分重放。
- [改进] 历史策略重放新增硬过滤与评分字段双层覆盖率门禁，缺少历史换手率、估值、市值或技术因子时 fail-closed，禁止回退当前快照或启用 LLM 排序。
- [新功能] Agent 控制台新增 AlphaSift 时间点策略重放面板，可检查快照股票数、字段覆盖率、缺失字段并查看单日重放候选。
- [测试] 新增因子快照幂等更新、日期隔离、字段缺失审计、策略重放排序及 API/Web 契约回归。
- [新功能] 新增有界历史因子后台采集任务，针对调用方提供的时间点股票池，通过 AKShare 历史日线和百度估值序列补齐换手率、PE/PB、市值及严格截至快照日计算的技术因子。
- [新功能] 新增 `POST /api/v1/alphasift/replay/portfolio-backtest`，按多日期策略重放结果执行下一交易日开盘等权组合回测，支持双边手续费、滑点、基准收益、超额收益、最大回撤和周期胜率。
- [改进] Agent 控制台时间点重放面板新增历史因子采集和跨日期组合回测，展示快照写入错误、组合/超额收益、回撤、数据覆盖及逐期权益。
- [测试] 新增历史估值 as-of 防未来数据、采集降级审计、组合成交时点、成本、基准、缺行情覆盖和后台任务 API/Web 回归。
- [改进] 时间点组合回测改为保留相邻快照重复入选的持仓，仅对新进入、被剔除和最终退出的头寸计入手续费与滑点，并返回逐期/累计换手率、延续持仓数和买卖计数。
- [改进] A 股组合回测默认启用可交易性门禁，按主板、创业板/科创板、北交所及 ST 名称阈值识别涨停不可买、跌停不可卖，并以停牌成交量阻断买卖；无法完成目标调仓时降低覆盖率而不伪造成交。
- [测试] 新增连续入选不重复计费、真实换手统计、持仓延续不受当日涨停误阻断，以及停牌/涨停买入/跌停卖出拒绝回归。
- [改进] 时间点组合回测在卖出因停牌或跌停受阻时按期末价格计值、不收取卖出成本，并将被动仓位带入下一调仓期继续尝试；逐期结果新增实际持仓数和被动持仓数，汇总指标新增期末未平仓数量。
- [测试] 新增卖出受阻仓位跨期延续并在后续可交易调仓日退出的回归覆盖，Web 逐期结果同步展示被动持仓数量。
- [新功能] 新增 `GET /api/v1/alphasift/replay/universe`，基于 Tushare `stock_basic` 上市/退市生命周期按指定日期还原 A 股成员，包含当时有效的已退市股票且不回退当前股票池；Agent 控制台新增历史股票池校验入口和成员数/截断状态展示。
- [改进] Tushare 数据源新增上市、退市和暂停上市状态合并读取；历史股票池接口明确披露权限依赖、结果截断以及名称/行业不具备时间点语义。
- [测试] 新增历史股票池上市/退市日期过滤、截断、数据源不可用、Tushare 多状态合并和 API 参数转发回归。
- [新功能] 新增持久化全市场历史因子采集作业及创建、列表、详情、恢复 API；每个快照日期独立还原历史 A 股成员，按日期和股票分批采集并在每批成功后保存游标、累计写入量、源错误和心跳。
- [改进] 全市场采集恢复使用任务租约防止强制接管后的旧线程重复推进检查点；Agent 控制台可创建作业、轮询进度、刷新后恢复最近作业，并对失败或遗留 processing 作业从检查点继续。
- [测试] 新增逐日期股票池隔离、生命周期元数据单次加载、分批完成、第二批故障恢复、任务租约接管、持久化作业列表及 API/Web 恢复流程回归。
- [新功能] AlphaSift 组合回测新增 `cash_ledger` 账户模式并成为 API/Web 默认值，按可用现金、A 股 100 股整手、双边成本和每期等权目标数量执行真实调仓，返回逐期现金、市值、持仓、成交和阻断明细。
- [改进] 现金账本会卖出漂移后的超配仓位并补买低配仓位，买入受现金和整手限制；最终停牌/跌停或缺行情不会伪造平仓，期末权益保留未平仓市值。旧 `equal_weight_approximation` 模式继续保留用于兼容。
- [测试] 新增现金非负、整手数量、跨期目标权重再平衡、最终跌停被动持仓和 API/Web 默认账本模式回归。
- [新功能] Agent 控制台新增候选前瞻评价，可按当前策略、市场和运行时间筛选，展示 1/5/10/20 个交易日覆盖率、胜率、平均/中位收益以及平均最大有利/不利波动。
- [改进] 新增 `POST /api/v1/vnpy-paper/agent-runs/backtest`，基于持久化候选价格和严格晚于决策日期的本地日线计算样本外结果，默认不联网补数、不重跑策略且不触发交易。
- [测试] 新增 Agent 前瞻评价的防未来数据、多周期矩阵、策略/日期筛选、跳过候选口径、缺价/缺行情审计以及 API/Web 映射回归。
