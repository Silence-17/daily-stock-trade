...
>
> The default-off intraday gate uses directional market indices and, for A shares, live advance/decline breadth. Strict provider-time verification is enabled by default: timestamps must be timezone-aware, no more than 15 minutes old, and not more than 60 seconds in the future. Efinance now restores EastMoney's real update time as UTC for stocks, ETFs, indices, and breadth; breadth publishes its earliest as-of only when every valid counted row has a timestamp. AkShare's direct Sina and Tencent single-quote fallbacks also retain explicit A-share provider time, while AkShare batch index/breadth endpoints remain unverifiable rather than substituting local fetch time. Strict breadth evidence must explicitly report exactly 100% timestamp coverage; missing or any non-100% coverage fails closed. Cross-provider and cross-exchange quotes are still not synchronized, so the linked-market gate must not be interpreted as exchange-level synchronized data.
>
