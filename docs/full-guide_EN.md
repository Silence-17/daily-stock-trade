...
> Paper-trading cash per order, daily budgets, minimum cash, and amount-based exposure limits are denominated in the account base currency. Non-CN orders use the Portfolio FX table to derive the quote-currency notional. Missing or stale FX blocks new buys with `fx_rate_unavailable` but never blocks risk-reducing sells; Agent audit data retains base amount, quote amount, rate, and source. Refresh rates from the Portfolio page or `POST /api/v1/portfolio/fx/refresh`.
...
> To accumulate production-shaped calibration samples unattended, explicitly set `DSA_AGENT_CALIBRATION_SHADOW_ENABLED=true` and `DSA_AGENT_CALIBRATION_SHADOW_PAIRS=cn:dual_low`; use `DSA_AGENT_CALIBRATION_SHADOW_INTERVAL_MINUTES` and `DSA_AGENT_CALIBRATION_SHADOW_MAX_RESULTS` for cadence and candidate count. The task starts 300 seconds after initial registration and creates only separate `agent_calibration_shadow` dry-run records. It never submits orders or changes saved automation settings. Every `market:strategy` pair must match AlphaSift's reported `market_scope`; incompatible pairs fail explicitly. AlphaSift 0.2.0 currently ships only CN-scoped strategies and its screen pipeline supports CN/US, so do not configure US/HK by default.
