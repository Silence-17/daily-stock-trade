...
>
> Automatic buys also support the default-off `auto_score_weighted_allocation_enabled` switch. When enabled, `auto_allocation_budget` (or `auto_cash_per_order` when omitted) becomes the round-level portfolio budget. `auto_allocation_method` selects either score weighting or score/inverse-volatility weighting from AlphaSift's point-in-time `volatility_20d_pct`. Risk mode uses `auto_risk_volatility_floor_pct` to cap extreme inverse-volatility leverage and excludes candidates whose risk input is missing or invalid instead of silently falling back. Cash reserve, remaining daily budget/order slots, maximum positions, and per-order, single-position, industry, total-exposure, and explicit-target caps constrain the round and trigger deterministic redistribution. Agent-run and candidate position-plan audits expose the risk source, raw/effective volatility, adjusted weight, allocation, executable lot-sized budget, and residual. This is a deterministic diagonal risk model, not yet a covariance/correlation or arbitrary-objective optimizer.
