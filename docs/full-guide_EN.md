...
>
> Candidate realtime quotes now use provider-level recovery on top of the configured source order. Three consecutive exceptions for the same market/provider open a five-minute cooldown and route requests to later sources. Exactly one half-open probe is admitted after cooldown; success closes the circuit and failure starts a new cooldown. A symbol-specific empty quote does not count as a provider failure. Valid state is atomically stored in `realtime_source_health.json` beside `DATABASE_PATH`; API startup restores failure/open state saved within 24 hours but never restores a half-open probe lease. Corrupt or stale files only emit a warning and do not block startup. Every Agent run also persists state, failure count, remaining cooldown, and a sanitized error under `diagnostics.source_routing.candidate_context.quote`, and the Agent console renders it beside snapshot routing.
