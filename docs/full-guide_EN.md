...
> Automatic sell risk controls support `auto_no_progress_days` and `auto_no_progress_min_return_pct`: when a holding reaches the configured age and its unrealized return is not above the threshold, the service creates a `no_progress_timeout` sell plan. An empty return threshold is treated as 0%. Holding age is derived by paging through the complete trade history and replaying the currently open FIFO lots, so a full exit followed by re-entry resets the clock. Risk-reducing sells run before data-quality, account, and market buy gates; an accepted sell remains represented in the Agent run audit when later buys are blocked.
