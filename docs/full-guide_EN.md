...
>
> The paper-trading page can independently enable market-breadth and hotspot-retreat gates. Breadth checks the latest persisted `MarketLightSnapshot.dimensions.breadth.score`; hotspot retreat compares `dimensions.limit.score` across the latest two snapshots as a deterministic limit-up/limit-down strength proxy. A trigger or missing required snapshot blocks new buys only and records all inputs in `diagnostics.market_context_risk` plus the Agent timeline. Stop-loss, take-profit, and other sell-side risk reduction still run first. These gates consume persisted post-market snapshots and are not intraday breadth signals.
>
