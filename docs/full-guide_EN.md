...
> The Agent console reads `GET /api/v1/vnpy-paper/agent-runs/daily-summary` to show a daily Agent summary with run counts, candidate/planned/submitted/skipped counts, status distribution, execution modes, data quality, Agent and LLM review status counts, review-quality status/flag counts and average score, workflow status/stage counts, top skip reasons, and active symbols. Candidate decision details show the versioned `order_result.strategy_evidence` contract with the effective strategy, rank, screen/final scores, bounded upstream rule matches, factor decomposition, and evidence status, plus the rule-based pre-trade `order_result.agent_review` status and summary. Missing rule/factor details are marked `summary_only` rather than inferred from an aggregate score; legacy runs without the field keep their original rationale/review display. The Agent plan card shows the rule-derived plan mode, risk level, and optional `llm_dynamic_plan` status. Run details derive `diagnostics.agent_workflow` to show the current stage, overall status, and next action across plan, data-quality, candidate-review, trade-plan, and execution stages; `diagnostics.agent_summary.review_quality` shows the run-level review status, score, coverage, risk flags, and whether manual confirmation is recommended. Run details can also manually call `POST /api/v1/vnpy-paper/agent-runs/{run_uid}/llm-recap` to generate an optional LLM recap stored in `diagnostics.llm_recap`; this is audit-only, does not submit orders, and does not change trade-plan status. The paper-trading setting `auto_llm_plan_enabled` is off by default; when enabled, the service calls the LLM before AlphaSift screening to create a dynamic run plan, allowing only known strategy ids and only tightening max results, cash per order, and min score within saved limits. `auto_llm_review_enabled` is also off by default; when enabled, the service calls the LLM after rule gates, stores `order_result.llm_review`, and skips the candidate on `blocked` or review-call failure. Both `llm_dynamic_plan` and `llm_review` record `prompt_version` and `evaluator_version`, and the Agent console shows those audit versions in the matching sections.
