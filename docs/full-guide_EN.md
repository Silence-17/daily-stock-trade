...
> `GET /api/v1/vnpy-paper/status` now returns `diagnostics.auto_trade_readiness`, a structured automated-trading readiness summary with overall status, next action, blockers, warnings, and component states for the local ledger, auto-trade switch, runtime scheduler, auto task, AlphaSift screening dependency, scheduling window, trading window, failure fuse, and vn.py bridge. The paper-trading page's availability diagnostics prefer this backend summary. `scheduler.loop_running` reports whether the scheduler loop is alive, while `scheduler.running` still means an analysis task is currently executing; readiness uses `loop_running` to decide whether timed auto trading can keep scheduling. `diagnostics.auto_trade_readiness.timing_alignment` compares the auto-task next trigger with the trading-window open and close times. When the time gate is enforced and the service starts outside the trading window, the first auto-buy background task is delayed to the next market open so a fixed interval does not repeatedly miss the session after a post-market start.
