...
> To accumulate production-shaped calibration samples unattended, explicitly set `DSA_AGENT_CALIBRATION_SHADOW_ENABLED=true` and `DSA_AGENT_CALIBRATION_SHADOW_PAIRS=cn:dual_low,us:us_large_cap_momentum`; use `DSA_AGENT_CALIBRATION_SHADOW_INTERVAL_MINUTES` and `DSA_AGENT_CALIBRATION_SHADOW_MAX_RESULTS` for cadence and candidate count. The task starts 300 seconds after initial registration and creates only separate `agent_calibration_shadow` dry-run records. It never submits orders or changes saved automation settings. Every `market:strategy` pair must match AlphaSift's reported `market_scope`; incompatible pairs fail explicitly. DSA overlays the US strategy without modifying the installed package. AlphaSift 0.2.0 screening still supports only CN/US, so do not configure HK.
