...
Runtime connection confirmation now understands the official CTP trading and market-data channel `login_status` fields and reports connected only when every available channel is logged in. The manual `External vn.py CTP Account Soak` workflow can materialize a temporary runner-side configuration from the protected `vnpy-ctp-paper-acceptance` Environment secret, enforce strict preflight, run a no-order account soak, and retain sanitized evidence for 14 days. It is intended only for an isolated SimNow or broker paper account; production trading credentials must remain on a controlled deployment host. See [vn.py paper trading](vnpy-paper-trading.md) for the full security and setup contract.
