...
> For unattended recovery, explicitly set `VNPY_AUTO_RECONNECT_ENABLED=true` and configure a 5-to-3600-second check interval with `VNPY_AUTO_RECONNECT_INTERVAL_SECONDS`. The background monitor retries only connections that explicitly report failed or disconnected state; it never repeatedly logs in an unconfirmed asynchronous gateway. `VNPY_AUTO_RECONNECT_CONFIRMATION_GRACE_SECONDS` adds a 5-to-600-second asynchronous confirmation grace period (30 by default) so login initialization is not retried prematurely. Each retry rereads the external connection JSON, while diagnostics and system health expose only counts, timestamps, results, and the next check. All three settings require a process restart.
