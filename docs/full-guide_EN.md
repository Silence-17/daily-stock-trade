...
>
> Every rule-based Agent plan also persists `diagnostics.agent_plan.recent_run_context`, a deterministic summary of the five most recent runs with the same trigger, strategy, and market. It includes status and quality distributions, candidate/planned/submitted/skipped totals, submission rate, current failure streak, and compact run rows. This works without an LLM; dynamic-plan prompt v2 reuses the same context, and the Agent console shows its recent count, historical submission rate, and latest state.
