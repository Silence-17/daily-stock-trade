...
>
> The Agent console calls `GET /api/v1/vnpy-paper/agent-runs/data-quality-trends` for 7/30/90-day cross-run quality distribution, degradation rate, warnings, source errors, and daily trends. Legacy runs without an overall quality snapshot are reported as `unknown` rather than healthy.
...
> Automatic sell risk controls also support the default-off `auto_rebalance_enabled` switch. When enabled, the service reuses single-position, total-position, and industry exposure limits; any excess exposure is converted into a `rebalance_*_exceeded` sell quantity. It can also read `auto_target_position_weights` / `auto_target_industry_weights` as target equity percentages and create `rebalance_target_position_weight_exceeded` / `rebalance_target_industry_weight_exceeded` sell plans for overweight holdings or industries. Target weights also participate in buy-side risk checks: a candidate is skipped with `target_position_weight_limit_reached` / `target_industry_weight_limit_reached` when the candidate or already planned cash would exceed the target. The raw candidate payload records `rebalance_plan`, target weight, threshold value, and excess value.
