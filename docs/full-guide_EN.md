...
>
> Automatic buys also support the default-off `auto_score_weighted_allocation_enabled` switch. When enabled, `auto_allocation_budget` (or `auto_cash_per_order` when omitted) becomes the round-level portfolio budget. It is distributed by candidate score and reallocated when a candidate reaches its per-order, single-position, industry, or explicit-target cap. Cash reserve, remaining daily budget/order slots, maximum positions, and total exposure constrain the whole round. Agent-run and candidate position-plan audits expose `portfolio_allocation`, executable lot-sized budget, and residual. This is a deterministic constrained allocator, not yet a covariance, volatility, or arbitrary-objective optimizer.
