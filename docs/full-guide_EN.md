...
> The same read-only evidence API and Agent console expose the latest transition-delivery summary: alert time, attempted/successful/failed/retryable counts, and per-channel success or error codes. An unavailable alert history degrades only the delivery status and does not hide the evidence result. This makes channel configuration failures visible without claiming that the affected channel has been repaired.
