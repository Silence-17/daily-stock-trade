...
> Automatic sell risk controls support `auto_no_progress_days` and `auto_no_progress_min_return_pct`: when a holding reaches the configured age and its unrealized return is not above the threshold, the service creates a `no_progress_timeout` sell plan. An empty return threshold is treated as 0%.
