...
For Compose, run `DSA_INCLUDE_VNPY_DOCKER=true docker compose -f docker/docker-compose.yml build`. The optional build installs `requirements-vnpy.txt` and imports the vn.py event/trading modules plus the built-in gateway inside the image; an import failure aborts the build. External plugins can be declared once with `VNPY_GATEWAY_PLUGINS_JSON='[{"package":"<gateway-package>==<version>","module":"<gateway_module>"}]'`; local setup, Docker, and Desktop builds install and verify the modules, while Desktop also collects them and runs a post-freeze import probe. The manifest rejects URLs, markers, duplicate entries, and invalid modules and must never contain credentials. Account connection JSON remains outside the repository, and production acceptance must use the external-gateway soak gate rather than treating `DSA_SIM` as real-account evidence.

The external-account target is A-share XTP rather than futures CTP/SimNow. Official `vnpy_xtp==2.2.32.2.3` is the verified Windows/Python 3.13 sample, with `VNPY_GATEWAY_CLASS=vnpy_xtp:XtpGateway`, `VNPY_GATEWAY_NAME=XTP`, and native SSE/SZSE declarations. Credential-free acceptance proves installation, MainEngine registration, dual-channel state, and frozen-artifact import only. A missing repository-external connection JSON must report `connect_settings_required`; present default fields with null or blank values report `default_setting_values_empty`, and neither case attempts a connection.

Runtime connection confirmation reads XTP trading and market-data `login_status` fields and reports connected only when both channels are logged in. After the account owner requests a stock-type test account from the official XTP portal, they add `VNPY_XTP_CONNECT_SETTINGS_JSON` to the `vnpy-xtp-paper-acceptance` Environment and run `External vn.py XTP A-share Account Soak` from `main`. The workflow validates the exact ten fields, client/port ranges, and protocol/log enums before installation, requires 60 continuous seconds of dual-channel connectivity, then runs a no-order soak and retains sanitized evidence for 14 days. Run `30001136306` passed the first 15-minute acceptance with 180/180 connected samples, 240 account events, 4,320 position events, and zero order/trade events. Production trading credentials must remain on a controlled deployment host. See [vn.py paper trading](vnpy-paper-trading.md) for the full security and setup contract.
...
For the optional vn.py runtime, use an isolated Python 3.13 environment. On Windows PowerShell run `.\scripts\setup_vnpy_runtime.ps1 -PythonExecutable "python"`; set `VNPY_GATEWAY_PLUGINS_JSON` first or pass `-GatewayPluginsJson` when external plugins are required. The script rejects Python 3.14 unless explicitly overridden, installs `requirements-vnpy.txt` plus the explicit plugin manifest, and smoke-checks a real OrderRequest, EventEngine, MainEngine, and the built-in immediate-fill gateway. The setup check also runs an explicit rejection, duplicate trade-event deduplication, and three disconnect/reconnect cycles with in-flight orders, verifying that `DsaSimulatedGateway` preserves cash, positions, and order cache while booking each order exactly once. This gateway provides a credential-free Web-to-vn.py paper order and fill path. Use `scripts/check_online_agent_vnpy_e2e.py` against a running API to verify online Agent decisions, plans, fills, and Portfolio deltas; it defaults to dry-run, requires explicit permission for built-in simulated orders, supports `--isolated-account` to fill a clean temporary ledger before restoring the original account, and uses `--trigger-mode scheduler` to prove a background-scheduled run correlated with its persisted task event. Real channels still require external connection settings. Enable `VNPY_PRODUCTION_PREFLIGHT_ENABLED=true` in the API runtime so startup connect and later reconnects reject simulated gateways, repository-local settings, and missing default keys before `connect()` is called. `scripts/check_vnpy_gateway_soak.py` runs zero-connect preflight in a child process before starting its independent MainEngine; production evidence should enable both `--require-external-gateway` and `--require-all-default-keys`. Use the read-only `scripts/check_vnpy_deployed_runtime_soak.py` against the running Web/API process to gate API/runtime/connection/callback registration, new event deltas within the observation window, handler failures, contract compatibility, external gateway identity, process changes, and reconnect counter monotonicity without creating runs, plans, or orders. See [vn.py paper trading](vnpy-paper-trading.md).

For one production evidence bundle, run `scripts/check_online_agent_production_acceptance.py --output-json <external-evidence-file>`. It performs the zero-connect external-gateway preflight, observes the deployed runtime and scheduler concurrently, and then evaluates persisted CN/HK/US calibration evidence. The default 24-hour contract requires the startup production-preflight gate, new account and position events, terminal evidence for all four background tasks, and objective version `candidate-return-risk-v1`. Atomic checkpoints preserve `running`, `completed`, or `interrupted` state; the orchestrator does not connect a gateway, trigger an Agent run, or place an order. It reports `production_ready=true` only when every stage passes and rejects DSA_SIM during preflight. The first calibration-shadow run reads each market/strategy pair's latest completed sample; when every pair is still pending it waits for the latest persisted eligibility time so one run covers every pair. A new deployment or an already eligible pair retains a five-minute startup buffer, and the evidence monitor runs five minutes after the shadow task. The calibration-evidence API exposes the earliest per-market eligibility as `next_eligible_at`, whole-batch eligibility as `all_eligible_at`, and the runtime task time as `next_scheduled_at`; the page prefers the last value.
...
>
> `python main.py --serve-only` / `--webui-only` only disables the daily analysis job. Independent background tasks such as Event Monitor and automatic paper trading can still run through the Web/API runtime scheduler when their own feature switches are enabled.
> When runtime scheduler configuration is rebuilt, process-local locks are reused by task name. If an older generation of the same task is still running, the replacement records a persisted `task_already_running` skip instead of executing concurrently. Task status exposes `overlap_guarded` and `previous_generation_running`, and the Web paper-trading page identifies an older generation that is still finishing.
>
...
> The runtime scheduler status now includes recent `task_events`; the Web paper-trading page uses them as a background task log, and `GET /api/v1/vnpy-paper/task-events` can filter persisted recent automatic-buy, automatic-retry, and event-monitor events by task name and started/completed/skipped/failed status, so recent diagnostics survive API process restarts.
> The full snapshot path of `GET /api/v1/vnpy-paper/status` uses a 10-second in-process TTL cache to avoid replaying the Portfolio ledger and refetching quote valuation on every refresh. Local fills, vn.py trade callbacks, account reset, and account restore invalidate the cache; diagnostics expose `snapshot_cache_hit` and `snapshot_cache_ttl_seconds`.
> Full system health also summarizes position-price coverage and freshness, price-source/provider distribution, missing, stale, or unknown-state symbols, and the observed price-date range. These metrics only aggregate the current Portfolio snapshot and do not trigger extra quote requests. The Web availability panel renders the same coverage and source summary.
> Readiness and system health also expose the persisted latest automatic-run outcome, including its timestamp/id, original skip or failure reason, and candidate/submission counts. This survives scheduler task-event cleanup; a historical non-success is reported as a warning and does not become a current hard blocker by itself.
> `diagnostics.backend` reports the API version, vn.py paper contract, optional build id, Python version, and process start time. The current Web client requires contract 3 and marks an omitted or older contract as needing an update. Deployments may set `DSA_BUILD_ID`; common CI and cloud Git-commit variables are detected automatically.
> `diagnostics.vnpy_runtime.connect` separates `request_accepted` from `connected`: the former only means `MainEngine.connect()` returned, while the latter requires confirmation from a gateway status hook. Connect exceptions degrade to `connect_failed` without aborting API startup. An asynchronous gateway whose state cannot be confirmed produces a warning; a gateway that explicitly reports disconnection is blocked and new submissions are skipped with `vnpy_gateway_disconnected`. Supported hooks are refreshed on every status read.
> For unattended recovery, explicitly set `VNPY_AUTO_RECONNECT_ENABLED=true` and configure a 5-to-3600-second base check interval with `VNPY_AUTO_RECONNECT_INTERVAL_SECONDS`. Consecutive failures exponentially increase the delay up to `VNPY_AUTO_RECONNECT_MAX_INTERVAL_SECONDS` (300 seconds by default and never below the base interval), while a confirmed recovery resets it. The background monitor retries only connections that explicitly report failed or disconnected state; it never repeatedly logs in an unconfirmed asynchronous gateway. `VNPY_AUTO_RECONNECT_CONFIRMATION_GRACE_SECONDS` adds a 5-to-600-second asynchronous confirmation grace period (30 by default) so login initialization is not retried prematurely. Each retry rereads the external connection JSON, while diagnostics and system health expose only counts, consecutive failures, base/current/maximum intervals, timestamps, results, and the next check. All four settings require a process restart.
> An operator can also use the Web **Reconnect gateway** action or `POST /api/v1/vnpy-paper/gateway/reconnect` for one immediate safe attempt without enabling the background monitor. A connected, confirmation-pending, or grace-protected gateway is not logged in again. The action only restores the gateway connection and never runs stock selection or submits an order.
> The first reconnect failure, the first transition to the maximum backoff, and recovery are sent through a separate event queue into the existing alert history and notification routing with cooldown deduplication. The Web action refreshes alert history after completion. Event diagnostics never store the connection-file path or credentials.
> The Web paper-trading page also calls `GET /api/v1/vnpy-paper/task-health` to show a task-health panel for automatic buys and recovery scans, including task registration, running/disabled state, persisted latest failed/skipped reason, and next run time.
> Pausing scheduled automatic buys only unregisters the new-buy task. While paper trading itself remains enabled, `vnpy_paper_auto_retry` runs once when registered and then keeps recovering existing orders every one to five minutes; in paused mode it reconciles and expires orders but never resubmits failed plans. The API lifespan binds its created or injected MainEngine/EventEngine to scheduler services, so Web requests and background tasks share the same vn.py runtime.
> With the trading-time gate enabled, automatic-trading intervals of 24 hours or longer are anchored five minutes after the relevant market window opens instead of inheriting an arbitrary API process start time. If no formal run has occurred in the current session, startup selects the current or next segment with at least five minutes of preparation time; if a formal run already occurred, it waits for the next trading session to avoid duplicate orders after a restart. `auto_trade_readiness.timing_alignment` projects the window for the task's actual trigger date. Custom intervals below 24 hours keep their existing interval semantics.
> Paper-trading cash per order, daily budgets, minimum cash, and amount-based exposure limits are denominated in the account base currency. Before the first cross-currency automatic buy, the service refreshes the required pair on demand, preferring yfinance and falling back to a dated Frankfurter reference rate. Rates can also be refreshed from the Portfolio page or `POST /api/v1/portfolio/fx/refresh`. Missing or stale FX still blocks new buys with `fx_rate_unavailable` but never blocks risk-reducing sells; Agent audit data retains base amount, quote amount, rate, and source.
> The Web paper-trading page also calls `GET /api/v1/vnpy-paper/task-event-summary` for a task-trend summary with recent completed/skipped/failed/started distribution, per-task failure rate, average duration, and latest failed/skipped time.
>
> Its long-window stability view calls `GET /api/v1/vnpy-paper/task-metrics` for 7/30/90-day terminal-run success, failure, and skip rates, average/P95 duration, current failure streak, per-task metrics, and daily trends. `started` events are counted separately and excluded from the rate denominator.
>
> The consecutive-failure fuse stays latched by default and can always be reset manually. For unattended recovery, explicitly enable cooldown recovery on the paper-trading page and set its duration. The open timestamp is persisted; only one probe run is released when the cooldown expires, and a failed probe opens a fresh fuse cycle. Status shows the next probe time and automatic releases are recorded in system alert history.
>
> Automated AlphaSift LLM ranking has a separate cross-run circuit breaker. By default, one timeout, invalid structured result, or explicit ranking failure makes subsequent automated runs use deterministic `screen_score` during a 60-minute cooldown. Only one 45-second half-open probe, capped by the normal request budget, is allowed after cooldown; concurrent runs skip LLM while that probe holds a five-minute lease. An abandoned probe is treated as failed after its lease expires and starts a fresh cooldown. Tune this with `VNPY_AUTO_ALPHASIFT_LLM_CIRCUIT_BREAKER_ENABLED`, `VNPY_AUTO_ALPHASIFT_LLM_FAILURE_THRESHOLD`, `VNPY_AUTO_ALPHASIFT_LLM_COOLDOWN_MINUTES`, and `VNPY_AUTO_ALPHASIFT_LLM_PROBE_TIMEOUT_SEC`. Run diagnostics and the Agent timeline retain state transitions, failure reasons, and probe results. Interactive screening is unaffected.
> When AlphaSift structured ranking uses a DashScope OpenAI-compatible channel with a Qwen 3 model, the request-scoped bridge disables thinking by default so the bounded request budget is spent producing complete JSON. Ranking output defaults to 3072 tokens, enough for the current five-candidate contract. An explicit `extra_body` in a LiteLLM `model_list` and an explicit `LLM_MAX_TOKENS` take precedence. This adjustment does not affect ordinary analysis or other model calls.
>
> The paper-trading page can independently enable market-light, market-breadth, and hotspot-retreat gates. Breadth checks the latest persisted `MarketLightSnapshot.dimensions.breadth.score`; hotspot retreat compares `dimensions.limit.score` across the latest two snapshots as a deterministic limit-up/limit-down strength proxy. When any gate is enabled, the latest snapshot must exist, load successfully, and have a valid date. The page configures a 1-to-30-calendar-day freshness limit, defaulting to seven days; future-dated or stale snapshots fail closed. A trigger or unavailable evidence blocks new buys only and records the snapshot age and all inputs in `diagnostics.market_context_risk` plus the Agent timeline. Stop-loss, take-profit, and other sell-side risk reduction still run first. These gates consume persisted post-market snapshots and are not intraday breadth signals.
>
> The default-off intraday gate uses directional market indices and, for A shares, live advance/decline breadth. Strict provider-time verification is enabled by default: timestamps must be timezone-aware, no more than 15 minutes old, and not more than 60 seconds in the future. Efinance now restores EastMoney's real update time as UTC for stocks, ETFs, indices, and breadth; breadth publishes its earliest as-of only when every valid counted row has a timestamp. When efinance is unavailable, AStockDataFetcher runs before AkShare batch endpoints and reads EastMoney's raw `f124/f297`; it accepts an index or breadth fallback only when the API total exactly matches the returned row count. AkShare's direct Sina and Tencent single-quote fallbacks also retain explicit A-share provider time, while AkShare batch index/breadth endpoints remain unverifiable rather than substituting local fetch time. Strict breadth evidence must explicitly report exactly 100% timestamp coverage; missing or any non-100% coverage fails closed. Linked realtime indices must have fresh provider time and remain within a 120-second maximum skew; closed-session bars are deliberately excluded from that comparison and remain non-synchronized background context.
>
> The Agent console calls `GET /api/v1/vnpy-paper/agent-runs/data-quality-trends` for 7/30/90-day cross-run quality distribution, degradation rate, warnings, source errors, daily trends, and per-source health observations for snapshot/daily providers plus candidate-context `quote/fund_flow/news`. Each source reports observation count, degraded count/rate, latest state, and latest/maximum failure count. Legacy runs without overall quality or source-health snapshots are not inferred to be healthy.
>
> Candidate realtime quotes and capital flow now use provider-level recovery. Quotes retain the configured source order; capital flow uses independent THS/AkShare paths in `tushare_ths -> akshare` order when Tushare is available, and transparently falls back when the token is absent or lacks permission. Three consecutive exceptions for the same market/provider open a five-minute cooldown and route requests to later sources. Exactly one half-open probe is admitted after cooldown; success closes the circuit and failure starts a new cooldown. A symbol-specific empty result does not count as a provider failure. Valid state is atomically stored in `provider_source_health.json` beside `DATABASE_PATH`; API startup restores failure/open state saved within 24 hours but never restores a half-open probe lease. The legacy `realtime_source_health.json` is migrated automatically, while corrupt or stale files only emit a warning. Tushare THS amounts are converted from CNY 10K to CNY and the 10-day value is summed from the latest ten daily net amounts. Every Agent run persists both routes under `diagnostics.source_routing.candidate_context.quote/fund_flow`, and the Agent console renders them beside snapshot routing.
> Persisted background task events are kept for 30 days by default. Tune this with `DSA_RUNTIME_SCHEDULER_TASK_EVENT_RETENTION_DAYS` and `DSA_RUNTIME_SCHEDULER_TASK_EVENT_CLEANUP_INTERVAL_SECONDS`; set retention days to `0` to disable automatic cleanup. This only removes `runtime_scheduler_task_events` task logs and does not delete Agent runs, trade plans, Portfolio trades, or cash ledger rows.
> The Web paper-trading page also calls `GET /api/v1/vnpy-paper/accounts?include_inactive=true` to show paper account history for the current account and archived reset ledgers, with filters for all, current, archived, and active non-current accounts. A non-current `vnpy_paper` account can be restored/switched after confirmation through `POST /api/v1/vnpy-paper/accounts/{account_id}/restore`; the backend archives the previous current paper account, clears trailing-stop peaks, and keeps historical ledger rows intact.
> `POST /api/v1/vnpy-paper/accounts/archived/cleanup` provides non-destructive bulk cleanup: it only hides non-current inactive `vnpy_paper` accounts from the default history view while keeping Portfolio accounts, trades, and cash ledgers intact. `GET /accounts?...&include_hidden=true` can still audit hidden accounts, and restoring a hidden account removes its hidden marker.
> The Web view for `GET /api/v1/vnpy-paper/performance` includes a performance summary, equity curve, and window-level performance matrix. The matrix expands strategy/industry sample counts, plan counts, fills, skips, filled cash amount, and fill rate for the current `run_limit` plus optional `created_from` / `created_to` Agent run creation-time window; it is for local paper-trading review, not a full historical-market backtest.
> The same endpoint also returns `daily_returns` and `monthly_returns`; the Web paper-trading page shows daily-return and monthly-return tables based on each day/month's last paper-equity point, including PnL, return, cumulative return, drawdown, and trade count. This improves paper-ledger review granularity but is still not a full backtest NAV curve.
> `GET /api/v1/vnpy-paper/trade-plans/recovery-summary` returns a read-only trade-plan recovery matrix. The Web paper-trading page uses it to show active submitted plans, suspected stuck plans, cancellable plans, retry-due plans, retry cooldowns, and retry-limit states; the endpoint does not mutate order state.
> `POST /api/v1/vnpy-paper/trade-plans/recovery/run` can run one bounded recovery scan after page confirmation. An active plan becomes eligible for injected `MainEngine.get_order` / `get_all_trades` reconciliation after a 60-second grace period. An observed active order older than 30 minutes receives one automatic cancel request; reconciliation updates do not reset the creation-time clock or regress `cancel_requested` to `submitted`. A cancel request has its own 30-minute timeout anchored at `raw.cancel.requested_at`. A plan with no gateway evidence waits until the original 30-minute timeout before safe archival. In addition to timeout, retry, submission, skip, and failure counts, the response returns `cancel_requested_count`, `reconciled_count`, `protected_count`, and `reconciliation_failed_count`.
> `vnpy_paper` trade callbacks are accumulated idempotently by `vt_tradeid`; one order can remain `part_filled` across multiple fills and becomes `filled` only after cumulative quantity reaches the plan target. Explicit gateway cancellation, rejection, or failure and all order/partial-fill/cancel timeout states are never automatically resubmitted. A MainEngine query failure also protects the active plan. A fill arriving during a cancel request or after timeout archival is still booked against the original `vt_orderid`, so a real fill is not lost.
> Agent-run detail dynamically derives `portfolio_change` from persisted trade plans and Portfolio `trade_id` evidence. It groups actual buys, sells, net quantity, and notional by symbol and distinguishes booked changes, pending reports, plan-only runs, and unchanged runs. Submitted orders and order-level partial-fill reports that have not reached the local ledger are not presented as holdings changes. Once a partial, late, approved, or recovered fill is booked, detail, timeline, both Web views, and JSON export read the updated result without freezing an account snapshot.
> Automatic sell risk controls support `auto_sell_position_pct` for per-exit position sizing. When empty, exits still sell the whole position; when set to `50`, stop-loss, take-profit, trailing-stop, or max-holding-days triggers submit a sell plan for 50% of the current position and record `sizing_method=position_pct`, the position quantity, and the sell percentage in the Agent audit `position_plan`.
> Automatic sell risk controls support `auto_no_progress_days` and `auto_no_progress_min_return_pct`: when a holding reaches the configured age and its unrealized return is not above the threshold, the service creates a `no_progress_timeout` sell plan. An empty return threshold is treated as 0%. Holding age is derived by paging through complete trade and split histories and replaying the currently open FIFO lots with corporate actions ordered before same-day trades, so a full exit followed by re-entry resets the clock. If split history is unavailable, holding age remains unknown rather than triggering a time-based exit from incomplete evidence. Risk-reducing sells run before data-quality, account, and market buy gates; an accepted sell remains represented in the Agent run audit when later buys are blocked.
> Automatic sell risk controls also support `auto_signal_exit_enabled`. When enabled, the service reads active `DecisionSignal` records for current holdings and turns defensive `sell/reduce/avoid` signals into `strategy_invalidated` sell plans, recording the signal summary in the raw candidate payload and Agent risk audit.
> Automatic sell risk controls also support the default-off `auto_rebalance_enabled` switch. When enabled, the service reuses single-position, total-position, and industry exposure limits; any excess exposure is converted into a `rebalance_*_exceeded` sell quantity. It can also read `auto_target_position_weights` / `auto_target_industry_weights` as target equity percentages and create `rebalance_target_position_weight_exceeded` / `rebalance_target_industry_weight_exceeded` sell plans for overweight holdings or industries. On the buy side, the order budget is capped by the smallest remaining stock or industry target gap. An existing position may be replenished toward an explicit stock target; after FX conversion the amount is rounded down to an executable market lot, or skipped with `target_weight_below_min_lot`. The raw candidate payload records `target_weight_sizing`, `rebalance_plan`, target/current values, remaining gap, and executable amount.
>
> Automatic buys also support the default-off `auto_score_weighted_allocation_enabled` switch. When enabled, `auto_allocation_budget` (or `auto_cash_per_order` when omitted) becomes the round-level portfolio budget. `auto_allocation_method` supports score weighting, score/inverse-20-day-volatility weighting, sequential pairwise-correlation constraints, and `target_tracking_min_variance_20d`. The minimum-variance method builds a covariance matrix from local trailing returns no later than the run date and balances an explicit remaining position-target vector (or candidate-score targets) against `auto_covariance_risk_penalty`; insufficient history fails closed. Cash reserve, remaining daily budget/order slots, maximum positions, and per-order, single-position, industry, total-exposure, explicit-target-gap, and market-lot constraints still apply. Agent-run and candidate position-plan audits expose the target vector, covariance matrix, convergence state, risk penalty, executable allocation, and residual.
> Every stock-selection run also receives a deterministic 0-100 data-quality score based on screen integrity, candidate coverage, and observed snapshot/daily plus candidate-context `quote/fund_flow/news` source health. `auto_min_data_quality_score` defaults to 60, and a missing or `null` legacy value is normalized to 60; a lower-scoring round fails closed as `data_quality_score_below_threshold`. Set it explicitly to 0 for audit-only scoring. Unobserved source-health telemetry scores 70 instead of being assumed healthy, while the existing `stale/unavailable` hard gate always remains active and cannot be bypassed by 0. A failed candidate-context source remains isolated telemetry and a scoring input, so it does not by itself fail the AlphaSift candidate response.
> Candidate-level missing fields are also handled according to enabled controls. When ST, suspension, or price-limit filters are enabled, a candidate explicitly missing `trading_status` is skipped as `candidate_trading_status_unavailable`. When a minimum turnover is configured but turnover data is unavailable, it is skipped as `liquidity_data_unavailable`. Other missing fields continue to affect the candidate quality score and rule-Agent warning instead of being rejected indiscriminately.

> Automatic selection also derives a cross-run forward-quality state from persisted candidates for the same strategy and market. It is audit-only by default. When `auto_cross_run_quality_gate_enabled` is enabled, a mature sample whose win rate is below the configured threshold, or an unavailable evaluation, blocks new buys; insufficient evidence does not block and automated risk-reducing sells still run. The evaluation reads only local daily bars strictly after each decision date and neither refreshes data online nor reruns historical strategies.

> Cross-run evaluation also applies the versioned `candidate-return-risk-v1` objective. It derives candidate-level daily returns, daily volatility, downside deviation, and 20% daily expected shortfall, then computes utility as `average forward return - 0.5 * horizon downside deviation - 0.25 * abs(average maximum adverse excursion)`. Daily closes must be complete; a multi-day move is never relabeled as a daily return. Negative mature utility tightens candidate count and per-order cash; when average return is also negative, the state becomes `blocked` and prevents new buys if the cross-run gate is enabled. The Agent console exposes the objective version, state, reason, daily-return coverage, and metrics.
>
> The paper-trading page also calls `GET /api/v1/vnpy-paper/agent-runs/return-risk-calibration-trends` for 7/30/90-day calibration trends under the current Agent strategy, market, and status filters. It counts persisted rolling run snapshots and their risk utility; overlapping forward windows are not claimed as independent samples, and legacy runs without an objective snapshot remain `unknown` rather than being inferred as healthy.
>
> Run `python scripts/check_agent_calibration_evidence.py --base-url http://127.0.0.1:8000 --required-version candidate-return-risk-v1 --output-json calibration-evidence.json` for a read-only production-calibration evidence gate. By default it only counts order-free runs with `trigger_source=agent_calibration_shadow`, then checks CN, HK, and US separately over 90 days for at least 20 completed runs, 10 observed objective snapshots, an 80% observation rate, 20 latest mature forward samples, five observation dates, and evidence no older than 72 hours. Repeated `--market` flags and explicit threshold options can narrow or tune the contract. Truncation, market-filter mismatches, missing versions, future/stale timestamps, and latest blocked/unavailable states exit nonzero. The output preserves the overlapping-window caveat and the script creates no Agent run, plan, or order. Completed `agent_calibration_shadow` scheduler events also retain a bounded per-market/strategy summary, run UID, candidate/plan/submission counts, and `submits_orders=false` so the zero-order invariant can be verified per pair.

> The evidence API and Agent console also read the same persisted cadence and show each market's strategy, current eligibility, and next eligible sampling time without running the Agent early or shortening the interval.
> The Agent console combines the current runtime's zero-connect gateway preflight and the persisted CN/HK/US calibration result into one production-readiness summary. The requests degrade independently, so an unavailable preflight does not hide Agent history or calibration evidence. This view does not connect a gateway, subscribe to market data, run the Agent, or create an order, and it does not replace external-account soak acceptance.
> When shadow sampling is enabled, the `agent_calibration_evidence` background task also persists the initial evidence state and later ready/pending transitions as `vnpy_paper` alerts through the existing alert notification route. Pending is `degraded`, ready is `resolved`, and unchanged states remain silent across polling cycles and process restarts. Alert persistence or notification failures do not fail the read-only evidence task and cannot trigger stock selection or orders.
> The same read-only evidence API and Agent console expose the latest transition-delivery summary: alert time, attempted/successful/failed/retryable counts, persisted per-channel attempt numbers, success or error codes, and waiting, due, succeeded, non-retryable, or exhausted retry state. The existing `vnpy_paper_auto_retry` task retries a retryable failure every five minutes, for at most three dispatch rounds, with progress retained across restarts. Success, missing channels, suppression, and non-retryable failures stop retries. This recovery path only resends the notification; it never runs the Agent or creates a plan or order, and alert-history failures cannot block order recovery in the same task. An unavailable alert history degrades only the delivery status and does not hide the evidence result, so channel configuration failures remain visible without being reported as repaired.

> To accumulate production-shaped calibration samples unattended, explicitly set `DSA_AGENT_CALIBRATION_SHADOW_ENABLED=true` and `DSA_AGENT_CALIBRATION_SHADOW_PAIRS=cn:dual_low,us:us_large_cap_momentum,hk:hk_liquid_momentum`; use `DSA_AGENT_CALIBRATION_SHADOW_INTERVAL_MINUTES` and `DSA_AGENT_CALIBRATION_SHADOW_MAX_RESULTS` for cadence and candidate count. The task starts 300 seconds after initial registration and creates only separate `agent_calibration_shadow` dry-run records. It never submits orders or changes saved automation settings. Every `market:strategy` pair must match `market_scope`. Each shadow quality snapshot is restricted to completed `agent_calibration_shadow` decisions for the same market and strategy in the latest 90 days; ordinary Agent decisions cannot inflate production calibration. Before each due shadow, missing forward bars are refreshed once per stock only when the shortest requested horizon can have elapsed; not-yet-due stocks are audited without a network request. The forward-evaluation API and persisted quality snapshot separately audit attempts, successes, failures, not-due skips, actual providers, saved rows, and resolved versus unresolved anchors. Read-only evidence recomputes current maturity from the same scoped decisions and local bars without refreshing market data, while legacy snapshots without explicit source/status scope are classified as unknown. DSA overlays the US strategy without modifying the installed package and provides an HK compatibility pipeline that reuses AlphaSift hard filters, factor scoring, and its market-independent LLM ranker over an explicit `ALPHASIFT_HK_TICKERS` universe. HK LLM ranking obeys the automatic Agent's request timeout, deterministic score fallback, and cross-run circuit breaker.

> Portfolio allocation can use the score/inverse-volatility method with a pairwise-correlation cap. It consumes the point-in-time 20-day volatility and local trailing returns dated no later than the run date, retaining higher-ranked candidates when a pair exceeds the configured threshold. Missing history or insufficient overlap fails closed per candidate instead of falling back to score weighting.
> The Agent console reads `GET /api/v1/vnpy-paper/agent-runs/daily-summary` to show a daily Agent summary with run counts, candidate/planned/submitted/skipped counts, status distribution, execution modes, data quality, Agent and LLM review status counts, review-quality status/flag counts and average score, workflow status/stage counts, top skip reasons, and active symbols. Candidate decision details show the versioned `order_result.strategy_evidence` contract with the effective strategy, rank, screen/final scores, bounded upstream rule matches, factor decomposition, and evidence status, plus the rule-based pre-trade `order_result.agent_review` status and summary. Missing rule/factor details are marked `summary_only` rather than inferred from an aggregate score; legacy runs without the field keep their original rationale/review display. The Agent plan card shows the rule-derived plan mode, risk level, and optional `llm_dynamic_plan` status. Run details derive `diagnostics.agent_workflow` to show the current stage, overall status, and next action across plan, data-quality, candidate-review, trade-plan, and execution stages; `diagnostics.agent_summary.review_quality` shows the run-level review status, score, coverage, risk flags, and whether manual confirmation is recommended. Run details can also manually call `POST /api/v1/vnpy-paper/agent-runs/{run_uid}/llm-recap` to generate an optional LLM recap stored in `diagnostics.llm_recap`; this does not submit orders or change trade-plan status. The paper-trading setting `auto_llm_plan_enabled` is off by default; when enabled, dynamic-plan v3 includes up to three completed recaps from runs with the same trigger source, strategy, and market as bounded, untrusted historical audit context. The system prompt forbids treating recap text as instructions, while the evaluator still permits only known strategy ids and prevents increases to max results or cash per order and decreases to min score. `auto_llm_review_enabled` is also off by default; when enabled, the service calls the LLM after rule gates, stores `order_result.llm_review`, and skips the candidate on `blocked` or review-call failure. Both `llm_dynamic_plan` and `llm_review` record `prompt_version` and `evaluator_version`, and the Agent console shows those audit versions in the matching sections.
> New candidate decisions also persist `strategy_evidence`, `position_plan`, `risk_review`, `agent_review`, and `llm_review` as dedicated decision-table and top-level API fields. Fill, cancel, or recovery updates to `order_result` no longer erase those explanations. Existing databases add the columns automatically, while legacy rows and older backends remain readable through the embedded `order_result` fallback.
>
> Every rule-based Agent plan also persists `diagnostics.agent_plan.recent_run_context`, a deterministic summary of the five most recent runs with the same trigger, strategy, and market. It includes status and quality distributions, candidate/planned/submitted/skipped totals, submission rate, current failure streak, and human acceptance feedback. The Agent console can save an approved, needs-changes, or rejected verdict with reviewer and notes, while the daily summary aggregates verdict counts. Feedback is context for later rule-plan audit and the optional dynamic-plan prompt; it never bypasses data-quality, position, or trading risk gates.

> Before the optional LLM dynamic plan, every automatic run now resolves a deterministic cross-market objective for CN, HK, US, JP, KR, or TW. Recent needs-changes/rejected feedback, consecutive failures, or degraded cross-run quality can only tighten the candidate count and per-order cash using the market risk profile. The effective limits are used by AlphaSift screening and trade-plan sizing, and the LLM may only tighten them further. The Agent console shows the objective mode, configured and effective limits, and trigger reasons.

> Agent candidate forward evaluation now joins persisted `agent_review` and `llm_review` snapshots to the 1/5/10/20-trading-day outcomes. Its long-horizon review-quality matrix groups rule reviewers and LLM models by prompt/evaluator version, then reports mature coverage, passed precision, blocked-loss avoidance, passed/blocked average returns, and return spread. Insufficient forward bars remain uncovered rather than being inferred as success. Enabling skipped candidates includes real `action=skip` risk rejections.

> Cross-run quality also derives one de-duplicated effective review policy from the same samples: an LLM verdict takes precedence when present, otherwise the rule Agent is used. Once passed or blocked samples independently reach the configured minimum mature count, passed precision or blocked-loss avoidance below the configured minimum win rate produces `blocked`; negative passed return or return spread produces `guarded`. The combined state keeps the stricter of selection quality and effective-review quality. With the cross-run gate enabled, `blocked` prevents new buys; without it, the state can only tighten the dynamic objective. Insufficient evidence never blocks.
> `GET /api/v1/vnpy-paper/status` now returns `diagnostics.system_health`, a cross-module health summary for the local ledger, selection source, automation loop, scheduling window, trading window, valuation, position-industry coverage, and vn.py bridge. The paper-trading page's availability diagnostics prefer this backend summary and fall back to `diagnostics.auto_trade_readiness` for older backends. Full status also exposes `diagnostics.industry_exposure` with resolved and missing position counts, coverage percentage, missing symbols, and market value by industry. Without industry controls it reads snapshot fields only and never performs per-symbol network lookups; configured industry limits or target weights enable strict resolution and fail closed on incomplete coverage. Lightweight status does not fetch the snapshot or industry data. `diagnostics.auto_trade_readiness` still provides automated-trading readiness, blockers, warnings, and component states; `scheduler.loop_running` reports whether the scheduler loop is alive, while `scheduler.running` still means an analysis task is currently executing. `diagnostics.auto_trade_readiness.timing_alignment` compares the auto-task next trigger with the trading-window open and close times. When the time gate is enforced and the service starts outside the trading window, the first auto-buy background task is delayed to the next market open so a fixed interval does not repeatedly miss the session after a post-market start.
> Full snapshots persist valuation-source health at most once per 15-minute UTC bucket and retain 120 days. The valuation health component and Web paper-trading page expose 7/30/90-day total and position-bearing observation counts, time-average/minimum coverage and freshness, position-observation-weighted coverage and freshness, degradation rates, and provider shares. Aggregation recomputes rates from position/available/fresh counts instead of trusting denormalized percentage fields, and caps fresh counts at available counts. Empty-position snapshots are counted separately and excluded from price-source averages, minima, and degradation denominators, so a snapshot with no price-source evidence is never treated as 100% healthy. Provider-attribution coverage audits whether provider counts cover all position observations. Empty effective history remains empty rather than being inferred as healthy. Lightweight status reads existing trends without writing an observation. History persistence failures return the stable `valuation_health_history_unavailable` marker without exposing the underlying exception text, while current valuation and paper-trading status remain available.
>
...
- **Automated Stock-Selection Agent Console** - The `/agent-console` sidebar entry centralizes paginated automated screening run history, strategy/market/status/time filters, dedicated `/agent-console/<runUid>` detail routes, Agent plan modes, default-off LLM dynamic plans, candidate decisions, rule-based Agent reviews, default-off LLM pre-trade reviews, review-quality summaries, optional LLM recaps, trade plans, and run timelines. Each automated run persists monotonic planning, preflight, AlphaSift screening, candidate decision/execution, and total timings; the existing timeline exposes total and AlphaSift duration through a `performance` event, including data-quality blocks and screening failures. It evaluates persisted candidates over strictly forward horizons, replays dated AlphaSift snapshots, and defaults portfolio replay to a cash ledger that sizes equal targets in whole A-share lots. Replay can also accept per-symbol equity percentages whose total is at most 100%; residual weight stays in cash, only configured symbols also selected in that period's Top K are traded, and configured-but-unselected symbols are audited. Explicit `corporate_actions` can model cash dividends and split adjustments against unadjusted prices. Each symbol's event clock advances to its trade or mark date, same-day actions precede trades, newly bought shares do not receive earlier same-day entitlements, and fractional split results require an explicit cash-in-lieu settlement price or fail closed. `minimum_commission` applies an absolute per-trade commission floor. In `explicit` tax mode, `sell_tax_bps` taxes only executed sell notional; `cn_historical_stamp_duty` instead applies the side-aware A-share schedule from 2005-01-24: 10 bps to both sides from 2005-01-24, 30 bps to both sides from 2007-05-30, 10 bps to both sides from 2008-04-24, 10 bps to sells only from 2008-09-19, and 5 bps to sells only from 2023-08-28. Dates before the covered schedule fail closed. Target and cash-affordability sizing include the same commission and buy-tax functions, while trade, period, and summary audits separate execution price, commission, tax, policy source, slippage, corporate actions, and net cash effect. The Web result shows annualized, benchmark, and excess return; drawdown; period win rate, return volatility, downside deviation, Sharpe, Sortino, and Calmar; coverage; and each effective buy/sell rate. Period risk statistics use rebalance-period net returns and are not represented as daily observations; ratios remain null rather than infinite when no downside or drawdown exists. The ledger sells overweight positions before buys and preserves blocked final positions at marked value. The older mean-return approximation remains opt-in and rejects explicit weights, corporate actions, or absolute fee/tax settings. Dated A-share membership prefers Tushare `stock_basic` and otherwise composes BaoStock Shanghai/Shenzhen lifecycle rows with BSE official current, code-mapping, and final-delisting sources. The fallback returns data only when SSE/SZSE/BSE coverage closes without unexplained gaps and never substitutes the current universe. Persistent full-market ingestion jobs freeze those date/symbol work items and their actual source, checkpoint each factor batch, record the daily provider per row, restore the latest job after a page refresh, and support lease-protected resume or force takeover after process restarts. Daily history uses a Sina, EastMoney, Tencent, then BaoStock fallback route. Current names/industries are not treated as point-in-time fields. These paths fail closed below complete-factor coverage thresholds and never fall back to current data or LLM ranking. The paper-trading page shows availability diagnostics, background task logs, task health, and JSON audit export
- **Persisted replay corporate actions** - Historical factor ingestion prefers Tushare `dividend`, then falls back to AkShare EastMoney dividend detail, CNInfo, and official BSE implementation-announcement PDFs. The free route accepts only implemented plans with a valid ex-date, converts per-ten-share pre-tax cash and stock-distribution ratios into per-share cash and holding multipliers, and stores the actual provider and source key idempotently. BSE history queries both the current 920 symbol and its official legacy-code mapping, then validates the implementation title, PDF symbol, ex-date, and bounded distribution-plan fields. Verified free coverage is SSE/SZSE/BSE; malformed or mismatched announcements fail closed. Source failures are audited without discarding successful factor rows. Cash-ledger replay reads stored events by default; the Web toggle can disable this, and explicit request events override a stored event with the same symbol, date, and type.
- **Recoverable full-market ingestion evidence** - Persistent job responses reconcile their saved task lease with the current process queue as `active`, `orphaned`, `retryable`, or `complete`. Ordinary resume safely takes over an orphaned post-restart job, while an actually active task remains protected by HTTP 409; explicit force takeover remains an operator-only API option. `scripts/check_full_market_ingestion_e2e.py` is read-only by default and gates market scale, checkpoint completion, complete-factor coverage, source-error ratio, regressions, and stalls. It reports written, complete, exact-daily, and valuation-complete rows separately. Creating an external job requires `--create --allow-external-ingestion --snapshot-date YYYY-MM-DD`; lifecycle completeness and implemented SSE/SZSE/BSE corporate-action ingestion no longer require Tushare permission.
- **Fractional-share cash in lieu** - A split-adjustment event may provide a positive `cash_in_lieu_price`. If the ratio produces a fractional share, replay keeps the whole-share floor, credits the fractional quantity at that explicit settlement price, and audits the quantity, price, event cash, and aggregate `cash_in_lieu_received`. Missing settlement prices still fail closed; replay does not infer them from market prices.
- **Versioned replay cost profiles** - `custom` preserves explicitly supplied commission, minimum commission, tax mode, and slippage. `cn_retail_reference` atomically applies 3 bps commission, CNY 5 minimum commission, historical A-share stamp duty, and 5 bps slippage; `cn_low_commission_reference` changes commission to 1 bps, and `zero_cost_baseline` provides a zero-cost comparison. These are repeatable assumptions, not broker quotes; actual account terms belong in `custom`. Methodology audits the requested values, effective values, profile name, and `dsa-cost-profile-v1` version.
- **Historical A-share stamp duty** - `sell_tax_mode=cn_historical_stamp_duty` resolves tax by execution side and date from 2005-01-24: 10 bps on both sides from 2005-01-24, 30 bps on both sides from 2007-05-30, 10 bps on both sides from 2008-04-24, 10 bps on sells only from 2008-09-19, and 5 bps on sells only from 2023-08-28. Every trade audits effective `tax_bps` and policy source, and buy tax participates in target-lot and cash-affordability sizing. The mode rejects non-CN markets and fails closed before the covered start date; `explicit` preserves the fixed sell-side `sell_tax_bps` contract. Sources are linked in `docs/alphasift-integration.md`.
...
Non-rule system events may use `resolved` when a previous risk guard or runtime failure has recovered. For example, the automated paper-trading drawdown latch writes `account_drawdown_recovered` after equity reaches its configured recovery buffer.
...
Run `python scripts/check_vnpy_gateway_preflight.py` to validate gateway loading, registration naming, and connection JSON key names without connecting. For a real channel, add `--require-external-gateway --require-all-default-keys` and point `--settings-path` to a file outside the repository. The running Web/API also exposes `GET /api/v1/vnpy-paper/gateway/preflight` and a Production Preflight action on the paper-trading page for the currently loaded runtime. Both surfaces return only key names, counts, and reason codes, never the settings path or values. A passing preflight does not connect, subscribe, place orders, or replace real-gateway connection and soak acceptance. See [vn.py Paper Trading](vnpy-paper-trading.md) for the complete contract.
