...
> `POST /api/v1/vnpy-paper/trade-plans/recovery/run` can run one bounded recovery scan after page confirmation. It reuses the background due-retry path and returns timeout-expired, attempted, submitted, skipped, and failed counts.
> If a `vnpy_paper` plan remains `part_filled` past the order timeout window without a trade callback, it is archived as `failed` / `vnpy_partial_fill_timeout` and emits an alert, but it is not added to the automatic retry queue. This avoids duplicate orders when a real gateway may have partially filled the order while local reconciliation is still missing.
