...
> Every stock-selection run also receives a deterministic 0-100 data-quality score based on screen integrity, candidate coverage, and observed snapshot/daily plus candidate-context `quote/fund_flow/news` source health. With `auto_min_data_quality_score` omitted, the score is audit-only; when configured, a lower-scoring round fails closed as `data_quality_score_below_threshold`. Unobserved source-health telemetry scores 70 instead of being assumed healthy, while the existing `stale/unavailable` hard gate always remains active. A failed candidate-context source remains isolated telemetry and a scoring input, so it does not by itself fail the AlphaSift candidate response.
