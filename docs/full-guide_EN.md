...
The official `vnpy_ctp==6.7.11.4` package is now the verified Windows/Python 3.13 external-plugin sample, with `VNPY_GATEWAY_CLASS=vnpy_ctp:CtpGateway` and `VNPY_GATEWAY_NAME=CTP`. Credential-free acceptance proves installation, MainEngine registration, and frozen-artifact import only. A missing repository-external connection JSON must report `connect_settings_required`; present default fields with null or blank values report `default_setting_values_empty`, and neither case attempts a connection. This release currently provides a prebuilt CPython 3.13 Windows x64 wheel only; Linux and macOS still require the upstream source-build prerequisites.

Runtime connection confirmation now understands the official CTP trading and market-data channel `login_status` fields and reports connected only when every available channel is logged in. The remote `vnpy-ctp-paper-acceptance` Environment exists with zero secrets and is restricted to `main` by both GitHub policy and the workflow; the current billing plan does not support reviewer or wait-timer rules. After the account owner manually adds an isolated paper-account secret, `External vn.py CTP Account Soak` validates the exact eight-field, non-empty string schema before checkout or dependency installation, then enforces strict preflight, runs a no-order soak, and retains sanitized evidence for 14 days. Production trading credentials must remain on a controlled deployment host. See [vn.py paper trading](vnpy-paper-trading.md) for the full security and setup contract.
...
> `POST /api/v1/vnpy-paper/trade-plans/recovery/run` can run one bounded recovery scan after page confirmation. An active plan becomes eligible for injected `MainEngine.get_order` / `get_all_trades` reconciliation after a 60-second grace period. A plan with no gateway evidence waits until the 30-minute timeout before safe archival. In addition to timeout, retry, submission, skip, and failure counts, the response returns `reconciled_count`, `protected_count`, and `reconciliation_failed_count`.
