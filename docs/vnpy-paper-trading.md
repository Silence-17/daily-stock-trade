...
- Agent run 详情会从运行状态、结构化 `agent_plan`、结构化 `agent_summary`、结构化 `agent_workflow`、数据质量、候选决策和交易计划合成 `timeline` 时间线；候选决策和交易计划的 `order_result` 会保存候选级 `strategy_evidence`、`position_plan`、`risk_review` 与 `agent_review`；Web 页面直接展示“Agent 计划”“运行总结”“Agent 工作流”“运行时间线”、候选级“策略证据”和“Agent 复核”，用于解释本轮任务为何按该策略、市场、预算和执行模式运行，以及最终为何成交、计划、警告或跳过。旧运行记录没有 `strategy_evidence` 时继续按原有理由和复核字段展示。
