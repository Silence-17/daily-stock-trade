...
- 开启 `auto_score_weighted_allocation_enabled` 后，系统把 `auto_allocation_budget` 视为每轮组合预算；留空时使用 `auto_cash_per_order`。预算按候选评分加权分配，每票仍以 `auto_cash_per_order` 为上限，并同时受现金保留、每日预算/订单槽位、最大持仓数、单票/总仓位/行业空间和显式目标权重约束。某候选触顶后，剩余预算会重新分配给仍有空间的候选。Agent run 的 `diagnostics.portfolio_allocation` 和候选 `position_plan.portfolio_allocation` 会记录评分权重、约束、分配金额、排除原因、整手后的可执行预算和残差。该能力默认关闭，当前是确定性约束分配器，不包含协方差、波动率或任意目标函数优化。
