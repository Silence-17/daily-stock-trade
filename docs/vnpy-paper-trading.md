...
- V1.4 自动轮次在候选筛选前审计全局 A 股开盘门槛。沪深 300 高开或极端低开时，整个 `09:30-09:35` 窗口先执行持仓卖出检查，再以零买入候选结束并记录 `cross_market_global_entry_gate`；10:40 只允许带标的身份的可恢复候选完成回撤承接复核；13:30/14:30 会重新选股，但必须先通过同日全市场涨跌家数的分歧、跌超 4000 家或修复门槛，且单次使用降级仓位上限。盘中宽度优先使用带供应商时间的东方财富完整分页快照，分页不完整时整批失败关闭；`cross_market_market_breadth` 保存当前涨跌家数、日内样本与放行原因。
...
- 当前策略的 `/status` 页面快照固定使用 `persist=false`，页面刷新不会以普通 Portfolio 行情覆盖专属任务写入的严格收盘价格与 provider timestamp；活动收盘证据只由 `cross_market_campaign_closing_snapshot` 写入。
- 跨市场盘中卖出、待成交复核、公司行动同步、持仓/现金/风险门禁及绩效读取同样只读 Portfolio；即使它们在 14:55 后继续运行，也不能覆盖专属收盘快照。其他策略仍保留原持久化语义。
...
- Agent 控制台的“生产就绪摘要”并列读取 `/gateway/preflight` 和 `/agent-runs/calibration-evidence`，集中显示 gateway 身份、启动期生产预检、脱敏配置键计数、阻断原因及三市场校准进度。两个请求独立降级：gateway 预检不可用不会隐藏 Agent run 历史或校准证据。该摘要只读，不连接 gateway、不订阅、不运行 Agent，也不创建订单。
...
- 通过 Web/API 成功提交到 vn.py bridge 的手工委托会持久化记录本地模拟账户、股票、方向、市场和委托数量。服务重启后收到成交回报时，系统按 `vt_tradeid + 交易日` 去重，把每笔实际成交写回原隔离账户并累计 `part_filled` / `filled` 状态；旧订单状态回报不会把已成交状态回退。没有本地登记或 Agent 交易计划的外部订单、身份字段不匹配的回报以及累计数量超过原委托的回报均 fail closed，不会污染本地账本。
...
- 状态接口的 `diagnostics.vnpy_sync_state.account` 仅公开余额、可用/冻结资金、盈亏、币种和更新时间，不返回外部柜台账户标识或原始账户回报；完整标识仍只保留在服务内部持久化状态中，用于事件关联和恢复。订单终态、持仓摘要与自动恢复所需字段保持不变。
...
- 默认关闭的“跨市场联动风控”按固定映射 v1 读取关联市场主指数：`cn -> hk,us`、`hk -> cn,us`、`us -> hk`、`jp/kr/tw -> us`。每个市场使用方向性指数的等权平均涨跌幅，美国 `VIX` 会被排除；任一关联市场低于阈值时使用 `cross_market_index_below_threshold`，任一关联市场缺少有效指数、实时 quote 缺少/失效 provider 时间或实时指数之间最大时间偏差超过 120 秒时使用 `cross_market_context_unavailable`。`diagnostics.market_context_risk` 记录映射版本、逐指数涨跌幅、聚合口径、阈值、阻断市场和 `quote_time_alignment`。已收盘市场的 `session_bar` 不参与时间偏差计算，仍只作为非同步背景证据；因此该门禁不会把它解释为交易所级同步行情。
- 实时指数取证保持 15 秒调用预算；需要拉取完整 A 股列表的涨跌家数取证使用独立 35 秒预算，以覆盖底层 provider 的 30 秒请求上限。进程内最多保留 4 个尚未返回的取证线程。超时或槽位耗尽不会等待无上限，也不会绕过门禁；当前候选分别以 `intraday_market_index_unavailable`、`intraday_market_breadth_unavailable` 或 `cross_market_context_unavailable` 停止新增买入，底层诊断用 `*_fetch_timeout` / `*_worker_pool_exhausted` 区分原因并记录耗时。超时线程只能等待 provider 自身返回，无法被 Python 安全强制终止，因此工作池上限是防止持续堆积的第二道边界。
- `DataFetcherManager` 会在成功的指数/宽度 fallback 结果追加 `provider` 和 `fetched_at`；各 provider 能确认时还会给出 `provider_timestamp`、`provider_timestamp_coverage_pct`、`data_date` 和 `data_granularity`。TickFlow 指数使用 quote 自带时间并标记 `realtime`，市场宽度仅在所有有效样本都有时间戳时给出最早 provider 时间。efinance 会把底层 EastMoney 更新时间恢复为绝对 UTC：股票、ETF 和指数逐项携带该时间，市场宽度仅在所有有效计数行都有时间时报告最早 as-of 和 100% 覆盖率。efinance 不可用时，AStockDataFetcher 会在 AkShare 批量端点前直接读取 EastMoney 原始 `f124/f297`；它要求 API `total` 与全部返回行一致，指数和宽度才会进入 fallback，宽度同样要求完整时间覆盖。AkShare 的新浪/腾讯单票直连也保留明确的 A 股时间，AkShare 批量指数/宽度端点仍不伪造时间。YFinance 两日 history 标记 `session_bar`，Tushare `index_daily` 标记 `end_of_day`。盘中本市场门禁拒绝 `end_of_day`、非当前市场日期的 session bar、未来/非法 provider 时间，以及超过 15 分钟的 provider 时间；严格模式还要求宽度覆盖率字段明确等于 100%，缺失或任何非 100% 值都会 fail-closed，拒绝明细写入 `rejected_indices`。跨市场联动允许已收盘市场的最新 `session_bar`，因为其用途是隔夜/关联市场背景而非声称同步盘中报价。
- 自动模拟交易支持可选连续失败熔断；开启后按同策略、同市场、同触发源统计最近失败运行，连续达到阈值时以 `failure_fuse_open` 跳过本轮自动买入。熔断只统计 `failed`、数据质量 `stale/unavailable`、非良性 error 以及后续熔断跳过记录，不把自动交易关闭、非交易日、交易时段外等正常跳过计为失败。熔断打开后保持锁存；可继续使用 Web“恢复熔断”或对应 API 手动重置，也可显式开启默认关闭的 `auto_failure_fuse_auto_recovery_enabled` 并设置 `auto_failure_fuse_cooldown_minutes`。自动恢复会持久化首次打开时间，冷却到期后只放行当前一轮作为恢复探测；若探测再次失败，后续任务会重新熔断并开始新的冷却周期。跨市场 `cross_market_paper_observation` 是强制零委托的证据任务，不受交易熔断锁存影响，数据恢复后下一轮会继续尝试记录完整或降级观察日；正式 `vnpy_paper` 交易仍完整执行熔断。状态接口通过 `diagnostics.failure_fuse` 返回打开时间、预计恢复时间、剩余冷却秒数和最近自动恢复时间，自动恢复写入 `failure_fuse_auto_recovered` 系统事件；任何复位都不删除历史 Agent run。
...
- 可执行 `python scripts/check_vnpy_adapter.py` 验证当前 Python 环境的 vn.py adapter 状态；未安装 vn.py 时脚本输出 `local_paper_fallback` 诊断并以 0 退出，安装 vn.py 后会尝试实例化真实 `OrderRequest`。使用 `--require-vnpy --reconnect-cycles 3` 还会对内置 `DSA_SIM` 执行三轮在途订单断线重连，校验订单缓存保留、恢复后恰好成交一次、订单/成交编号不重复以及资金和持仓连续性。该脚本在独立 MainEngine 中显式使用 `fixed_delay_limit`，不会读取、修改或验证部署进程的 `DSA_SIM_MATCHING_MODE`；跨市场生产配置 `next_minute_vwap` 由专用网关测试和在线状态另行验收。桥接提交可以由宿主进程注入 `MainEngine`，也可以显式设置 `VNPY_RUNTIME_ENABLED=true` 让 DSA 在启动时创建 EventEngine/MainEngine，并按需 add gateway/connect。
...
- Docker 默认镜像保持轻量本地 paper 能力；自行构建时可用 `--build-arg INCLUDE_VNPY=true` 安装 `requirements-vnpy.txt`，Compose 则使用 `DSA_INCLUDE_VNPY_DOCKER=true`。如需外部插件，同时传入 `VNPY_GATEWAY_PLUGINS_JSON`；构建会安装并导入每个声明模块，未开启 vn.py 却声明插件时直接失败。可选镜像仍不会猜测具体券商 gateway，也不会把连接参数或账户凭据写入镜像。
...
- 绩效接口会基于本地 paper 账本返回 `daily_returns` 和 `monthly_returns`，按每日/每月最后权益聚合盈亏、收益率、累计收益率、回撤和交易数；`trade_metrics` 同时汇总累计手续费、税费、总交易成本、扣费后已实现盈亏和盈利因子。动态滑点已进入下一分钟 VWAP 冲击后的真实模拟成交价，不作为第二笔费用重复扣除。Web 模拟交易页展示最近日度收益表和月度收益表。该视图用于 paper 运行复盘，不等同接入完整历史行情后的回测级净值曲线。
- 后台恢复任务会先主动对账提交满 60 秒的 `vnpy_paper` 活跃计划。MainEngine 仍观测到活动委托且计划创建已超过 30 分钟时，系统只发起一次撤单并进入 `cancel_requested`；对账回调更新 `updated_at` 不会重置该委托的超时计时，普通活动状态也不会覆盖撤单处理中状态。撤单请求从 `raw.cancel.requested_at` 独立计时，超过 30 分钟仍无终态才归档为 `vnpy_cancel_timeout`。若超时后没有任何 MainEngine 订单或成交证据，`submitted` / `part_filled` 分别归档为 `vnpy_order_timeout` / `vnpy_partial_fill_timeout`。所有状态不明超时结果都禁止自动重提；撤单期间或归档后的迟到成交仍按原 `vt_orderid` 回写为实际成交状态。恢复响应通过 `cancel_requested_count` 单独报告本轮自动撤单数。
...
- `GET /performance?run_limit=50&created_from=2026-07-01T09:00:00&created_to=2026-07-02T15:00:00`：读取本地 paper 绩效摘要，聚合当前模拟账户权益、相对初始资金收益、成交额、买卖次数、累计手续费/税费、扣费后已实现盈亏、盈利因子、FIFO 卖出胜率、权益路径、日度收益、月度收益、最大回撤、换手率、当前仓位、最近 Agent run 状态、成交计划状态、跳过原因、成交股票分布和按策略/行业归因；`trade_metrics.cost_basis` 明示费用和滑点口径，`created_from` / `created_to` 可选，用于按 Agent run 创建时间筛选窗口级归因。
...
- `GET /agent-runs/calibration-evidence`：只读评估 A 股、港股、美股最近 90 天 `agent_calibration_shadow` 已完成 run，返回统一 ready/pending 结论、逐市场运行/有效观测/成熟样本/观察天数、旧快照范围不明计数、明确失败项，以及最近校准状态告警的投递与有限重试状态。成熟样本同时返回持久化快照值、当前 shadow-only 本地重算值和门禁采用值；重算固定不刷新行情。缺少 `cross_run_quality.trigger_source/run_status` 范围证明的旧 shadow 快照按 unknown 处理。该接口固定排除普通 Agent run，不创建 run、交易计划或订单；Agent 控制台在当前值与快照值不同时并列展示。
...
- `POST /agent-runs/backtest`：除候选与策略的 1/5/10/20 交易日前瞻矩阵外，还返回逐日收益/风险指标和 `review_quality_matrix`。前瞻矩阵给出逐日观测数、日均收益、日波动率、下行偏差、期限下行偏差、最差 20% 日度收益均值、版本化风险效用和最大有利/不利波动；复核矩阵按规则 reviewer 或 LLM model、`prompt_version`、`evaluator_version` 分组，分别计算成熟样本覆盖率、通过精度、阻断避损率、通过/阻断平均收益与收益差。启用 `refresh_missing` 时，响应还返回 `refresh_succeeded_count`、`refresh_failed_count`、`refresh_source_counts`、`refresh_saved_row_count`、`refresh_resolved_anchor_count` 和 `refresh_unresolved_anchor_count`；动态供应商键保持原名。`include_skipped=true` 会纳入真实 `action=skip` 风控阻断候选，但仍只读取决策日之后的本地日线。
...
当前外部账户目标是 A 股中泰 XTP，而不是期货 CTP/SimNow。Windows/Python 3.13 的官方 `vnpy_xtp==2.2.32.2.3` 使用清单 `[{"package":"vnpy_xtp==2.2.32.2.3","module":"vnpy_xtp"}]`，Gateway 设置为 `VNPY_GATEWAY_CLASS=vnpy_xtp:XtpGateway`、`VNPY_GATEWAY_NAME=XTP`。本地零连接验收已真实导入原生 wheel、向 MainEngine 注册 `XtpGateway`，确认原生交易所仅为 SSE/SZSE、识别 10 个默认连接字段，并确认未连接、未订阅、未下单；缺少仓库外连接 JSON 时按预期只返回 `connect_settings_required`。手动工作流 `.github/workflows/vnpy-external-gateway-smoke.yml` 会重复安装、零连接注册和冻结后导入探针，不读取 Secrets。XTP 测试账号、地址、端口和授权码必须由中泰 XTP 官方申请结果提供，本仓库不生成或保存账户参数。

运行时和订单提交桥现在都会读取 XTP Gateway 的 `td_api.login_status` 与 `md_api.login_status`：只有交易和行情两个通道都登录才确认 `connected`，任一通道未登录或掉线都会保持/切换为 `disconnected`。原有 Gateway 状态钩子和 `gateway.connected` 仍优先，无法识别的插件继续返回 `connection_unconfirmed`，不会用 `connect()` 未抛异常冒充登录成功。
...
python scripts\check_vnpy_deployed_runtime_soak.py `
  ...
  --expected-build-id $env:DSA_BUILD_ID `
...
DSA_SIM_MATCHING_MODE=fixed_delay_limit
...
- `DsaSimulatedGateway` 的连接 JSON 可显式设置 `reject_every_nth_order`（默认 `0`，不注入拒单）和 `duplicate_trade_event_count`（默认 `1`，范围 1-5）做故障验收；生产模拟运行应保持默认值。`--fault-matrix` 只在脚本自建的 MainEngine 中临时启用，并在后续重连循环前恢复默认值，同时显式锁定 `fixed_delay_limit`，不会受生产严格撮合环境变量影响。
- 同一委托的订单状态与成交回报使用同一把可重入锁串行同步。成交回报提交 Portfolio 流水、计划/决策状态、`fill_sync` 和 `execution` 后，迟到的订单状态或 MainEngine 对账只返回已成交结果，不得以旧提交态覆盖成交价、费用、下一分钟 VWAP 或滑点证据。
