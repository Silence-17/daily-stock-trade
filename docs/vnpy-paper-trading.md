...
- `GET /agent-runs/calibration-evidence` 同时返回最近校准状态告警的投递摘要，包括告警状态/时间、投递总数、成功/失败/可重试数量和不含敏感诊断的渠道结果。Agent 控制台直接展示 `delivered`、`failed`、`not_configured`、`suppressed`、`not_attempted`、`not_recorded` 或 `unavailable`；告警历史读取失败会单独降级，不隐藏校准证据主体。
...
- `GET /agent-runs/calibration-evidence`：只读评估 A 股、港股、美股最近 90 天 `agent_calibration_shadow` 已完成 run，返回统一 ready/pending 结论、逐市场运行/观测/成熟样本/观察天数和明确失败项。该接口固定排除普通 Agent run，不创建 run、交易计划或订单；Agent 控制台以桌面表格和移动端逐市场明细展示同一合同。
