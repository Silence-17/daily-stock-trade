...
| Web 操作页 | 已有选股页、模拟交易页和独立自动选股 Agent 控制台，支持暂停确认、一键恢复、立即 dry-run 演练、后台调度状态、持久化可筛选后台任务日志、任务趋势摘要、7/30/90 天后台任务长期指标、任务健康检查、模拟账户历史筛选/恢复/切换/隐藏式批量清理、权益曲线、日度收益表、月度收益表、按 Agent run 创建时间筛选的窗口级绩效矩阵、已记录候选的 1/5/10/20 日样本外前瞻评价、时间点因子采集/覆盖检查/单日重放、逐日期历史全市场分批恢复、进程重启孤儿租约诊断与安全接管、现金/整手/等权或显式股票目标权重组合回测、自动或显式分红/拆并股、显式零碎股现金补偿、固定或 2005 年以来 A 股历史双边/单边印花税、版本化常规/低佣/零成本档位、自定义实际账户费率与成本审计、可交易窗口、风控统计、复核质量摘要、运行时间线、分页历史、策略/市场/状态/时间范围筛选、`/agent-console/<runUid>` 独立详情路由、查询参数兼容深链和单次/最近 run JSON 导出 | 约 99% | 仍缺外部权限下的公司行动成功证据 |
...
- 长期稳定版仍缺真实 gateway 长跑、外部权限下的公司行动成功证据，以及生产样本下多模型/多市场长期校准；5,529 只全市场长任务、2005 年以来历史税制、版本化参考成本档位、自定义实际账户费率、Agent run 人工验收、历史意见上下文复用、版本化收益/风险目标和风险门禁隔离已完成。
...
- 页面已展示 AlphaSift 源健康、本轮降级原因、模拟交易跨模块健康摘要、跨 run 整体质量、具体来源降级趋势、snapshot 动态路由，以及候选实时行情/资金流/新闻 provider 的优先级、熔断、冷却、半开探测、跨 API 重启恢复状态和下轮路由建议；全市场长任务证据已完成，下一项继续补公司行动外部权限证据与真实 gateway 长跑验收。
...
- 纸面交易绩效摘要已有基础能力，已包含权益曲线、日度收益、月度收益、FIFO 卖出胜率、最大回撤、换手率、策略/行业贡献评估、窗口级绩效矩阵、Agent run 创建时间窗口筛选、已记录候选的严格后向多周期评价、单日时间点策略重放和现金/整手/等权或显式股票目标权重组合回测；组合回测可从 Tushare 因子采集结果自动读取或显式覆盖现金分红、拆并股，按显式结算价兑付零碎股，选择版本化参考成本档位或自定义实际账户成本，并在固定卖出税或 2005 年以来 A 股分方向历史印花税之间选择，审计事件及逐笔/逐期/汇总成本。历史全 A 股逐日期成员解析和分批可恢复因子采集的在线长任务已完成，仍缺 Tushare `dividend` 外部权限下的公司行动成功证据。
- 全市场验收门禁、重启孤儿租约自动接管和无 Tushare 权限的完整历史成员 fallback 已完成。作业冻结真实 `universe_source`，日线按新浪 -> EastMoney -> 腾讯 -> BaoStock 降级并逐行审计 provider；验收不再把写入行数当作完整覆盖。5,529 项干净长任务完成 111 批，完整因子覆盖率 99.367%、精确日线覆盖率 99.8734%、估值完整率 99.5117%、源错误率 0.4883%，最终 `ok=true` 且 `recovery_state=complete`；公司行动在线成功证据仍需 Tushare `dividend` 权限。
