...
| 自动模拟交易 | 已能从 AlphaSift 候选生成交易计划，支持 dry-run、手动审批、按交易时段写入本地 paper 订单，并支持可交易窗口诊断、首次自动买入调度对齐下一交易窗口、基础止损/止盈/移动止损/持仓天数卖出、超时未走强卖出、按比例分批卖出、active 防守决策信号触发的策略失效卖出、单票/组合/行业仓位暴露上限、跨币种预算、按股票/行业目标缺口缩量补仓、候选评分加权约束分配和基础组合再平衡、股票黑名单、候选级/账户级/市场级基础风控 | 约 99% | 仍缺真实 gateway 长跑验证和风险模型/任意目标函数优化 |
| Web 操作页 | 已有选股页、模拟交易页和独立自动选股 Agent 控制台，支持暂停确认、一键恢复、立即 dry-run 演练、后台调度状态、持久化可筛选后台任务日志、任务趋势摘要、7/30/90 天后台任务长期指标、任务健康检查、模拟账户历史筛选/恢复/切换/隐藏式批量清理、权益曲线、日度收益表、月度收益表、按 Agent run 创建时间筛选的窗口级绩效矩阵、已记录候选的 1/5/10/20 日样本外前瞻评价、时间点因子采集/覆盖检查/单日重放、逐日期历史全市场分批恢复、现金/整手/等权目标组合回测、可交易窗口、风控统计、复核质量摘要、运行时间线、分页历史、策略/市场/状态/时间范围筛选、`/agent-console/<runUid>` 独立详情路由、查询参数兼容深链和单次/最近 run JSON 导出 | 约 99% | 仍缺外部权限下的全市场长任务证据、企业行动和任意权重优化器 |
...
- 长期稳定版仍缺真实 gateway 长跑、基于风险模型或任意目标函数的组合优化、外部权限下的全市场长任务证据和 Agent 跨运行闭环；候选评分加权约束分配、显式股票/行业目标缺口补仓、基础跨币种估值、预算风控、逐日期历史股票池、可恢复全市场因子采集、样本外前瞻评价、时间点策略重放以及现金/整手/等权目标组合回测已完成。
...
- Agent 计划阶段已有基础诊断、规则派生计划档位/降级动作、最近运行上下文、默认关闭的 LLM 动态计划、基础复核质量摘要和基础可解释状态机；仍缺跨市场动态目标、更完整的每日目标函数、长周期质量评估和可跨轮迁移阶段的状态机。
...
- 自动交易已有基础止损、止盈、移动止损、持仓天数卖出、超时未走强卖出、按比例分批卖出、active 防守决策信号触发的策略失效卖出，以及基于候选评分、现金/每日预算、单票/总仓位/行业仓位暴露上限、跨币种预算和股票/行业目标权重的约束分配、缺口补仓与卖出侧组合再平衡；仍缺风险模型、协方差约束和任意目标函数驱动的完整优化策略。
...
- 扩展交易规则：基础止损、止盈、移动止损、持仓天数、超时未走强、按比例分批卖出、策略失效卖出、评分加权约束分配、暴露上限驱动和目标权重驱动的缺口补仓/基础组合再平衡，以及跨币种估值和预算风控已落地；继续补真实 gateway 长跑验证和基于风险模型的组合优化。
...
- 当前 AlphaSift snapshot/daily 源已有 Web 可见失败计数、错误摘要和具体来源跨 run 降级率；仍缺基于长期质量动态调整来源权重的闭环。
...
- 基础数据质量合同已下沉到 AlphaSift screen 候选和 Web 选股页；Web 已展示 AlphaSift source health，Agent 控制台已提供整体质量和具体来源降级率趋势，继续补动态来源权重。
...
- 页面已展示 AlphaSift 源健康、本轮降级原因、模拟交易跨模块健康摘要、跨 run 整体质量和具体来源降级趋势；继续补基于趋势的动态来源权重。
...
- 绩效摘要已有 MVP，并已包含本地 paper 权益路径页面曲线、日度收益、月度收益、最大回撤、换手率、按策略/行业归因、窗口级绩效矩阵、Agent run 创建时间窗口筛选、已记录候选的 1/5/10/20 日样本外前瞻评价、带字段覆盖门禁的单日时间点策略重放，以及按现金、A 股整手和等权目标数量执行的跨日组合回测；Tushare 生命周期元数据已按每个快照日期独立还原全 A 股成员并接入持久化、分批、租约保护的可恢复因子采集，仍缺成功在线长任务证据、企业行动处理和任意目标权重/多约束优化器。
...
- 纸面交易绩效摘要已有基础能力，已包含权益曲线、日度收益、月度收益、FIFO 卖出胜率、最大回撤、换手率、策略/行业贡献评估、窗口级绩效矩阵、Agent run 创建时间窗口筛选、已记录候选的严格后向多周期评价、单日时间点策略重放和现金/整手/等权目标组合回测；历史全 A 股逐日期成员解析和分批可恢复因子采集已完成，仍缺具备外部权限后的成功在线长任务证据、企业行动和任意权重优化。
...
- 增加 Web 测试，覆盖任务历史和详情。
- 基础模拟交易绩效摘要已覆盖收益、收益率、权益曲线、日度收益、月度收益、成交额、买卖次数、FIFO 卖出胜率、最大回撤、换手率、当前仓位、成交/计划、跳过原因、策略/行业归因、窗口级绩效矩阵、Agent run 创建时间窗口筛选、已记录候选的多周期前瞻评价、时间点策略重放，以及带成本/基准、重叠持仓保留、卖出受阻跨期延续和基础停牌/涨跌停约束的跨日组合回测；继续增加历史全市场股票池来源和精确目标权重优化。
...
- 卖出、止损、止盈、暴露上限、跨币种预算、评分加权约束分配和目标权重驱动的基础调仓规则已落地；继续补真实 gateway 长跑验证和基于风险模型/任意目标函数的组合优化。
