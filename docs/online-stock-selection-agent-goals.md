...
截至 2026-07-16，本系统已经具备 AlphaSift 在线选股、本地模拟交易账本、Web 入口、定时自动买入、可交易窗口诊断、跨模块系统健康视图、关键异常 alert 路由通知、vn.py 提交态审计、内置即时撮合 gateway、成交回报同步入口、规则 Agent 买入前二次复核、默认关闭的 LLM 动态计划、默认关闭的 LLM 买入前复核、基础复核质量摘要、手动可选 LLM 复盘和 Agent run 人工验收反馈，但还没有达到完整 Agent 化、稳定自动化和真实 gateway 长跑验收目标。
...
- 页面级“为什么不可用”已有后端 readiness 诊断摘要、跨模块 `system_health` 健康视图、API/contract/build/Python/进程启动信息、旧后端 contract 兼容提示、AlphaSift 依赖状态、状态加载失败分类和持仓估值降级提示；后台任务已有 7/30/90 天长期指标，Agent run 已有 7/30/90 天整体数据质量与 provider 明细趋势，snapshot 和资金流已消费 30 天趋势形成请求级权重，仍需补新闻请求级权重和更完整的自动恢复策略。
...
- 当前 AlphaSift snapshot/daily 源已有 Web 可见失败计数、错误摘要和具体来源跨 run 降级率；候选补充阶段的行情、资金流和新闻也已作为 `candidate_context.quote/fund_flow/news` 接入同一健康快照、质量评分和跨 run 趋势，并新增 `quote/<provider>`、`fund_flow/<provider>`、`news/<provider>` 明细。自动 Agent 已按最近 30 天同策略/市场历史与当前连续失败生成有边界的 snapshot 来源权重，并以请求级参数动态调整 `tushare_ths -> akshare` 资金流顺序；provider 明细不重复计入本轮质量分。新闻已有 provider 故障转移和细粒度趋势观测，但请求级趋势重排仍待完成。行情与资金流都具备 5 分钟冷却、单半开恢复和 24 小时内跨 API 重启健康延续。
...
- 页面已展示 AlphaSift 源健康、本轮降级原因、模拟交易跨模块健康摘要、跨 run 整体质量、具体来源降级趋势、snapshot 动态路由，以及候选实时行情/资金流 provider 的熔断、冷却和跨 API 重启恢复状态；下一项继续补外部权限下的长任务证据与真实 gateway 长跑验收。
...
- 运行中状态、任务级下次执行时间、最近错误、读取持久化最近事件的后台任务健康检查、持久化可筛选后台任务日志、任务趋势摘要、7/30/90 天终态运行长期指标、provider 级跨 run 数据质量趋势、任务事件保留/低频清理策略和 Agent run 时间线已有基础展示；继续补新闻请求级权重和真实环境长期可用率。
...
- 端到端主路径已有基础覆盖；scheduler 已覆盖配置重载跨代重叠、审计跳过和异常释放，真实 MainEngine/EventEngine 已覆盖多候选混合结果、显式拒单、重复成交回报和循环重连，仍缺真实 gateway 长时间运行场景矩阵。
...
- 增加风控拒单测试矩阵。
- scheduler 自动任务测试已覆盖开启、关闭、重启 reconcile，以及重载前任务未结束时的跨代防重、持久化跳过审计和异常后锁释放；继续补真实长时间运行证据。
- 增加 Web 测试，覆盖任务历史和详情。
