...
- 生产校准现在有独立只读验收门禁 `scripts/check_agent_calibration_evidence.py`，默认按 A 股/港股/美股分别门禁 90 天运行量、目标快照观测率、最新成熟样本、观测日期、时效、版本和摘要完整性。2026-07-20 真实基线为 A 股 17 个 completed run、16 个目标快照（94.12%）、4 个观测日、0 个成熟样本，港股/美股为 0；因此能力已可测，但长期证据仍明确未通过。
...
未完成程度：当前约 1% 未完成，功能面已基本闭环，剩余为生产样本下的长期校准和全市场证据。
