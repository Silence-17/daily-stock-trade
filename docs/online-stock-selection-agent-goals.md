...
- 绩效摘要已有 MVP，并已包含本地 paper 权益路径页面曲线、日度收益、月度收益、最大回撤、换手率、按策略/行业归因、窗口级绩效矩阵、Agent run 创建时间窗口筛选、已记录候选的 1/5/10/20 日样本外前瞻评价、带字段覆盖门禁的单日时间点策略重放，以及按现金、A 股整手和等权或显式股票目标权重执行的跨日组合回测；Tushare 优先、EastMoney/巨潮/北交所官方公告 fallback 的已实施公司行动已接入持久化采集，SSE/SZSE/BSE 在线证据已完成，拆并股支持按显式结算价兑付零碎股。A 股印花税已按 2005/2007/2008/2023 官方生效日和买卖方向自动切换，买方税进入整手与现金约束；2005-01-24 之前仍 fail-closed。常规、低佣和零成本参考档位已有版本审计，实际券商条款通过 `custom` 显式输入而不由系统猜测。全市场在线长任务证据已完成；生产校准证据已有非零退出门禁、只读 scheduler 监控、在线 API 和 Agent 控制台三市场缺口视图。当前 completed shadow runs 为 CN 10、HK 9、US 11；旧质量快照因缺少来源范围证明分别有 10/9/11 个被降为 unknown，严格 shadow-only 当前五日成熟样本三市场均为 0。后续自然到期 shadow 将开始积累可验证的新范围快照，运行量、有效观测、观察天数和成熟样本仍未达到最终门槛。
