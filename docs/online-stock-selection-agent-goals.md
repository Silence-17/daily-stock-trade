...
| 稳定性与可观测 | 有状态接口、错误提示、跨模块 `system_health` 健康视图、AlphaSift source health 页面视图、候选级数据质量标注、自动交易数据质量诊断、可交易窗口诊断、读取持久化最近事件的后台任务健康检查、持久化可筛选后台任务日志、任务趋势摘要、7/30/90 天终态运行长期指标、7/30/90 天跨 run 数据质量趋势、任务事件保留/低频清理策略、Agent run 时间线、基础失败重试、交易计划恢复矩阵、手动恢复扫描、部分成交超时安全归档、熔断状态/恢复入口、自动交易系统事件告警历史和 alert 路由通知尝试审计 | 约 95% | 缺少完整自动恢复策略、细粒度来源权重趋势和性能优化 |
...
- 当前 AlphaSift snapshot/daily 源已有 Web 可见失败计数和错误摘要，Agent run 已有整体筛选质量、警告和 source error 的跨 run 趋势；仍需要细化到具体来源的权重与长期可用率。
...
- 基础数据质量合同已下沉到 AlphaSift screen 候选和 Web 选股页；Web 已展示 AlphaSift source health，Agent 控制台已提供跨 run `ok`、`partial`、`stale`、`unavailable`、`unknown` 趋势，继续补具体来源权重和长期可用率。
...
- 页面已展示 AlphaSift 源健康、本轮降级原因、模拟交易跨模块健康摘要和跨 run 整体数据质量趋势；继续补细粒度来源权重趋势。
