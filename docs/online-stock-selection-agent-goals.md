...
截至 2026-07-23，本系统已经具备 AlphaSift 在线选股、本地 A 股模拟交易账本、Web 入口、定时自动买入、可交易窗口诊断、跨模块系统健康视图、关键异常 alert 路由通知、vn.py 提交态审计、内置即时撮合 gateway、成交回报同步入口、规则 Agent 买入前二次复核、默认关闭的 LLM 动态计划、默认关闭的 LLM 买入前复核、基础复核质量摘要、手动可选 LLM 复盘、Agent run 人工验收反馈，以及可分别验证手动触发和后台调度触发的在线 Agent -> DSA_SIM 验收门禁。Agent 控制台已把外部 gateway 零连接预检与三市场校准证据集中为只读生产就绪摘要。A 股、美股和港股均已有可执行策略；真实 US/HK shadow 均完成 3 候选、1 计划、0 下单。官方 `vnpy_xtp==2.2.32.2.3` 和中泰 XTP 股票测试账号已完成 15 分钟远端零下单验收、300 秒部署长跑、受控 100 股提交与撤单、隔离账本一致性及 Agent dry-run；正式自动执行已切换到 XTP，并对齐下一交易日 09:35。尚未达到的目标收敛为开市窗口真实成交回报、1/4 小时部署长跑，以及生产样本下多模型、多市场长期校准。
...
| 自动模拟交易 | 已能从 AlphaSift 候选生成交易计划，支持 dry-run、手动审批、按交易时段写入本地 paper 或 XTP 测试账户订单；XTP 已完成受控提交/撤单，正式自动任务启用每日 1 笔、1000 元预算和交易窗口门禁，并对齐下一交易日 09:35；其余具备可交易窗口诊断、止损/止盈/移动止损、仓位暴露、跨币种预算、组合分配和候选/账户/市场基础风控 | 约 99% | 仍缺 XTP 开市成交与长期长跑验证 |
...
| vn.py 集成 | 已有 `OrderRequest` / `CancelRequest` 映射、`MainEngine.send_order` / `cancel_order` 桥接、订单/成交/账户/持仓回写、EventEngine attach、opt-in runtime、外部 Gateway 插件安装/打包、内置即时撮合 gateway、官方 XTP 双通道连接与账户/持仓回报、部署长跑、手工订单提交/撤单和通用受限撤单 API | 约 99% | XTP 开市成交、重连故障注入和 1/4 小时长跑仍待完成 |
...
- 外部 Gateway 不再停留在“人工预装插件”的不可复现状态：`VNPY_GATEWAY_PLUGINS_JSON` 统一驱动隔离环境、Docker 和 Windows/macOS Desktop 的插件安装、模块导入、PyInstaller 收集与冻结后探针；未开启 vn.py 却声明插件、URL/marker/重复或非法清单、安装失败和产物漏模块都会 fail-closed。Windows/Python 3.13 已实际安装官方 `vnpy_xtp==2.2.32.2.3`，注册 `XtpGateway`，在不读取账户配置、不连接、不订阅、不下单的前提下识别 SSE/SZSE、10 个连接键和交易/行情双通道状态。生产门禁也会拒绝必需键存在但值为 `null` 或空字符串的配置。剩余外部通道缺口收敛为账户所有者提供仓库外 XTP 股票测试账户参数，并执行真实连接/回报/重连/长跑验收。
