...
| 模块 | 当前状态 | 完成度 | 未完成重点 |
| --- | --- | ---: | --- |
| 在线选股能力 | AlphaSift 可用，策略接口返回 8 个策略，候选级数据质量/缺失字段/来源已有基础标注，Web 可查看 source health，自动 Agent 已按历史/当前健康动态调整 snapshot 来源顺序，实时行情、资金流和新闻具备 provider 级故障切换；行情与资金流健康可跨 API 重启延续；5,529 只全市场长任务已通过 | 约 99% | 仍需生产样本与不同市场长期校准证据 |
...
- 已新增默认关闭的 `agent_calibration_shadow` 定时采样任务，使用显式 `市场:策略` 配对和 AlphaSift `market_scope` 前置校验，只写独立 dry-run Agent 样本且强制零订单，不修改正式自动交易设置、最后运行、告警或熔断历史。当前部署只启用 `cn:dual_low`；AlphaSift 0.2.0 没有 US 范围的内置策略且不支持 HK screen，因此 US/HK 长期样本仍是外部依赖缺口。
...
未完成程度：当前约 1% 未完成，功能面已基本闭环；A 股生产样本已可无人值守积累，剩余为样本成熟所需时间、US/HK AlphaSift 能力和真实 gateway 长跑证据。
