...
| 模块 | 当前状态 | 完成度 | 未完成重点 |
| --- | --- | ---: | --- |
| 在线选股能力 | AlphaSift 可用，策略接口返回 9 个策略（8 个内置 A 股策略和 DSA 扩展的 `us_large_cap_momentum`），候选级数据质量/缺失字段/来源已有基础标注，Web 可查看 source health；美股 snapshot 在 yfinance 限流时可按请求降级到腾讯行情；5,529 只全市场长任务已通过 | 约 99% | 港股 screen 尚不受 AlphaSift 0.2.0 支持；仍需生产样本长期校准证据 |
...
- 生产校准现在有独立只读验收门禁 `scripts/check_agent_calibration_evidence.py`，默认按 A 股/港股/美股分别门禁 90 天运行量、目标快照观测率、最新成熟样本、观测日期、时效、版本和摘要完整性。2026-07-21 首个有效后台 shadow 样本后，A 股为 19 个 completed run、18 个目标快照（94.74%）、5 个观测日、0 个成熟样本，港股/美股仍无有效样本；因此能力已可测且 A 股会自动积累，但长期证据仍明确未通过。
- 已新增默认关闭的 `agent_calibration_shadow` 定时采样任务，使用显式 `市场:策略` 配对和 AlphaSift `market_scope` 前置校验，只写独立 dry-run Agent 样本且强制零订单，不修改正式自动交易设置、最后运行、告警或熔断历史。DSA 扩展策略已让 `us:us_large_cap_momentum` 可用；真实 US shadow `ss-agent-20260720165408-d91ad2b9` 生成 3 个候选和 1 个计划且零下单。HK screen 仍是 AlphaSift 0.2.0 的外部能力缺口。
...
- AlphaSift 可用，当前提供 9 个策略：8 个内置 A 股策略 `dual_low`、`quality_value`、`balanced_alpha`、`momentum_quality`、`capital_heat`、`oversold_reversal`、`shrink_pullback`、`volume_breakout`，以及 DSA 扩展的美股策略 `us_large_cap_momentum`。
- Web 选股页可启动筛选任务。
...
未完成程度：当前约 1% 未完成，功能面已基本闭环；A 股和美股生产样本已可无人值守积累，剩余为样本成熟所需时间、HK AlphaSift 能力和真实 gateway 长跑证据。
