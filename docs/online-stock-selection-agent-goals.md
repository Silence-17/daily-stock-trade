...
- 生产校准现在有独立只读验收门禁 `scripts/check_agent_calibration_evidence.py`，默认只统计 `trigger_source=agent_calibration_shadow`，避免手工或普通自动交易样本误充生产校准证据；并按 A 股/港股/美股分别门禁 90 天运行量、目标快照观测率、最新成熟样本、观测日期、时效、版本和摘要完整性。2026-07-21 的 shadow 专属门禁记录 A 股 6/6（2 个观测日）、港股 5/5（1 个观测日）、美股 7/7（1 个观测日），观测率均为 100%，但成熟样本均为 0，因此长期证据明确未通过。门禁执行前后 shadow run 总数均为 20，确认检查只读。
