...
| 自动模拟交易 | 已能从 AlphaSift 候选生成交易计划，支持 dry-run、手动审批、按交易时段写入本地 paper 订单，并支持可交易窗口诊断、首次自动买入调度对齐下一交易窗口、基础止损/止盈/移动止损/持仓天数卖出、超时未走强卖出、按比例分批卖出、active 防守决策信号触发的策略失效卖出、单票/组合/行业仓位暴露上限、跨币种预算、按股票/行业目标缺口缩量补仓和基础组合再平衡、股票黑名单、候选级/账户级/市场级基础风控 | 约 99% | 仍缺真实 gateway 长跑验证和完整多约束组合优化 |
...
- 长期稳定版仍缺真实 gateway 长跑、完整多约束组合优化、外部权限下的全市场长任务证据和 Agent 跨运行闭环；显式股票/行业目标缺口补仓、基础跨币种估值、预算风控、逐日期历史股票池、可恢复全市场因子采集、样本外前瞻评价、时间点策略重放以及现金/整手/等权目标组合回测已完成。
...
- 自动交易已有基础止损、止盈、移动止损、持仓天数卖出、超时未走强卖出、按比例分批卖出、active 防守决策信号触发的策略失效卖出，以及基于单票/总仓位/行业仓位暴露上限、跨币种预算和股票/行业目标权重的缺口补仓与卖出侧组合再平衡；仍缺任意权重和多约束优化驱动的完整再平衡策略。
