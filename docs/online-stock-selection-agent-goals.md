...
- 校准证据状态告警现已形成页面可见的投递闭环：只读 API 和 Agent 控制台展示最近 transition、尝试/成功/失败/可重试数量及脱敏渠道结果。真实运行中 Feishu 返回 `send_failed`，页面明确显示 1 次失败且可重试；该外部机器人尚未启用，但系统不再沉默失败，也不会让通知异常影响证据监控或触发订单。
