...
| 自动模拟交易 | 已能从 AlphaSift 候选生成交易计划，支持 dry-run、手动审批、按交易时段写入本地 paper 订单，并支持可交易窗口诊断、首次自动买入调度对齐下一交易窗口、基础止损/止盈/移动止损/持仓天数卖出、超时未走强卖出、按比例分批卖出、active 防守决策信号触发的策略失效卖出、单票/组合/行业仓位暴露上限、跨币种预算、按股票/行业目标缺口缩量补仓、候选评分加权、20 日逆波动率或两两相关性上限约束分配和基础组合再平衡、股票黑名单、候选级/账户级/市场级基础风控 | 约 99% | 仍缺真实 gateway 长跑验证、协方差目标函数和任意目标权重优化 |
...
- 长期稳定版仍缺真实 gateway 长跑、协方差目标函数/任意目标权重优化、外部权限下的全市场长任务证据和多模型人工验收闭环；候选评分加权、20 日逆波动率、基于 trailing returns 的顺序两两相关性约束、风险字段缺失阻断、显式股票/行业目标缺口补仓、基础跨币种估值、预算风控、逐日期历史股票池、可恢复全市场因子采集、样本外前瞻评价、跨运行状态迁移与可选门禁、时间点策略重放以及现金/整手/等权目标组合回测已完成。
...
- 自动交易已有基础止损、止盈、移动止损、持仓天数卖出、超时未走强卖出、按比例分批卖出、active 防守决策信号触发的策略失效卖出，以及基于候选评分、同一决策时点 20 日逆波动率或本地 trailing returns 顺序两两相关性上限、现金/每日预算、单票/总仓位/行业仓位暴露上限、跨币种预算和股票/行业目标权重的约束分配、缺口补仓与卖出侧组合再平衡；仍缺协方差目标函数和任意目标权重驱动的完整优化器。
...
- 扩展交易规则：基础止损、止盈、移动止损、持仓天数、超时未走强、按比例分批卖出、策略失效卖出、评分加权/20 日逆波动率约束分配、暴露上限驱动和目标权重驱动的缺口补仓/基础组合再平衡，以及跨币种估值和预算风控已落地；继续补真实 gateway 长跑验证和协方差/任意目标函数组合优化。
...
- 卖出、止损、止盈、暴露上限、跨币种预算、评分加权/20 日逆波动率约束分配和目标权重驱动的基础调仓规则已落地；继续补真实 gateway 长跑验证和协方差/相关性及任意目标函数组合优化。
