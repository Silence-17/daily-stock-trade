...
| Web 操作页 | 已有选股页、模拟交易页和独立自动选股 Agent 控制台，支持暂停确认、一键恢复、立即 dry-run 演练、后台调度状态、持久化可筛选后台任务日志、任务趋势摘要、7/30/90 天后台任务长期指标、任务健康检查、模拟账户历史筛选/恢复/切换/隐藏式批量清理、权益曲线、日度收益表、月度收益表、按 Agent run 创建时间筛选的窗口级绩效矩阵、已记录候选的 1/5/10/20 日样本外前瞻评价、时间点因子采集/覆盖检查/单日重放、逐日期历史全市场分批恢复、进程重启孤儿租约诊断与安全接管、现金/整手/等权或显式股票目标权重组合回测、自动或显式分红/拆并股、显式零碎股现金补偿、固定或 2005 年以来 A 股历史双边/单边印花税、版本化常规/低佣/零成本档位、自定义实际账户费率与成本审计、可交易窗口、风控统计、复核质量摘要、运行时间线、分页历史、策略/市场/状态/时间范围筛选、`/agent-console/<runUid>` 独立详情路由、查询参数兼容深链和单次/最近 run JSON 导出 | 约 99% | 仍缺 BSE 公司行动可靠来源与长期样本 |
...
- 长期稳定版仍缺真实 gateway 长跑、BSE 公司行动可靠来源，以及生产样本下多模型/多市场长期校准；5,529 只全市场长任务、SSE/SZSE 免费公司行动在线证据、2005 年以来历史税制、版本化参考成本档位、自定义实际账户费率、Agent run 人工验收、历史意见上下文复用、版本化收益/风险目标和风险门禁隔离已完成。
...
- 页面已展示 AlphaSift 源健康、本轮降级原因、模拟交易跨模块健康摘要、跨 run 整体质量、具体来源降级趋势、snapshot 动态路由，以及候选实时行情/资金流/新闻 provider 的优先级、熔断、冷却、半开探测、跨 API 重启恢复状态和下轮路由建议；全市场长任务与 SSE/SZSE 公司行动证据已完成，下一项继续补 BSE 公司行动可靠来源与真实 gateway 长跑验收。
...
- 绩效摘要已有 MVP，并已包含本地 paper 权益路径页面曲线、日度收益、月度收益、最大回撤、换手率、按策略/行业归因、窗口级绩效矩阵、Agent run 创建时间窗口筛选、已记录候选的 1/5/10/20 日样本外前瞻评价、带字段覆盖门禁的单日时间点策略重放，以及按现金、A 股整手和等权或显式股票目标权重执行的跨日组合回测；Tushare 优先、EastMoney/巨潮 fallback 的已实施公司行动已接入持久化采集，SSE/SZSE 在线证据已完成，拆并股支持按显式结算价兑付零碎股。A 股印花税已按 2005/2007/2008/2023 官方生效日和买卖方向自动切换，买方税进入整手与现金约束；2005-01-24 之前仍 fail-closed。常规、低佣和零成本参考档位已有版本审计，实际券商条款通过 `custom` 显式输入而不由系统猜测。全市场在线长任务证据已完成，剩余为 BSE 公司行动可靠来源。
- 全市场采集作业已把持久化租约与进程内任务队列对账并暴露 `active/orphaned/retryable/complete`，服务重启遗留作业可由普通恢复安全接管，活跃作业继续以 409 防重复；在线验收器门禁规模、完整因子覆盖率、精确日线/估值覆盖、源错误、检查点回退和停滞。历史成员改为 Tushare 优先、BaoStock 沪深生命周期加北交所官方名单/代码映射/最终摘牌公告的完整 fallback；2026-07-17 在线请求返回 5,529 只且 BSE 未解释缺口为 0。干净长任务完成 5,529 项和 111 批，完整因子 5,494（99.367%）、精确日线 5,522（99.8734%）、估值完整 5,502（99.5117%）、源错误 27（0.4883%），验收无失败项；SSE/SZSE 公司行动已有免费源在线持久化证据，BSE 仍保持显式空覆盖。
