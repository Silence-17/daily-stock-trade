...
- 每日校准 shadow 会在生成新 `cross_run_quality` 快照前，按股票去重补齐已经达到最短观察期的缺失前瞻日线，并显式统计补数尝试和未到期跳过；正式自动交易与只读证据监控不承担联网补数，因此成熟样本会随真实交易日推进而增长，同时保持订单路径无副作用。
