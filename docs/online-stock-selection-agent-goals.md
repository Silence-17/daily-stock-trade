...
| 选股 Agent | 已有自动选股运行记录、结构化 Agent 计划、跨市场动态目标、规则派生计划档位/执行路由/预算上限/降级动作、默认关闭的 LLM 动态计划、每轮运行总结、基础复核质量摘要、基础 Agent 工作流状态机、严格样本外跨运行质量状态与可选买入门禁、版本化逐日收益/风险效用目标、按日结构化运行总结、候选级仓位计划/规则风控复核、规则 Agent/LLM 模型与版本的 1/5/10/20 日复核质量矩阵、最终复核成熟质量自动校准/门禁、默认关闭的 LLM 买入前复核、手动可选 LLM 复盘、run 级人工验收及后续上下文复用、候选决策审计和交易计划审计，但还不是完整 Agent 工作流 | 约 99% | 缺少生产样本长期校准和不同市场长期验证证据 |
