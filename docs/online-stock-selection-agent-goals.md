...
截至 2026-07-10，本系统已经具备 AlphaSift 在线选股、本地模拟交易账本、Web 入口、定时自动买入、可交易窗口诊断、关键异常 alert 路由通知、vn.py 提交态审计、成交回报同步入口、规则 Agent 买入前二次复核、默认关闭的 LLM 动态计划、默认关闭的 LLM 买入前复核、基础复核质量摘要和手动可选 LLM 复盘，但还没有达到完整 Agent 化、稳定自动化和 vn.py 标准化模拟交易的目标。
...
| Web 操作页 | 已有选股页、模拟交易页和独立自动选股 Agent 控制台，支持暂停确认、一键恢复、立即 dry-run 演练、后台调度状态、持久化可筛选后台任务日志、任务趋势摘要、任务健康检查、模拟账户历史筛选/恢复/切换/隐藏式批量清理、权益曲线、日度收益表、月度收益表、按 Agent run 创建时间筛选的窗口级绩效矩阵、可交易窗口、风控统计、复核质量摘要、运行时间线、分页历史、策略/市场/状态/时间范围筛选、`/agent-console/<runUid>` 独立详情路由、查询参数兼容深链和单次/最近 run JSON 导出 | 约 99% | 缺少完整历史行情驱动的回测级绩效矩阵和真实回测指标 |
...
| 稳定性与可观测 | 有状态接口、错误提示、AlphaSift source health 页面视图、候选级数据质量标注、自动交易数据质量诊断、可交易窗口诊断、读取持久化最近事件的后台任务健康检查、持久化可筛选后台任务日志、任务趋势摘要、任务事件保留/低频清理策略、Agent run 时间线、基础失败重试、交易计划恢复矩阵、手动恢复扫描、部分成交超时安全归档、熔断状态/恢复入口、自动交易系统事件告警历史和 alert 路由通知尝试审计 | 约 89% | 缺少长期指标、完整自动恢复策略和性能优化 |
...
- 页面级“为什么不可用”已有后端 readiness 诊断摘要、AlphaSift 依赖状态、状态加载失败分类和持仓估值降级提示；仍需补跨模块统一健康视图。

需要做：

- 拆分 `GET /vnpy-paper/status` 为轻量状态和完整快照两个层级。
- 给持仓估值增加短 TTL 缓存和价格源失败页面级降级说明已完成；继续补价格源健康指标。
- Web 页面已展示本地账本、自动任务、AlphaSift 依赖、调度窗口、交易窗口、熔断、vn.py bridge 可用性摘要和持仓估值降级提示，并对状态路由 404、状态超时和本地连接失败提供明确提示；继续补后端版本和跨模块统一健康视图。
...
- Agent 计划阶段已有基础诊断、规则派生计划档位/降级动作、默认关闭的 LLM 动态计划、基础复核质量摘要和基础可解释状态机；仍缺跨市场动态目标、更完整的每日目标函数、长周期质量评估和跨运行状态机。
...
- 每轮运行总结已有基础结构化 `agent_summary`，跨 run 的每日结构化总结已落地；手动可选 LLM 复盘已能基于结构化审计数据写入 `diagnostics.llm_recap`，但尚未进入自动交易决策闭环。
...
- vn.py 订单状态回写支持部分成交审计，`parttraded` / `partial_filled` 会把交易计划和候选决策标记为 `part_filled`，记录已成交数量/价格并等待成交回报写入本地 Portfolio；若长期未收到成交回报，恢复扫描会以 `vnpy_partial_fill_timeout` 归档并阻止自动重试，避免重复委托。
...
- 交易计划表已有基础状态、手动审批成交、受控计划重试、retry 次数/冷却审计、vn.py `part_filled` 部分成交审计、`part_filled` 超时安全归档、`submitted` 订单超时归档、`cancel_requested` 主动撤单能力、撤单请求超时归档、活跃提交态防重复保护、只读交易计划恢复矩阵和手动恢复扫描，但还缺少完整订单生命周期的真实 gateway 长跑验证。
- 自动失败恢复已有页面/API 受控重试、后台到期重试调度、页面手动恢复扫描、提交态/部分成交/撤单请求超时归档、数据质量阻断告警历史、熔断状态诊断、手动恢复基础、交易计划恢复矩阵、告警中心系统事件历史和 alert 路由通知尝试审计；仍缺更完整的自动恢复策略。
...
- 当前 AlphaSift snapshot/daily 源已有 Web 可见失败计数和错误摘要；仍需要更完整的指标采集和跨 run 趋势。
- 自动选股依赖行情、日线、资金和新闻，多源失败时缺少统一质量评分。
- 数据质量 `stale/unavailable` 和 AlphaSift 筛选异常已写入告警中心系统事件历史并复用 alert 路由外发通知，Web 模拟交易页已展示最近自动交易告警历史；仍缺自动恢复策略。

需要做：

- 基础数据质量合同已下沉到 AlphaSift screen 候选和 Web 选股页；Web 已展示 AlphaSift source health，继续补跨 run 统一数据源健康视图：`ok`、`partial`、`stale`、`unavailable`。
- 候选结果已标注数据来源、缺失字段和是否使用 screen cache；后续继续补来源权重、独立状态源和缺失字段处置策略。
- 自动交易已有基础 gate，只允许消费 `ok` 或可接受的 `partial` 数据；后续需扩展更细的来源权重、候选级缺失字段策略和自动恢复策略。
- 页面已展示 AlphaSift 源健康和本轮降级原因；继续补跨模块统一健康指标。
...
- `vnpy_paper` 当前覆盖买入委托提交、自动按比例卖出提交、主动撤单请求、订单/成交状态回写、提交态/部分成交/撤单请求超时归档和活跃委托防重复；真实 gateway 长运行和部分成交后续对账仍未完成。
...
- 绩效摘要已有 MVP，并已包含本地 paper 权益路径页面曲线、日度收益、月度收益、最大回撤、换手率、按策略/行业归因、窗口级绩效矩阵和 Agent run 创建时间窗口筛选；仍缺完整历史行情回测和更严格的回测级绩效评估矩阵。
...
- 运行中状态、任务级下次执行时间、最近错误、读取持久化最近事件的后台任务健康检查、持久化可筛选后台任务日志、任务趋势摘要、任务事件保留/低频清理策略和 Agent run 时间线已有基础展示；继续补长期指标。
...
- 纸面交易绩效摘要已有基础能力，已包含权益曲线、日度收益、月度收益、FIFO 卖出胜率、最大回撤、换手率、策略/行业贡献评估、窗口级绩效矩阵和 Agent run 创建时间窗口筛选；仍缺完整历史行情回测和回测级绩效矩阵。
...
- 增加 Web 测试，覆盖任务历史和详情。
- 基础模拟交易绩效摘要已覆盖收益、收益率、权益曲线、日度收益、月度收益、成交额、买卖次数、FIFO 卖出胜率、最大回撤、换手率、当前仓位、成交/计划、跳过原因、策略/行业归因、窗口级绩效矩阵和 Agent run 创建时间窗口筛选；继续增加完整回测和回测级绩效矩阵。
