...
截至 2026-07-23，本系统已经具备 AlphaSift 在线选股、本地模拟交易账本、Web 入口、定时自动买入、可交易窗口诊断、跨模块系统健康视图、关键异常 alert 路由通知、vn.py 提交态审计、内置即时撮合 gateway、成交回报同步入口、规则 Agent 买入前二次复核、默认关闭的 LLM 动态计划、默认关闭的 LLM 买入前复核、基础复核质量摘要、手动可选 LLM 复盘、Agent run 人工验收反馈，以及可分别验证手动触发和后台调度触发的在线 Agent -> DSA_SIM 验收门禁。Agent 控制台已把外部 gateway 零连接预检与三市场校准证据集中为只读生产就绪摘要。A 股、美股和港股均已有可执行策略；真实 US/HK shadow 均完成 3 候选、1 计划、0 下单，并验证腾讯行情 fallback、港股直连行情与首次交易按需汇率刷新。官方 `vnpy_ctp==6.7.11.4` 已完成无凭据安装、注册、零连接预检和 Windows 冻结打包验收。尚未达到的目标包括外部账户连接与真实 gateway 长跑，以及生产样本下多模型、多市场长期校准。
...
| 模块 | 当前状态 | 完成度 | 未完成重点 |
| --- | --- | ---: | --- |
| 在线选股能力 | AlphaSift 可用，策略接口返回 10 个策略（8 个内置 A 股策略、`us_large_cap_momentum` 和 `hk_liquid_momentum`），候选级数据质量/缺失字段/来源已有基础标注；美股在 yfinance 限流时降级到腾讯行情，港股使用有界股票池、腾讯直连快照和 AlphaSift 原生 LLM ranker；5,529 只全市场长任务已通过 | 约 99% | 仍需生产样本长期校准证据 |
...
| Web 操作页 | 已有选股页、模拟交易页和独立自动选股 Agent 控制台；控制台集中展示外部 gateway 零连接预检与三市场校准证据，并对预检接口失败独立降级；其余支持暂停确认、一键恢复、立即 dry-run 演练、后台调度状态、持久化可筛选后台任务日志、任务趋势摘要、7/30/90 天后台任务长期指标、任务健康检查、模拟账户历史筛选/恢复/切换/隐藏式批量清理、权益曲线、日度收益表、月度收益表、按 Agent run 创建时间筛选的窗口级绩效矩阵、已记录候选的 1/5/10/20 日样本外前瞻评价、时间点因子采集/覆盖检查/单日重放、逐日期历史全市场分批恢复、进程重启孤儿租约诊断与安全接管、现金/整手/等权或显式股票目标权重组合回测、自动或显式分红/拆并股、显式零碎股现金补偿、固定或 2005 年以来 A 股历史双边/单边印花税、版本化常规/低佣/零成本档位、自定义实际账户费率与成本审计、可交易窗口、风控统计、复核质量摘要、运行时间线、分页历史、策略/市场/状态/时间范围筛选、`/agent-console/<runUid>` 独立详情路由、查询参数兼容深链和单次/最近 run JSON 导出 | 约 99% | 仍缺生产样本长期校准 |
| vn.py 集成 | 已有可选 `OrderRequest` / `CancelRequest` 映射、`MainEngine.send_order` / `cancel_order` 桥接入口、订单/成交/账户/持仓回写 API、注入式 EventEngine attach、opt-in runtime bootstrap、统一外部 Gateway 插件安装/打包清单、内置即时撮合 gateway、Python 3.13 隔离环境中的真实 vn.py 4.4.0 Agent 计划到 Portfolio 成交验收、官方 CTP 插件零连接注册与冻结打包验收，以及 Windows Desktop 和 Linux Docker 的 opt-in vn.py 发布产物验收 | 约 97% | 外部账户连接、真实回报、重连和长期订阅稳定性仍未验证 |
...
- 外部 Gateway 不再停留在“人工预装插件”的不可复现状态：`VNPY_GATEWAY_PLUGINS_JSON` 统一驱动隔离环境、Docker 和 Windows/macOS Desktop 的插件安装、模块导入、PyInstaller 收集与冻结后探针；未开启 vn.py 却声明插件、URL/marker/重复或非法清单、安装失败和产物漏模块都会 fail-closed。Windows/Python 3.13 已实际安装官方 `vnpy_ctp==6.7.11.4`，注册 `CtpGateway`，在不读取账户配置、不连接、不订阅、不下单的前提下识别 8 个连接键，并完成包含 `--collect-all vnpy_ctp` 的冻结构建及产物插件探针。生产门禁也会拒绝必需键存在但值为 `null` 或空字符串的配置。剩余外部通道缺口收敛为部署方提供仓库外账户参数，并执行真实连接/回报/重连/长跑验收。
- 2026-07-23 修复后的部署态 DSA_SIM 完成 300 秒只读长跑：148/148 次采样成功，API、runtime、连接、EventEngine bridge 和四类回调注册率均为 100%，进程/Gateway 变化与 handler 失败均为 0。独立 Gateway 另完成 300 秒零下单断线恢复验收，在第 120 秒注入断线、第 122 秒恢复，记录 1 次重连尝试、1 次成功、0 次失败、2 次账户事件，最终 connected，150 次采样中 149 次连通（99.3333%）。随后只读校准门禁继续严格返回 15 项阈值缺口：CN 11/1、HK 10/1、US 12/1 个运行/有效观测，均为 1 个观察日和 0 个五日成熟样本。该证据增强内置模拟链路的长跑与恢复可信度，不替代外部 gateway，也不制造成熟样本。
- 2026-07-23 校准 shadow 首轮调度不再继承 API 注册时刻：全部组合尚未到期时等待最晚持久化资格时间，首次部署或已有组合可采样时保留 5 分钟启动缓冲，证据监控在其后 5 分钟运行。只读 API 同时保留单市场 `next_eligible_at`、新增整批 `all_eligible_at` 并叠加 runtime 的真实 `next_scheduled_at`，Agent 控制台优先展示真实整批任务时间。重启后任务从错误的 18:00 对齐到 11:05:46；真实 scheduler 在 237.594 秒内完成 CN `ss-agent-20260723030546-471be8da`、US `ss-agent-20260723030715-06cfb4f8`、HK `ss-agent-20260723030850-6a3a7d19`，共 9 个候选、7 个 dry-run 计划，0 cadence skip、0 失败、0 提交。Agent run 61->64，Portfolio 成交保持 23。随后只读监控耗时 0.183 秒，生产证据改善为 CN 11/1、HK 10/1、US 12/1 个运行/有效观测，均覆盖 1 个观察日；三市场 `latest_observation_missing` 已消失，失败项 18->15，但成熟五日样本仍为 0，因此门禁继续正确显示积累中。
- 2026-07-22 可重复验收门禁在当前 API/vn.py 4.4.0 运行态分别通过手工触发与 scheduler 触发正向成交：run `ss-agent-20260722095106-c813b58c` 和 `ss-agent-20260722095410-1488e608` 均生成 3 个可追溯候选和 3 个终态 `filled` 计划，精确资金变化为 -29,664 元，持仓变化为 `000001 +900`、`600016 +2800`、`601668 +2100`；scheduler 任务耗时 64.405 秒且持久化事件关联同一 run UID。两轮均使用隔离账本，随后恢复账户 2、全部自动化设置并隐藏测试账户，DSA_SIM 保持 connected 且四类 EventEngine 回调完整。该证据证明内置模拟交易与无人值守调度链路闭环，不替代真实外部 gateway 生产验收。
...
- 长期稳定版仍缺真实 gateway 长跑，以及生产样本下多模型/多市场长期校准；5,529 只全市场长任务、SSE/SZSE/BSE 免费公司行动在线证据、2005 年以来历史税制、版本化参考成本档位、自定义实际账户费率、Agent run 人工验收、历史意见上下文复用、版本化收益/风险目标和风险门禁隔离已完成。
- 生产校准现在有独立只读验收门禁 `scripts/check_agent_calibration_evidence.py`，默认只统计 `trigger_source=agent_calibration_shadow`，避免手工或普通自动交易样本误充生产校准证据；并按 A 股/港股/美股分别门禁 90 天运行量、目标快照观测率、最新成熟样本、观测日期、时效、版本和摘要完整性。2026-07-21 最新三市场任务在 114.977 秒内完成，每市场 3 个候选，共生成 2 个 dry-run 计划且 0 提交；随后门禁记录 A 股 7/7（2 个观测日）、港股 6/6（1 个观测日）、美股 8/8（1 个观测日），观测率均为 100%，但成熟样本均为 0，因此长期证据明确未通过。门禁执行前后 shadow run 总数均为 23，确认检查只读。
- 已新增默认关闭的 `agent_calibration_shadow` 定时采样任务，使用显式 `市场:策略` 配对和 `market_scope` 前置校验，只写独立 dry-run Agent 样本且强制零订单，不修改正式自动交易设置、最后运行、告警或熔断历史。真实 US shadow `ss-agent-20260720165408-d91ad2b9` 与 HK shadow `ss-agent-20260720171834-1109843a` 均生成 3 个候选和 1 个计划且零下单；CN/US/HK 均可进入无人值守样本积累。
- shadow scheduler 已增加基于持久化同来源/市场/策略最近完成 run 的跨进程采样节流，进程重启后的 5 分钟首次任务不会重复灌入未到期样本；任务事件保留逐组合 `cadence_skips`、样本年龄、剩余时间和下一可采样时间。最多 5 分钟宽限只吸收三市场串行执行与调度抖动，不放宽配置的小时/日级采样节奏。
- 2026-07-21 重启运行态验收中，最终首次 shadow 任务在 0.02 秒内以三组 cadence skip 结束，尝试/候选/计划/提交/失败均为 0；CN/US/HK 分别关联最近 run 并给出次日 02:49-02:51 的可采样时间。随后只读门禁仍为 CN 9、US 10、HK 8，总数没有被两次重启抬高，剩余失败项明确只是观测日和成熟前瞻样本尚未达到阈值。
- 校准证据状态告警现已形成页面可见且可恢复的投递闭环：只读 API 和 Agent 控制台展示最近 transition、尝试/成功/失败/可重试数量、逐渠道尝试号及等待/到期/送达/不可重试/耗尽状态。`vnpy_paper_auto_retry` 约每 5 分钟消费可重试失败，逐渠道持久化计数并跨重启延续，最多 3 轮投递；5 秒到期宽限避免固定节拍在截止前擦边后永久错过。该路径只重发通知，不运行 Agent 或创建订单，告警历史异常也不会阻断同轮订单恢复。真实重启验收从 Feishu `#1` 推进到 `#2 waiting`，修正节拍边界后推进到 `#3 exhausted`，三轮均为 `send_failed`；47 个 Agent run、126 个计划和 22 笔成交保持不变。外部机器人尚未启用，但系统不再沉默失败，也不会无限重试或让通知异常影响证据监控。
- 生产校准证据 API 与 Agent 控制台现直接展示各市场/策略的持久化采样 cadence、当前 eligibility 和下一次可采样时间；页面不再只显示“积累中”，且读取过程不运行 Agent、不缩短间隔、不制造样本。
- vn.py 标准化本地模拟链路已可启动并完成真实事件回写，官方 CTP 插件也完成无凭据结构与打包验收；剩余重点约为 3%：外部账户连接、真实回报和长跑恢复验收。
...
- AlphaSift 可用，当前提供 10 个策略：8 个内置 A 股策略 `dual_low`、`quality_value`、`balanced_alpha`、`momentum_quality`、`capital_heat`、`oversold_reversal`、`shrink_pullback`、`volume_breakout`，以及 DSA 扩展的美股策略 `us_large_cap_momentum` 和港股兼容策略 `hk_liquid_momentum`。
- Web 选股页可启动筛选任务。
...
- 每轮规则 Agent 计划会持久化 `recent_run_context`，严格按同触发来源/策略/市场汇总最近 5 次 run 的状态、数据质量、候选/计划/成交/跳过、成交率、连续失败和最多 3 份已完成 LLM 复盘；默认关闭的 LLM 动态计划 v3 把复盘标为不可信历史审计上下文，并继续受只收紧 evaluator 约束，Agent 控制台展示最近运行状态。
...
- 每日校准 shadow 会在生成新 `cross_run_quality` 快照前，只读取最近 90 天同市场/策略、已完成的 `agent_calibration_shadow` 决策，并按股票去重补齐已经达到最短观察期的缺失前瞻日线；审计请求成功/失败、未到期跳过、实际供应商、写入行数及补齐/仍缺失锚点。正式自动交易与只读证据监控不承担联网补数，普通 Agent 决策不能充当生产校准成熟样本。
...
- Agent 计划阶段已有基础诊断、跨市场动态目标、规则派生计划档位/降级动作、最近运行与人工验收上下文、默认关闭的 LLM 动态计划、基础复核质量摘要、按规则/LLM 模型版本分组的长周期复核质量矩阵、成熟质量自动校准/门禁、版本化逐日收益/风险目标、基础可解释状态机和跨运行前瞻质量迁移；生产证据已具备只读验收合同，仍缺达到门槛的长期样本和不同市场验证证据。
...
- 每轮运行总结已有基础结构化 `agent_summary`，跨 run 的每日结构化总结和最近 5 次运行上下文已落地；手动可选 LLM 复盘会基于结构化审计数据写入 `diagnostics.llm_recap`，并在用户显式开启 LLM 动态计划时进入下一轮同来源/策略/市场上下文。复盘文本不能作为指令，不能放宽候选数、单票预算或最低分边界。
...
- 自动买入在交易时段限制生效时，24 小时及以上周期会把首次后台任务锚定到可交易窗口开始后 5 分钟；窗口中重启且当天尚未正式运行时选择仍有准备余量的当前或下一段窗口，当天已运行则跳到下一交易日，避免启动时刻漂移、午休前擦边和重启重复下单。小于 24 小时的轮询保持原间隔语义；状态接口通过 `auto_trade_readiness.timing_alignment` 按任务实际日期关联调度器下次运行时间和交易窗口。
...
- 市场环境过滤已有基础大盘红绿灯、持久化快照市场宽度、涨跌停强度退潮 gate、可配置快照新鲜度、盘中主指数/A 股实时涨跌家数宽度和版本化跨市场联动规则；指数 fallback 已区分 provider、抓取时刻、实时/session bar/收盘日线粒度，并可默认严格要求指数与宽度均有完整、新鲜的 provider quote as-of。efinance 的股票、ETF、指数和全市场宽度已恢复 provider 更新时间为 UTC，宽度仅在全部有效计数行具备时间时报告聚合 as-of；AStockDataFetcher 会在 efinance 后、AkShare 批量端点前直接读取 EastMoney 原始 `f124/f297`，且仅在响应总数完整时提供指数或宽度；AkShare 的新浪/腾讯单票直连也保留 provider 时间。跨市场联动会对所有标为实时的关联指数要求新鲜 provider 时间，并拒绝最大时间偏差超过 120 秒的组合；已收盘 `session_bar` 明确不宣称同步且继续作为背景允许。剩余缺口是生产样本阈值校准；严格模式会对不可证明时间、截断响应或宽度覆盖率未明确达到 100% 的证据 fail-closed，操作员只能通过可审计开关显式降级。
...
- 页面已展示 AlphaSift 源健康、本轮降级原因、模拟交易跨模块健康摘要、跨 run 整体质量、具体来源降级趋势、snapshot 动态路由，以及候选实时行情/资金流/新闻 provider 的优先级、熔断、冷却、半开探测、跨 API 重启恢复状态和下轮路由建议；全市场长任务与 SSE/SZSE/BSE 公司行动证据已完成，下一项继续真实 gateway 长跑验收和生产校准。
...
- 绩效摘要已有 MVP，并已包含本地 paper 权益路径页面曲线、日度收益、月度收益、最大回撤、换手率、按策略/行业归因、窗口级绩效矩阵、Agent run 创建时间窗口筛选、已记录候选的 1/5/10/20 日样本外前瞻评价、带字段覆盖门禁的单日时间点策略重放，以及按现金、A 股整手和等权或显式股票目标权重执行的跨日组合回测；组合回测已展示年化/基准/超额收益、周期胜率、周期波动率、下行偏差、Sharpe、Sortino 和 Calmar，且不会把周期统计标成日频指标。Tushare 优先、EastMoney/巨潮/北交所官方公告 fallback 的已实施公司行动已接入持久化采集，SSE/SZSE/BSE 在线证据已完成，拆并股支持按显式结算价兑付零碎股。A 股印花税已按 2005/2007/2008/2023 官方生效日和买卖方向自动切换，买方税进入整手与现金约束；2005-01-24 之前仍 fail-closed。常规、低佣和零成本参考档位已有版本审计，实际券商条款通过 `custom` 显式输入而不由系统猜测。全市场在线长任务证据已完成；生产校准证据已有非零退出门禁、只读 scheduler 监控、在线 API 和 Agent 控制台三市场缺口视图。当前 completed shadow runs 为 CN 10、HK 9、US 11；旧质量快照因缺少来源范围证明分别有 10/9/11 个被降为 unknown，严格 shadow-only 当前五日成熟样本三市场均为 0。后续自然到期 shadow 将开始积累可验证的新范围快照，运行量、有效观测、观察天数和成熟样本仍未达到最终门槛。
- 全市场采集作业已把持久化租约与进程内任务队列对账并暴露 `active/orphaned/retryable/complete`，服务重启遗留作业可由普通恢复安全接管，活跃作业继续以 409 防重复；在线验收器门禁规模、完整因子覆盖率、精确日线/估值覆盖、源错误、检查点回退和停滞。历史成员改为 Tushare 优先、BaoStock 沪深生命周期加北交所官方名单/代码映射/最终摘牌公告的完整 fallback；2026-07-17 在线请求返回 5,529 只且 BSE 未解释缺口为 0。干净长任务完成 5,529 项和 111 批，完整因子 5,494（99.367%）、精确日线 5,522（99.8734%）、估值完整 5,502（99.5117%）、源错误 27（0.4883%），验收无失败项；SSE/SZSE/BSE 公司行动均已有免费源在线持久化证据，北交所旧代码历史通过官方映射闭环。
...
- 2026-07-22 同一验收门禁已在隔离账户补齐当前运行态的双触发正向证据：直接触发和 scheduler 触发分别完成 3/3 成交，候选、决策、计划、成交、资金和持仓变化全部一致；scheduler 终态事件关联 run `ss-agent-20260722095410-1488e608`，耗时 64.405 秒。两轮结束后账户、自动交易开关、1440 分钟间隔、`vnpy_paper` 执行模式和交易时段门禁均恢复，测试账户清理成功。
- 2026-07-23 三市场校准真实批次按最晚资格时间自动启动并完成 3/3 市场、9 个候选、7 个 dry-run 计划、0 提交；后续证据监控确认每市场新增首个具备明确 shadow scope 的有效观测，且 Agent run 增量为 3、Portfolio 成交增量为 0。该证据关闭了“注册时刻漂移导致当天延迟”和“最早市场触发导致较晚市场漏采”的调度缺口，但不虚构尚未成熟的五日收益样本。
...
- 纸面交易绩效摘要已有基础能力，已包含权益曲线、日度收益、月度收益、FIFO 卖出胜率、最大回撤、换手率、策略/行业贡献评估、窗口级绩效矩阵、Agent run 创建时间窗口筛选、已记录候选的严格后向多周期评价、单日时间点策略重放和现金/整手/等权或显式股票目标权重组合回测；组合回测可从持久化因子采集结果自动读取或显式覆盖现金分红、拆并股，按显式结算价兑付零碎股，选择版本化参考成本档位或自定义实际账户成本，并在固定卖出税或 2005 年以来 A 股分方向历史印花税之间选择，审计事件及逐笔/逐期/汇总成本。历史全 A 股逐日期成员解析和分批可恢复因子采集的在线长任务已完成，SSE/SZSE/BSE 公司行动也已有免费源成功证据。
- 全市场验收门禁、重启孤儿租约自动接管和无 Tushare 权限的完整历史成员 fallback 已完成。作业冻结真实 `universe_source`，日线按新浪 -> EastMoney -> 腾讯 -> BaoStock 降级并逐行审计 provider；验收不再把写入行数当作完整覆盖。5,529 项干净长任务完成 111 批，完整因子覆盖率 99.367%、精确日线覆盖率 99.8734%、估值完整率 99.5117%、源错误率 0.4883%，最终 `ok=true` 且 `recovery_state=complete`；公司行动按 Tushare -> EastMoney -> 巨潮 -> 北交所官方公告降级，真实 `600519` 采集持久化 3 条且零错误，真实 `920976` 采集持久化 4 条且零错误。
...
状态：功能项已完成；后续长跑稳定性证据归入 Phase 2 和 Phase 4，不再沿用早期的 35% 未完成估算。
...
未完成程度：当前约 5% 未完成，集中在真实 gateway 长跑、外部数据长期样本和恢复验收。
...
- 基础策略绩效摘要已有页面展示，并返回本地 paper 权益路径曲线、日度收益、月度收益、最大回撤、换手率、当前仓位、分策略/分行业归因、窗口级绩效矩阵和 Agent run 创建时间窗口筛选；历史组合回测已补齐年化收益、基准、周期胜率、周期波动率、下行偏差、Sharpe、Sortino 与 Calmar，剩余长期判断依赖生产样本自然成熟。
- 一键暂停确认、一键恢复、单次导出、最近运行筛选批量导出、后台任务日志、账户重置、旧归档账户历史筛选、恢复/切换和隐藏式清理已有基础能力。

未完成程度：当前约 1% 未完成，功能面已基本闭环；A 股、美股和港股生产样本均可无人值守积累，剩余为样本成熟所需时间和真实 gateway 长跑证据。
...
- Web 模拟交易页已新增当前 runtime 的零连接“生产预检”，通过只读 API 重新读取外部 JSON 并展示 gateway 是否注册/外部、配置键覆盖和失败 reason code；响应固定声明未连接、未订阅、未下单且不暴露路径或参数值。该能力完成了配置到启动门禁之间的操作闭环，但不替代真实账户连接和长跑证据。
- 修复 `agent_calibration_evidence` 后台任务误从交易服务读取不存在的 `db` 属性；任务现在复用 Agent repository 的数据库管理器，并由拒绝虚构服务属性的回归测试覆盖，生产校准证据可按日无人值守刷新。
