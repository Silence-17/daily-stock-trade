...
Windows Desktop 的 opt-in vn.py 打包已完成实物验收：Python 3.13.14 冻结产物成功导入 vn.py/TA-Lib/内置 gateway，并在无控制台启动下返回健康状态、确认 `DSA_SIM` 连接和四类 EventEngine 回调；Electron 产出 220.6 MiB NSIS 安装器，安装包内后端哈希与已验收产物一致。macOS 脚本已同步相同的依赖、收集与冻结探针契约，但因当前为 Windows 环境未执行 macOS 实物构建。
