...
- Web 页面已展示本地账本、选股来源、自动化调度、调度窗口、交易窗口、持仓估值覆盖/新鲜率/来源分布、AlphaSift 依赖、熔断、vn.py bridge 可用性摘要、持仓估值降级提示、7/30/90 天后台任务长期指标和跨 run 数据质量趋势，并对状态路由 404、状态超时和本地连接失败提供明确提示；继续补后端版本、估值来源跨时间趋势和恢复策略。
