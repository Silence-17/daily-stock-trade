...
| 选股 Agent | 已有自动选股运行记录、结构化 Agent 计划、跨市场动态目标、规则派生计划档位/执行路由/预算上限/降级动作、默认关闭的 LLM 动态计划、每轮运行总结、基础复核质量摘要、基础 Agent 工作流状态机、严格样本外跨运行质量状态与可选买入门禁、按日结构化运行总结、候选级仓位计划/规则风控复核、规则 Agent 买入前二次复核、默认关闭的 LLM 买入前复核、手动可选 LLM 复盘、run 级人工验收及后续上下文复用、候选决策审计和交易计划审计，但还不是完整 Agent 工作流 | 约 94% | 缺少多模型/长周期复核质量评估和更丰富的每日收益目标函数 |
...
- Agent 计划阶段已有基础诊断、跨市场动态目标、规则派生计划档位/降级动作、最近运行与人工验收上下文、默认关闭的 LLM 动态计划、基础复核质量摘要、基础可解释状态机和跨运行前瞻质量迁移；仍缺更丰富的每日收益/风险目标函数和多模型/长周期复核质量评估。
