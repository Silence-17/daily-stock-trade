...
截至 2026-07-15，本系统已经具备 AlphaSift 在线选股、本地模拟交易账本、Web 入口、定时自动买入、可交易窗口诊断、跨模块系统健康视图、关键异常 alert 路由通知、vn.py 提交态审计、内置即时撮合 gateway、成交回报同步入口、规则 Agent 买入前二次复核、默认关闭的 LLM 动态计划、默认关闭的 LLM 买入前复核、基础复核质量摘要、手动可选 LLM 复盘和 Agent run 人工验收反馈，但还没有达到完整 Agent 化、稳定自动化和真实 gateway 长跑验收目标。
...
| 模块 | 当前状态 | 完成度 | 未完成重点 |
| --- | --- | ---: | --- |
| 在线选股能力 | AlphaSift 可用，策略接口返回 8 个策略，候选级数据质量/缺失字段/来源已有基础标注，Web 可查看 source health，自动 Agent 已按历史/当前健康动态调整 snapshot 来源顺序，实时行情与新闻具备 provider 级故障切换和恢复，实时行情健康可跨 API 重启延续 | 约 90% | 长任务稳定性与资金流独立来源仍需增强 |
...
| Web 操作页 | 已有选股页、模拟交易页和独立自动选股 Agent 控制台，支持暂停确认、一键恢复、立即 dry-run 演练、后台调度状态、持久化可筛选后台任务日志、任务趋势摘要、7/30/90 天后台任务长期指标、任务健康检查、模拟账户历史筛选/恢复/切换/隐藏式批量清理、权益曲线、日度收益表、月度收益表、按 Agent run 创建时间筛选的窗口级绩效矩阵、已记录候选的 1/5/10/20 日样本外前瞻评价、时间点因子采集/覆盖检查/单日重放、逐日期历史全市场分批恢复、现金/整手/等权或显式股票目标权重组合回测、自动或显式分红/拆并股、显式零碎股现金补偿、固定或 A 股历史印花税、最低佣金与成本审计、可交易窗口、风控统计、复核质量摘要、运行时间线、分页历史、策略/市场/状态/时间范围筛选、`/agent-console/<runUid>` 独立详情路由、查询参数兼容深链和单次/最近 run JSON 导出 | 约 99% | 仍缺外部权限下的全市场与公司行动长任务证据、券商佣金套餐及 2008-09-19 前税制 |
...
| 稳定性与可观测 | 有状态接口、错误提示、跨模块 `system_health` 健康视图、AlphaSift source health 页面视图、候选级数据质量标注、自动交易数据质量诊断、可交易窗口诊断、读取持久化最近事件的后台任务健康检查、持久化可筛选后台任务日志、任务趋势摘要、7/30/90 天终态运行长期指标、7/30/90 天跨 run 数据质量趋势、7/30/90 天收益/风险校准趋势、snapshot 动态来源权重与审计、实时行情 provider 熔断/冷却/半开恢复、跨 API 重启健康延续与 Agent 审计、任务事件保留/低频清理策略、Agent run 时间线、基础失败重试、交易计划恢复矩阵、手动恢复扫描、MainEngine 主动订单对账、暂停买入时独立恢复、多笔成交幂等累计、撤单竞态/迟到成交恢复、对账异常保护、熔断状态/手动恢复/持久化冷却自动恢复、自动交易系统事件告警历史和 alert 路由通知尝试审计 | 约 99% | 缺少真实 gateway 长跑恢复验收和资金流独立来源 |

综合判断：

- 本地 paper 的在线选股、自动计划、风控、审计和恢复链路已达到可用 MVP；生产验收仍取决于真实数据与运行窗口。
- 长期稳定版仍缺真实 gateway 长跑、外部权限下的全市场与公司行动长任务证据、生产样本下多模型/多市场长期校准，以及历史回测侧券商佣金套餐与 2008-09-19 前税制；Agent run 人工验收、历史意见上下文复用、版本化收益/风险目标和风险门禁隔离已完成。
...
- 状态接口已有轻量查询、完整快照短 TTL 缓存、页面级估值降级提示，以及价格覆盖率、新鲜率、来源/provider 分布、缺失/陈旧/状态未知代码和价格日期范围；仍需补价格源健康的跨时间趋势和外部环境长跑证据。
- 页面级“为什么不可用”已有后端 readiness 诊断摘要、跨模块 `system_health` 健康视图、API/contract/build/Python/进程启动信息、旧后端 contract 兼容提示、AlphaSift 依赖状态、状态加载失败分类和持仓估值降级提示；后台任务已有 7/30/90 天长期指标，Agent run 已有 7/30/90 天整体数据质量趋势，仍需补细粒度来源权重趋势和更完整的自动恢复策略。

需要做：

- 拆分 `GET /vnpy-paper/status` 为轻量状态和完整快照两个层级。
- 持仓估值短 TTL 缓存、价格源失败页面级降级说明和结构化价格源健康指标已完成；继续补价格源健康的跨时间趋势。
- Web 页面已展示后端 API/contract/build 兼容状态、本地账本、选股来源、自动化调度、调度窗口、交易窗口、持仓估值覆盖/新鲜率/来源分布、AlphaSift 依赖、熔断、vn.py bridge 可用性摘要、持仓估值降级提示、7/30/90 天后台任务长期指标和跨 run 数据质量趋势，并对旧后端、状态路由 404、状态超时和本地连接失败提供明确提示；继续补估值来源跨时间趋势和恢复策略。
...
- 强化市场级风控：大盘红绿灯、盘后市场宽度和热点退潮阻断已有基础实现；继续补盘中指数趋势、实时宽度与多市场联动。
- 所有风控结果写入候选决策和交易计划。

验收标准：

- 最大持仓数、单票金额、总持仓金额、总仓位比例、行业金额、行业仓位比例、每日次数、每日预算、股票黑名单、ST/退市、停牌、涨跌停、流动性、账户级现金/回撤、市场红绿灯、盘后市场宽度/热点退潮和连续失败熔断拒单已有明确 reason code；盘中市场联动仍需补齐。
...
- 当前 AlphaSift snapshot/daily 源已有 Web 可见失败计数、错误摘要和具体来源跨 run 降级率；候选补充阶段的行情、资金流和新闻也已作为 `candidate_context.quote/fund_flow/news` 接入同一健康快照、质量评分和跨 run 趋势。自动 Agent 已按最近 30 天同策略/市场历史与当前连续失败生成有边界的 snapshot 来源权重和动态顺序，并保留显式优先级；新闻已有 provider 故障转移，实时行情已有异常熔断、冷却、单半开恢复和 24 小时内跨 API 重启健康延续，资金流仍缺独立多源恢复。
...
- 数据质量 `stale/unavailable` 和 AlphaSift 筛选异常已写入告警中心系统事件历史并复用 alert 路由外发通知，Web 模拟交易页已展示最近自动交易告警历史；实时行情与新闻具备自动切换/恢复，实时行情状态可跨 API 重启延续，仍缺资金流独立多源恢复。
...
- 页面已展示 AlphaSift 源健康、本轮降级原因、模拟交易跨模块健康摘要、跨 run 整体质量、具体来源降级趋势、snapshot 动态路由，以及候选实时行情 provider 的熔断、冷却和跨 API 重启恢复状态；继续补资金流独立多源。
...
- 绩效摘要已有 MVP，并已包含本地 paper 权益路径页面曲线、日度收益、月度收益、最大回撤、换手率、按策略/行业归因、窗口级绩效矩阵、Agent run 创建时间窗口筛选、已记录候选的 1/5/10/20 日样本外前瞻评价、带字段覆盖门禁的单日时间点策略重放，以及按现金、A 股整手和等权或显式股票目标权重执行的跨日组合回测；Tushare 生命周期元数据与已实施公司行动已接入持久化、分批、租约保护的可恢复采集，拆并股支持按显式结算价兑付零碎股，A 股印花税可按 2008/2023 官方生效日自动切换，仍缺外部权限下的成功在线长任务证据、券商佣金套餐和更早税制。
...
- LLM 重排超时已有降级、跨 run 熔断、冷却恢复和单半开探测回归；仍需继续扩展数据源失败、行情缺价、重复订单和 scheduler 长运行组合矩阵，非交易时段已有基础 gate 测试。
- 纸面交易绩效摘要已有基础能力，已包含权益曲线、日度收益、月度收益、FIFO 卖出胜率、最大回撤、换手率、策略/行业贡献评估、窗口级绩效矩阵、Agent run 创建时间窗口筛选、已记录候选的严格后向多周期评价、单日时间点策略重放和现金/整手/等权或显式股票目标权重组合回测；组合回测可从 Tushare 因子采集结果自动读取或显式覆盖现金分红、拆并股，按显式结算价兑付零碎股，配置最低佣金，并在固定卖出税或 A 股历史印花税之间选择，审计事件及逐笔/逐期/汇总成本。历史全 A 股逐日期成员解析和分批可恢复因子/公司行动采集已完成，仍缺具备外部权限后的成功在线长任务证据、券商佣金套餐与 2008-09-19 前税制。
...
- 增加 Web 测试，覆盖任务历史和详情。
- 基础模拟交易绩效摘要已覆盖收益、收益率、权益曲线、日度收益、月度收益、成交额、买卖次数、FIFO 卖出胜率、最大回撤、换手率、当前仓位、成交/计划、跳过原因、策略/行业归因、窗口级绩效矩阵、Agent run 创建时间窗口筛选、已记录候选的多周期前瞻评价、时间点策略重放，以及带成本/基准、显式股票目标权重、重叠持仓保留、卖出受阻跨期延续、公司行动、A 股历史印花税和基础停牌/涨跌停约束的跨日组合回测；继续补历史全市场在线证据、券商佣金套餐与更早税制。
