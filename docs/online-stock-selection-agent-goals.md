...
- 端到端主路径已有基础覆盖；scheduler 已覆盖配置重载跨代重叠、审计跳过和异常释放，仍缺多候选、拒单、数据源失败和真实长时间运行场景矩阵。
- 自动交易 dry-run 已有基础回归测试。
- LLM 重排超时已有降级、跨 run 熔断、冷却恢复和单半开探测回归；scheduler reconcile 的同名任务防重已有确定性并发回归，仍需继续扩展数据源失败、行情缺价、重复订单和真实长时间运行组合矩阵，非交易时段已有基础 gate 测试。
