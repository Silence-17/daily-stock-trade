...
| 模块 | 当前状态 | 完成度 | 未完成重点 |
| --- | --- | ---: | --- |
| 在线选股能力 | AlphaSift 可用，策略接口返回 10 个策略（8 个内置 A 股策略、`us_large_cap_momentum` 和 `hk_liquid_momentum`），候选级数据质量/缺失字段/来源已有基础标注；美股在 yfinance 限流时降级到腾讯行情，港股使用有界股票池和腾讯直连快照；5,529 只全市场长任务已通过 | 约 99% | 仍需生产样本长期校准证据；AlphaSift 0.2.0 不提供港股 LLM 重排 |
...
- 生产校准现在有独立只读验收门禁 `scripts/check_agent_calibration_evidence.py`，默认按 A 股/港股/美股分别门禁 90 天运行量、目标快照观测率、最新成熟样本、观测日期、时效、版本和摘要完整性。A 股当前为 19 个 completed run、18 个目标快照（94.74%）、5 个观测日、0 个成熟样本；2026-07-21 门禁记录港股 1/1、美股 4/4 个已观测 run，观测率均为 100%，但成熟样本均为 0 且只有 1 个观测日，因此长期证据明确未通过。
...
未完成程度：当前约 1% 未完成，功能面已基本闭环；A 股、美股和港股生产样本均可无人值守积累，剩余为样本成熟所需时间、港股 LLM 重排和真实 gateway 长跑证据。
