...
- `POST /api/v1/alphasift/replay/portfolio-backtest`：在多个已保存快照日期上逐次重放策略，默认取 Top 5，并默认使用 `cash_ledger`。未传 `target_weights` 时按每期等权目标金额计算数量；传入 `{ "600519": 60, "000001": 30 }` 时，权重表示组合权益百分比，仅当期策略 Top K 中同样出现的配置标的参与调仓，未使用比例保留为现金，已配置但未入选的代码会逐期返回审计。单票权重必须大于 0 且不超过 100，总和不得超过 100；该字段不支持旧 `equal_weight_approximation` 模式。A 股数量向下取整为 100 股整手，先卖出超配/剔除仓位再用实际可用现金补买低配/新增仓位。信号日之后首根日线开盘调仓，最后一期在第 20 根后向交易 bar 收盘尝试退出；默认买卖每边手续费 3 bps、滑点 5 bps，`minimum_commission` 可配置每笔最低佣金，`sell_tax_bps` 只对卖出执行金额征收。目标数量和现金可买数量均包含绝对佣金，逐笔返回执行价、佣金、税费、滑点和净现金影响，逐期/汇总返回独立成本合计。系统还启用 A 股停牌、涨停不可买和跌停不可卖门禁。最终卖出受阻或缺少退出行情时保留仓位市值和期末未平仓数，不伪造成交。旧 `equal_weight_approximation` 可显式选择用于兼容，但不接受最低佣金、卖出税或显式目标权重。该接口只计算，不下单。

当前重放边界：系统可以用 Tushare 上市/退市生命周期为每个快照日期独立还原全 A 股成员，并通过持久化作业分批、可恢复地采集行情/估值因子，再以现金、整手股数和等权或显式股票目标权重执行跨日组合回测。最低佣金和卖出税可按回测显式配置，但系统不会猜测券商套餐，也不会按历史日期自动切换费税制度。全市场成功在线运行仍依赖具备 `stock_basic` 权限的 Tushare token 和长期可用的 AKShare 上游；当前尚未处理分红送转/复权，历史回测也不自动求解多约束目标权重。
