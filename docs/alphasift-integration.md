...
当前重放边界：系统可以用 Tushare 上市/退市生命周期为每个快照日期独立还原全 A 股成员，并通过持久化作业分批、可恢复地采集行情/估值因子和已实施公司行动，再以现金、整手股数和等权或显式股票目标权重执行跨日组合回测。回测默认读取持久化事件，`include_persisted_corporate_actions=false` 可关闭；请求内显式事件会覆盖同股票、同日、同类型的持久化事件，并可为零碎股提供实际现金结算价。系统不会猜测券商套餐、按历史日期自动切换费税制度或用行情价格冒充零碎股结算价。全市场及公司行动成功在线运行仍依赖具备相应权限的 Tushare token 和长期可用的上游；历史回测也不自动求解多约束目标权重。
