...
- LLM 环境：DSA 调用 AlphaSift 时会桥接 DSA 已解析的 `LITELLM_MODEL`、`LITELLM_FALLBACK_MODELS`、`LLM_CHANNELS`、`LLM_<NAME>_*`、`LITELLM_CONFIG`、渠道额外请求头和各模型密钥；AlphaSift 独立运行时仍使用自己的 `.env`/环境变量。DSA 会在调用期为 AlphaSift 选股 LLM 重排默认注入 `LLM_TIMEOUT_SEC=180` 与 `LLM_MAX_TOKENS=3072`，分别限制单次请求耗时和输出 token 上限；3072 可容纳当前五候选结构化合同，避免 1024 tokens 在首个候选对象闭合前截断。用户显式配置同名环境变量时仍保留用户值。
- 候选增强性能：DSA 在每次 screen 的宿主回调中强制执行 `dsa.max_candidates=3`，即使当前 AlphaSift 内部仍按默认 5 次调用，也只有前 3 个候选会实际拉取上下文；预排序上下文取得的实时行情会直接传给基础面聚合的估值块，不再为同一候选重复拉取行情。后排序完整上下文对最多 3 个最终候选使用固定上限的并发增强，并在汇总时恢复原排名和 warning 顺序。单票失败仍只降级该候选，不改变 AlphaSift 主结果的 fail-open 语义。
- 快照源：DSA 调用 AlphaSift 时，未显式配置 `SNAPSHOT_SOURCE_PRIORITY` 会从 token-aware 基础顺序开始：有 `TUSHARE_TOKEN` 时为 `tushare,sina,efinance,akshare_em,em_datacenter`，无 token 时为 `sina,efinance,akshare_em,em_datacenter`。自动 Agent 会结合最近 30 天同策略/市场的持久化降级观测和当前进程连续失败状态计算健康权重，只调整顺序、不删除来源；screen 与 Agent run 通过 `source_routing` 审计基础顺序、有效顺序和每源权重。用户显式配置的顺序始终原样保留。同时注入 `DAILY_SOURCE=auto`、`DAILY_FETCH_RETRIES=3`、`DAILY_FETCH_MAX_WORKERS=1` 和默认候选上下文 `news,fund_flow,announcement,quote`。候选上下文会额外持久化 `quote/<provider>`、`fund_flow/<provider>`、`news/<provider>` 趋势证据；这些明细不重复计入本轮质量分。候选资金流以 `tushare_ths -> akshare` 为基础顺序，并结合最近 30 天同策略/市场 provider 退化率和当前熔断状态生成请求级有效顺序；该顺序只作用于本次基本面请求并进入缓存键，不修改全局配置。候选新闻以当前已配置且可用的搜索 provider 顺序为基础，按同一 30 天趋势进行请求级重排；显式路由进入新闻缓存键，异常 provider 会继续 fallback，尝试链作为 `news/<provider>` 证据持久化，且不修改全局 provider 列表；缓存命中仍可用于候选质量判断，但不会把旧尝试重复累计为新的 provider 观测。新闻 API/传输失败连续 3 次会进入 5 分钟冷却，冷却后仅放行一个半开探测，状态与行情/资金流共用 `provider_source_health.json` 并可跨 API 重启恢复；无相关新闻只记为不确定，不推进熔断。审计中的 `effective_priority` 保留本轮实际使用顺序，`post_run_sources` 和 `next_recommended_priority` 分别记录增强后的来源状态与下轮建议，避免用事后状态改写本轮执行事实。
...
候选项建议包含 `code`、`name`、`score`、`reason`、`risk_level`、`risk_flags`、`price`、`change_pct`、`amount`、`industry`、`factor_scores`，以及 LLM 字段：`llm_score`、`llm_confidence`、`llm_thesis`、`llm_catalysts`、`llm_risks`、`llm_watch_items` 等。DSA API 返回阶段会额外补充候选级 `data_quality`、`missing_fields`、`data_sources` 和 `quality_notes`，用于在 Web 选股页和自动交易审计中解释候选字段完整性、数据来源和降级原因；screen last-good cache 回退时，候选还会带 `cache_used=true`、`stale=true`、`cached_at` 和 `stale_age_hours`。
...
AlphaSift 侧已在 `ZhuLinsen/alphasift@0a7b9cd59e81718f851890535241bc105d4ddc64` 提供 DSA provider context 支持、DSA adapter contract，并支持复用 DSA 调用期注入或用户显式配置的 `LLM_TIMEOUT_SEC`；同一 pin 还会读取 `LLM_MAX_TOKENS` 限制 LLM 重排输出，且 timeout 后不再盲目重试无 JSON mode 请求。
...
- `/api/v1/alphasift/status`：返回 AlphaSift 启用/可用状态、非敏感 diagnostics 和 `source_health`。Web 选股页会展示 snapshot / daily 源状态、失败次数、冷却时间和最近错误摘要，用于定位当前选股降级是否来自单一数据源。
...
- `POST /api/v1/alphasift/replay/ingestion/tasks`：为调用方显式提供的历史股票池提交有界后台采集任务，单次最多 50 个日期、50 只股票；使用 `stock_zh_a_hist` 获取历史日线/换手率，使用 `stock_zh_valuation_baidu` 获取历史 PE(TTM)、PB 和总市值，并只取快照日当日或之前的估值。配置 Tushare 后，同一任务还会通过 `dividend` 获取已实施的现金分红和送转事件，按股票、除权除息日和事件类型幂等保存；该外部源失败只记录 `corporate_actions` 阶段错误，不丢弃已成功采集的因子。`GET /api/v1/alphasift/replay/ingestion/tasks/{task_id}` 查询进度和写入/错误摘要。
...
- `POST /api/v1/alphasift/replay/portfolio-backtest`：在多个已保存快照日期上逐次重放策略，默认取 Top 5，并默认使用 `cash_ledger`。未传 `target_weights` 时按每期等权目标金额计算数量；传入 `{ "600519": 60, "000001": 30 }` 时，权重表示组合权益百分比，仅当期策略 Top K 中同样出现的配置标的参与调仓，未使用比例保留为现金，已配置但未入选的代码会逐期返回审计。单票权重必须大于 0 且不超过 100，总和不得超过 100。`corporate_actions` 可显式传入 `cash_dividend`（`cash_dividend_per_share`）或 `split_adjustment`（`split_ratio`）事件；账本以未复权行情为前提，按每个标的的交易/估值日期推进事件，同日先处理公司行动再交易。现金分红只发放给事件时已有仓位，拆并股调整数量；若产生非整数股，事件必须提供实际结算口径的 `cash_in_lieu_price`，账本向下保留整股并兑付零碎股现金，未提供时 fail-closed。A 股数量向下取整为 100 股整手，先卖出超配/剔除仓位再用实际可用现金补买低配/新增仓位。信号日之后首根日线开盘调仓，最后一期在第 20 根后向交易 bar 收盘尝试退出。`cost_profile=custom` 保留显式佣金、最低佣金、税制与滑点；`cn_retail_reference` 原子应用佣金 3 bps、最低 5 元、A 股历史印花税和滑点 5 bps，`cn_low_commission_reference` 将佣金改为 1 bps，其余相同，`zero_cost_baseline` 用于零费用对照。参考档位是版本化回测假设，不代表任何券商报价；实际账户应使用 `custom`。`sell_tax_mode=explicit` 时，`sell_tax_bps` 只对卖出执行金额征收；历史模式则按实际成交日和买卖方向计税。目标数量和现金可买数量均包含绝对佣金与买方税，逐笔返回执行价、佣金、税费、税制来源、滑点和净现金影响，逐期/汇总返回独立成本合计；方法审计同时返回请求成本、实际生效成本、档位名和 `dsa-cost-profile-v1` 版本。系统还启用 A 股停牌、涨停不可买和跌停不可卖门禁。最终卖出受阻或缺少退出行情时保留仓位市值和期末未平仓数，不伪造成交。旧 `equal_weight_approximation` 可显式选择用于兼容，但不接受公司行动、最低佣金、卖出税或显式目标权重。该接口只计算，不下单。

当前重放边界：系统可以用 Tushare 上市/退市生命周期为每个快照日期独立还原全 A 股成员，并通过持久化作业分批、可恢复地采集行情/估值因子和已实施公司行动，再以现金、整手股数和等权或显式股票目标权重执行跨日组合回测。回测默认读取持久化事件，`include_persisted_corporate_actions=false` 可关闭；请求内显式事件会覆盖同股票、同日、同类型的持久化事件，并可为零碎股提供实际现金结算价。`sell_tax_mode=cn_historical_stamp_duty` 按实际成交日自动应用 2005-01-24 双边 10 bps、2007-05-30 双边 30 bps、2008-04-24 双边 10 bps、2008-09-19 起仅卖方 10 bps、2023-08-28 起仅卖方 5 bps，并逐笔披露政策来源；买方税会进入目标整手和现金可买数量。`explicit` 保留固定卖方 `sell_tax_bps`。系统不会猜测券商佣金套餐、2005-01-24 之前的税制或用行情价格冒充零碎股结算价。全市场及公司行动成功在线运行仍依赖具备相应权限的 Tushare token 和长期可用的上游；历史回测也不自动求解多约束目标权重。

历史印花税制度依据：[财政部、税务总局 2005 年调整通知](https://www.mof.gov.cn/gkml/caizhengwengao/caizhengbuwengao2005/caizhengbuwengao20053/200805/t20080525_42735.htm)；[财税〔2007〕84 号](https://www.mofcom.gov.cn/zcfb/zgdwjjmywg/art/2007/art_99829a5529ff4223a7bf6ca48ffc4b24.html)；[财政部 2008-04-24 税率调整](https://www.mof.gov.cn/zhengwuxinxi/caizhengxinwen/200805/t20080519_29133.htm)；[财政部 2008-09-19 单边征收通知](https://www.mof.gov.cn/zhengwuxinxi/caizhengxinwen/200809/t20080919_76432.htm)；[财政部、税务总局公告 2023 年第 39 号](https://fgk.chinatax.gov.cn/zcfgk/c102416/c5211343/content.html)。
