...
当前重放边界：系统可以用 Tushare 上市/退市生命周期为每个快照日期独立还原全 A 股成员，并通过持久化作业分批、可恢复地采集行情/估值因子，再以现金、整手股数和等权或显式股票目标权重执行跨日组合回测。最低佣金、卖出税、现金分红和拆并股可按回测显式配置，但系统不会猜测券商套餐、按历史日期自动切换费税制度、自动抓取公司行动或推断零碎股现金补偿。全市场成功在线运行仍依赖具备 `stock_basic` 权限的 Tushare token 和长期可用的 AKShare 上游；历史回测也不自动求解多约束目标权重。
