...
- `POST /api/v1/alphasift/replay/portfolio-backtest`：在多个已保存快照日期上逐次重放策略，默认取 Top 5，并默认使用 `cash_ledger`。未传 `target_weights` 时按每期等权目标金额计算数量；传入 `{ "600519": 60, "000001": 30 }` 时，权重表示组合权益百分比，仅当期策略 Top K 中同样出现的配置标的参与调仓，未使用比例保留为现金，已配置但未入选的代码会逐期返回审计。单票权重必须大于 0 且不超过 100，总和不得超过 100。`corporate_actions` 可显式传入 `cash_dividend`（`cash_dividend_per_share`）或 `split_adjustment`（`split_ratio`）事件；账本以未复权行情为前提，按每个标的的交易/估值日期推进事件，同日先处理公司行动再交易。现金分红只发放给事件时已有仓位，拆并股调整数量；若产生非整数股，事件必须提供实际结算口径的 `cash_in_lieu_price`，账本向下保留整股并兑付零碎股现金，未提供时 fail-closed。A 股数量向下取整为 100 股整手，先卖出超配/剔除仓位再用实际可用现金补买低配/新增仓位。信号日之后首根日线开盘调仓，最后一期在第 20 根后向交易 bar 收盘尝试退出；默认买卖每边手续费 3 bps、滑点 5 bps，`minimum_commission` 可配置每笔最低佣金，`sell_tax_bps` 只对卖出执行金额征收。目标数量和现金可买数量均包含绝对佣金，逐笔返回执行价、佣金、税费、滑点和净现金影响，逐期/汇总返回独立成本合计。系统还启用 A 股停牌、涨停不可买和跌停不可卖门禁。最终卖出受阻或缺少退出行情时保留仓位市值和期末未平仓数，不伪造成交。旧 `equal_weight_approximation` 可显式选择用于兼容，但不接受公司行动、最低佣金、卖出税或显式目标权重。该接口只计算，不下单。

当前重放边界：系统可以用 Tushare 上市/退市生命周期为每个快照日期独立还原全 A 股成员，并通过持久化作业分批、可恢复地采集行情/估值因子和已实施公司行动，再以现金、整手股数和等权或显式股票目标权重执行跨日组合回测。回测默认读取持久化事件，`include_persisted_corporate_actions=false` 可关闭；请求内显式事件会覆盖同股票、同日、同类型的持久化事件，并可为零碎股提供实际现金结算价。`sell_tax_mode=cn_historical_stamp_duty` 按实际卖出日自动应用 2008-09-19 起 10 bps、2023-08-28 起 5 bps 的 A 股出让方印花税，并逐笔披露政策来源；`explicit` 保留固定 `sell_tax_bps`。系统不会猜测券商佣金套餐、2008-09-19 之前的税制或用行情价格冒充零碎股结算价。全市场及公司行动成功在线运行仍依赖具备相应权限的 Tushare token 和长期可用的上游；历史回测也不自动求解多约束目标权重。

历史印花税制度依据：[财政部 2008-09-19 单边征收通知](https://www.mof.gov.cn/zhengwuxinxi/caizhengxinwen/200809/t20080919_76432.htm)；[财政部、税务总局公告 2023 年第 39 号](https://fgk.chinatax.gov.cn/zcfgk/c102416/c5211343/content.html)。
