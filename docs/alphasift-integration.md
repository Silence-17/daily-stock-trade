...
- 快照源：DSA 调用 AlphaSift 时，未显式配置 `SNAPSHOT_SOURCE_PRIORITY` 会从 token-aware 基础顺序开始：有 `TUSHARE_TOKEN` 时为 `tushare,sina,efinance,akshare_em,em_datacenter`，无 token 时为 `sina,efinance,akshare_em,em_datacenter`。自动 Agent 会结合最近 30 天同策略/市场的持久化降级观测和当前进程连续失败状态计算健康权重，只调整顺序、不删除来源；screen 与 Agent run 通过 `source_routing` 审计基础顺序、有效顺序和每源权重。用户显式配置的顺序始终原样保留。同时注入 `DAILY_SOURCE=auto`、`DAILY_FETCH_RETRIES=3`、`DAILY_FETCH_MAX_WORKERS=1` 和默认候选上下文 `news,fund_flow,announcement,quote`。候选资金流在 Tushare 可用时使用独立 `tushare_ths -> akshare` 路由；provider 熔断、冷却和跨重启状态写入 `source_routing.candidate_context.fund_flow`，不改变 AlphaSift snapshot 来源优先级。
