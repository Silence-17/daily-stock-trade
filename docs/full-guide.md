...
> 对于调用方已经掌握的历史股票池，可通过 `POST /api/v1/alphasift/replay/ingestion/tasks` 后台采集历史日线、换手率、PE/PB、市值和技术因子，再调用 `POST /api/v1/alphasift/replay/portfolio-backtest` 执行跨日期组合回测。API/Web 默认使用 `cash_ledger`：采用下一交易日开盘成交、买卖每边手续费 3 bps、滑点 5 bps，按账户现金和每期等权目标计算 A 股 100 股整手数量；也可传 `target_weights`（Web 示例：`600519=60, 000001=30`）指定股票目标权益百分比，总和最多 100%，剩余比例保留为现金。显式权重只作用于当期策略 Top K 同时选中的股票，未入选配置会进入逐期审计。`minimum_commission` 表示每笔最低佣金，`sell_tax_bps` 表示只对卖出执行金额扣除的税率；两者默认 0，需按券商和回测期间制度显式填写。`corporate_actions` 可传现金分红和拆并股事件，使用未复权价格账本、同日公司行动先于交易；事件时未持仓不会追溯权益，零碎股因未配置现金补偿而 fail-closed。目标整手和现金上限均包含佣金，结果会分别披露执行价、佣金、税、滑点、公司行动和净现金影响。账本先卖超配再买低配；最终卖出受阻会保留未平仓市值。Web 面板默认用 `000300` 做基准。系统不使用当前全市场列表推导过去股票池，以避免隐蔽的生存者偏差。
