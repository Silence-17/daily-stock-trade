...
可选 vn.py runtime 建议使用 Python 3.13 隔离环境。Windows PowerShell 运行 `.\scripts\setup_vnpy_runtime.ps1 -PythonExecutable "python"`；脚本会拒绝上游未声明支持的 Python 3.14，安装 `requirements-vnpy.txt`，并用真实 OrderRequest、EventEngine、MainEngine 和内置即时撮合 gateway smoke 验收。安装验收还会运行显式拒单、重复成交回报去重和三轮在途订单断线重连矩阵，确认 `DsaSimulatedGateway` 保留资金、持仓和订单缓存并且每单只入账一次。该网关无需账户参数即可在 Web 走完整 vn.py 模拟订单/成交回写；真实通道仍须另装具体 gateway 并使用外部连接参数。配置真实通道后，使用 `scripts/check_vnpy_gateway_soak.py` 做零下单长跑，可按最低连接率和必需事件返回机器可判定的验收结果，详见 [vn.py 模拟交易](vnpy-paper-trading.md)。
...
> 对于调用方已经掌握的历史股票池，可通过 `POST /api/v1/alphasift/replay/ingestion/tasks` 后台采集历史日线、换手率、PE/PB、市值和技术因子；配置 Tushare 后还会采集并幂等保存已实施的现金分红与送转事件，事件源失败会降级记录且不丢弃已成功采集的因子。随后调用 `POST /api/v1/alphasift/replay/portfolio-backtest` 执行跨日期组合回测。API/Web 默认使用 `cash_ledger`：采用下一交易日开盘成交，按账户现金和每期等权目标计算 A 股 100 股整手数量；也可传 `target_weights`（Web 示例：`600519=60, 000001=30`）指定股票目标权益百分比，总和最多 100%，剩余比例保留为现金。显式权重只作用于当期策略 Top K 同时选中的股票，未入选配置会进入逐期审计。成本可使用 `custom` 显式填写，或选择版本化的 `cn_retail_reference`（佣金 3 bps、最低 5 元、历史税制、滑点 5 bps）、`cn_low_commission_reference`（佣金 1 bps，其余相同）和 `zero_cost_baseline`。参考档位仅是可重复的回测假设，不代表券商报价；实际账户应选 `custom`。非自定义档位由后端原子覆盖全部成本参数，结果审计请求值、实际生效值和档位版本。`sell_tax_mode=explicit` 使用固定卖方 `sell_tax_bps`，选择 `cn_historical_stamp_duty` 后则按实际成交日和方向应用 2005-01-24 以来的 A 股历史印花税：2005-01-24 双边 10 bps、2007-05-30 双边 30 bps、2008-04-24 双边 10 bps、2008-09-19 起仅卖方 10 bps、2023-08-28 起仅卖方 5 bps。每笔成交披露实际税率和政策来源，买方税进入目标整手和现金可买数量，2005-01-24 之前 fail-closed。回测默认读取持久化公司行动，Web 可关闭；`corporate_actions` 仍可显式传入现金分红和拆并股，并覆盖同股票、同日、同类型的已保存事件。账本使用未复权价格，同日公司行动先于交易；事件时未持仓不会追溯权益。拆并股产生非整数股时可提供 `cash_in_lieu_price`，系统向下保留整股并把零碎部分兑付现金；未提供结算价时 fail-closed，且不会用行情价格猜测。目标整手和现金上限均包含佣金与买方税，结果会分别披露执行价、佣金、税、滑点、公司行动、零碎股补偿和净现金影响。账本先卖超配再买低配；最终卖出受阻会保留未平仓市值。Web 面板默认用 `000300` 做基准，并展示生效成本和历史税制。系统不使用当前全市场列表推导过去股票池，以避免隐蔽的生存者偏差。
> 配置具备 Tushare `stock_basic` 权限的 `TUSHARE_TOKEN` 后，`GET /api/v1/alphasift/replay/universe?snapshot_date=YYYY-MM-DD` 可按上市/退市日期还原当日 A 股成员并包含当时尚未退市的历史股票；数据源不可用时返回 424，不回退当前股票池。返回名称和行业仍是 Tushare 当前维护值。Agent 控制台“全市场采集”会通过 `POST /api/v1/alphasift/replay/full-market-ingestion/jobs` 为每个快照日期独立冻结成员并按批采集，页面刷新后自动恢复最近作业；失败可从最后成功批次继续，服务重启遗留的 processing 作业可显式强制接管，任务租约会阻止旧执行者重复提交检查点。
...
> 自动卖出风控支持 `auto_no_progress_days` 与 `auto_no_progress_min_return_pct`：持仓达到指定天数且浮盈不高于阈值时会以 `no_progress_timeout` 生成卖出计划；收益阈值留空时按 0% 处理。持仓天数按完整成交历史分页读取并 FIFO 重放当前未平仓批次，清仓后重新建仓会重置；卖出检查先于数据质量、账户和市场买入门禁执行，后续买入被拦截时已提交卖单仍计入 Agent run 审计。
