...
XTP 的交易与行情 API 都通过 `login_status` 确认，两个通道均登录才报告 connected。账户所有者从中泰 XTP 官网申请“股票类型”测试账户后，在 `vnpy-xtp-paper-acceptance` Environment 添加 `VNPY_XTP_CONNECT_SETTINGS_JSON`，再从 `main` 运行 `External vn.py XTP A-share Account Soak`。工作流在安装前验证十个官方字段、客户号/端口范围和行情协议/日志枚举，要求双通道连续稳定 60 秒后再执行零下单长跑并保留 14 天脱敏证据。run `30001136306` 已以 180/180 connected、240 个账户事件、4320 个持仓事件和 0 订单/成交事件通过首轮 15 分钟验收。生产实盘凭据仍只能在受控部署主机验收。完整安全边界与设置方法见 [vn.py 模拟交易说明](vnpy-paper-trading.md)。
...
> `POST /api/v1/vnpy-paper/trade-plans/recovery/run` 可在页面确认后手动运行一次受限恢复扫描，复用后台恢复逻辑。活跃计划提交满 60 秒后即可通过注入的 `MainEngine.get_order` / `get_all_trades` 对账并补同步漏失回报；MainEngine 仍观测到的活动委托超过创建后 30 分钟时会自动发起一次撤单，对账刷新不会重置时钟或把 `cancel_requested` 回退为 `submitted`。撤单请求从 `raw.cancel.requested_at` 独立计算 30 分钟终态回报超时；无网关证据的计划则按原始创建时间安全归档。响应除超时归档、重试、提交、跳过和失败计数外，还返回 `cancel_requested_count`、`reconciled_count`、`protected_count` 和 `reconciliation_failed_count`。
...
> 需要无人值守积累生产形态的校准样本时，可显式设置 `DSA_AGENT_CALIBRATION_SHADOW_ENABLED=true` 和 `DSA_AGENT_CALIBRATION_SHADOW_PAIRS=cn:dual_low,us:us_large_cap_momentum,hk:hk_liquid_momentum`，并通过 `DSA_AGENT_CALIBRATION_SHADOW_INTERVAL_MINUTES`、`DSA_AGENT_CALIBRATION_SHADOW_MAX_RESULTS` 控制周期和候选数。任务首次注册延迟 300 秒，只生成独立 `agent_calibration_shadow` dry-run 记录，绝不提交订单或修改保存的自动交易配置；每个 `市场:策略` 必须匹配 `market_scope`。每次到期 shadow 会先按股票去重补齐已经达到最短前瞻周期的缺失日线，尚未到期的候选只记录跳过；前瞻 API 与质量快照分别审计尝试、成功、失败、未到期跳过、实际供应商、写入行数和刷新后已补齐/仍缺失锚点。DSA 在不修改安装包的前提下叠加美股策略，并为港股提供复用 AlphaSift 硬过滤、因子评分和原生 LLM ranker 的兼容管线；港股股票池可用 `ALPHASIFT_HK_TICKERS` 显式限制。港股 LLM 复排遵守自动 Agent 的请求级超时、确定性评分降级和跨轮熔断策略。
...
> 模拟交易的每票金额、每日预算、最低现金和金额型仓位上限均按账户基准币种计价。首次跨币种自动买入会按需刷新所需汇率对，优先 yfinance 并以 Frankfurter 日期化参考汇率降级；也可在持仓页面或 `POST /api/v1/portfolio/fx/refresh` 手动刷新。在线源与有效缓存均不可用时，系统仍以 `fx_rate_unavailable` 阻止新增买入但不阻止卖出；Agent 审计保留基准金额、交易币种金额和汇率来源。
> Agent 控制台会读取 `GET /api/v1/vnpy-paper/agent-runs/daily-summary` 展示“今日 Agent 总结”，按日期聚合 run 数、候选/计划/成交/跳过计数、状态分布、执行模式、数据质量、Agent 复核状态、LLM 复核状态、复核质量状态/风险标记/平均分、工作流状态/阶段、主要跳过原因和热门标的；候选决策详情会展示 `order_result.strategy_evidence` 的策略、排名、筛选/最终分数、上游规则命中、因子分解和证据状态，以及 `order_result.agent_review` 的规则 Agent 买入前复核状态和摘要。没有逐规则或因子证据时状态为 `summary_only`，不会根据总分反推规则命中；旧 run 没有该字段时仍按原有理由和复核字段展示。Agent 计划卡片会展示规则派生的计划档位、风控档位、同触发来源/策略/市场最近 5 次运行上下文和可选 `llm_dynamic_plan` 状态。运行详情会派生 `diagnostics.agent_workflow`，按计划、数据质量、候选复核、交易计划和执行阶段展示当前阶段、整体状态和下一步，并通过 `diagnostics.agent_summary.review_quality` 展示本轮复核质量、覆盖率、风险标记和是否建议人工确认；`diagnostics.stage_timings` 记录计划、前置门禁、AlphaSift 筛选、候选决策/执行和总耗时，时间线以 `performance` 事件展示总耗时与 AlphaSift 耗时。运行详情还可手动触发 `POST /api/v1/vnpy-paper/agent-runs/{run_uid}/llm-recap` 生成可选 LLM 复盘并写入 `diagnostics.llm_recap`；该入口不提交订单，也不会改变交易计划状态。模拟交易设置中的 `auto_llm_plan_enabled` 默认关闭；开启后会在 AlphaSift 选股前调用 LLM 生成本轮动态计划，并将同来源最近 3 份已完成复盘作为有界、不可信的历史审计上下文；系统提示明确禁止把复盘文本当作指令，evaluator 仍只允许已知策略，并确保候选数和单票预算不增加、最低分不降低。动态计划提示词版本为 `vnpy_paper_dynamic_agent_plan_v3`。`auto_llm_review_enabled` 同样默认关闭；开启后会在规则风控通过后调用 LLM 生成 `order_result.llm_review`，`blocked` 或调用失败会跳过候选。`llm_dynamic_plan` 与 `llm_review` 都会记录 `prompt_version` 和 `evaluator_version`，Agent 控制台会在对应位置展示这些审计版本。
