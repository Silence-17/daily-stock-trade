...
>
> 自动买入还支持默认关闭的 `auto_score_weighted_allocation_enabled`。开启后，`auto_allocation_budget`（留空则使用 `auto_cash_per_order`）作为每轮组合预算；`auto_allocation_method` 可选按评分分配，或使用 AlphaSift 同一决策时点的 `volatility_20d_pct` 做评分/逆波动率分配。风险模式通过 `auto_risk_volatility_floor_pct` 限制极低波动率的权重放大，缺失或非法风险字段会逐票 fail-closed。整轮同时受现金保留、每日预算/订单槽位、最大持仓数、单票/总仓位/行业空间和显式目标权重约束；Agent run 与候选仓位计划会记录 `portfolio_allocation`、风险输入、整手后的可执行预算和残差。当前是确定性的对角线风险模型，不是协方差/相关性或任意目标函数优化器。
