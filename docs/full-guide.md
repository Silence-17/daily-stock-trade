...
> 对于调用方已经掌握的历史股票池，可通过 `POST /api/v1/alphasift/replay/ingestion/tasks` 后台采集历史日线、换手率、PE/PB、市值和技术因子；配置 Tushare 后还会采集并幂等保存已实施的现金分红与送转事件，事件源失败会降级记录且不丢弃已成功采集的因子。随后调用 `POST /api/v1/alphasift/replay/portfolio-backtest` 执行跨日期组合回测。API/Web 默认使用 `cash_ledger`：采用下一交易日开盘成交、买卖每边手续费 3 bps、滑点 5 bps，按账户现金和每期等权目标计算 A 股 100 股整手数量；也可传 `target_weights`（Web 示例：`600519=60, 000001=30`）指定股票目标权益百分比，总和最多 100%，剩余比例保留为现金。显式权重只作用于当期策略 Top K 同时选中的股票，未入选配置会进入逐期审计。`minimum_commission` 表示每笔最低佣金；`sell_tax_mode=explicit` 使用固定 `sell_tax_bps`，选择 `cn_historical_stamp_duty` 后则按实际卖出日自动应用 2008-09-19 起 10 bps、2023-08-28 起 5 bps 的 A 股印花税。每笔卖出披露实际税率和政策来源，未覆盖的更早日期 fail-closed。回测默认读取持久化公司行动，Web 可关闭；`corporate_actions` 仍可显式传入现金分红和拆并股，并覆盖同股票、同日、同类型的已保存事件。账本使用未复权价格，同日公司行动先于交易；事件时未持仓不会追溯权益。拆并股产生非整数股时可提供 `cash_in_lieu_price`，系统向下保留整股并把零碎部分兑付现金；未提供结算价时 fail-closed，且不会用行情价格猜测。目标整手和现金上限均包含佣金，结果会分别披露执行价、佣金、税、滑点、公司行动、零碎股补偿和净现金影响。账本先卖超配再买低配；最终卖出受阻会保留未平仓市值。Web 面板默认用 `000300` 做基准。系统不使用当前全市场列表推导过去股票池，以避免隐蔽的生存者偏差。
