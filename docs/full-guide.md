...
>
> 自动买入还支持默认关闭的 `auto_score_weighted_allocation_enabled`。开启后，`auto_allocation_budget`（留空则使用 `auto_cash_per_order`）作为每轮组合预算，按候选评分分配并在单票触顶后重新分配。整轮同时受现金保留、每日预算/订单槽位、最大持仓数、单票/总仓位/行业空间和显式目标权重约束；Agent run 与候选仓位计划会记录 `portfolio_allocation`、整手后的可执行预算和残差。当前是确定性约束分配，不是协方差或任意目标函数优化器。
