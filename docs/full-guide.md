...
> 需要无人值守积累生产形态的校准样本时，可显式设置 `DSA_AGENT_CALIBRATION_SHADOW_ENABLED=true` 和 `DSA_AGENT_CALIBRATION_SHADOW_PAIRS=cn:dual_low,us:us_large_cap_momentum,hk:hk_liquid_momentum`，并通过 `DSA_AGENT_CALIBRATION_SHADOW_INTERVAL_MINUTES`、`DSA_AGENT_CALIBRATION_SHADOW_MAX_RESULTS` 控制周期和候选数。任务首次注册延迟 300 秒，只生成独立 `agent_calibration_shadow` dry-run 记录，绝不提交订单或修改保存的自动交易配置；每个 `市场:策略` 必须匹配 `market_scope`。DSA 在不修改安装包的前提下叠加美股策略，并为港股提供复用 AlphaSift 硬过滤和因子评分的兼容管线；港股股票池可用 `ALPHASIFT_HK_TICKERS` 显式限制。AlphaSift 0.2.0 没有港股 LLM 重排，因此港股结果会明确使用确定性因子排序。
