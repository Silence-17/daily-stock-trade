...
官方 CTP 的连接确认已兼容交易与行情通道 `login_status`，两者均登录才报告 connected。手工工作流 `External vn.py CTP Account Soak` 可从受保护的 `vnpy-ctp-paper-acceptance` Environment secret 创建 runner 临时配置，执行严格预检和零下单账户长跑并上传 14 天脱敏证据；仅应用于隔离的 SimNow/券商模拟账户，生产实盘凭据应在受控部署主机本地验收。完整安全边界与设置方法见 [vn.py 模拟交易说明](vnpy-paper-trading.md)。
