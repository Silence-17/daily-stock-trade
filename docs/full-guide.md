...
> 需要无人值守积累生产形态的校准样本时，可显式设置 `DSA_AGENT_CALIBRATION_SHADOW_ENABLED=true` 和 `DSA_AGENT_CALIBRATION_SHADOW_PAIRS=cn:dual_low,us:us_large_cap_momentum`，并通过 `DSA_AGENT_CALIBRATION_SHADOW_INTERVAL_MINUTES`、`DSA_AGENT_CALIBRATION_SHADOW_MAX_RESULTS` 控制周期和候选数。任务首次注册延迟 300 秒，只生成独立 `agent_calibration_shadow` dry-run 记录，绝不提交订单或修改保存的自动交易配置；每个 `市场:策略` 必须匹配 AlphaSift 返回的 `market_scope`，不兼容配对会明确失败。DSA 会在不修改安装包的前提下叠加美股策略；AlphaSift 0.2.0 的 screen 仍只支持 CN/US，不要配置 HK。
