...
- Automatic buys now have an optional constrained portfolio-allocation stage with score-weighted, score/inverse-20-day-volatility, and correlation-capped risk methods. The correlation method combines AlphaSift's point-in-time volatility with local trailing returns no later than the run date, retains higher-ranked candidates, and excludes later candidates above the pairwise threshold. Missing history or overlap fails closed and the audit includes observations, pairwise coefficients, threshold, conflicting symbol, caps, intended budget, and executable A-share-lot residual. This is a deterministic sequential constraint; covariance-objective and arbitrary-target optimization remain incomplete.
...
The latest post-change runtime smoke uses listener PID `11696`; vn.py runtime, built-in `DSA_SIM` gateway connection, MainEngine bridge, and all four EventEngine callbacks are available while the saved execution mode remains `paper`. The paper-trading page returns HTTP 200. AlphaSift status reports four complete snapshot-source health entries and `dynamic_health` routing; with no current degradation it retains `sina,efinance,akshare_em,em_datacenter`. Portfolio allocation remains disabled by default and its persisted method remains `score_weighted`; the OpenAPI settings enum now exposes `target_tracking_min_variance_20d`, with a default covariance penalty of 0.25 ready for opt-in use. The minimum data-quality score and cross-run quality gate are both disabled by default. The persisted cross-run defaults are a 5-trading-day horizon, 10 mature samples, 45% minimum win rate, and 200 decisions. The read-only `/agent-runs/cross-run-quality` smoke resolved to `insufficient_evidence` with 3 candidates, 0 mature samples, and no gate block. No auto-trade run or order was triggered during verification.
