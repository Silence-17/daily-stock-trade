...
- Persisted Agent candidates have strict forward multi-horizon evaluation; Tushare lifecycle metadata reconstructs each dated A-share universe and feeds lease-protected resumable factor ingestion, while replay now executes equal or explicit per-symbol targets with cash and whole-lot quantities. Minimum commission, sell tax, cash dividends, and split adjustments are configurable and audited, but broker schedules, historical tax regimes, external corporate-action ingestion, and fractional-share cash-in-lieu are not inferred automatically. A successful live full-market run still requires external Tushare permission and stable long-running upstream access.
