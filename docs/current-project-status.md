...
- Agent run details now support persisted human acceptance feedback (`approved`, `needs_changes`, or `rejected`) with reviewer and notes. Feedback appears in the run list, detail timeline, and daily summary, and is carried into later same-strategy/market run context without changing or bypassing risk gates.
...
The latest post-change runtime smoke uses listener PID `8868` and build id `local-human-feedback-final`; vn.py runtime, built-in `DSA_SIM` gateway connection, MainEngine bridge, and all four EventEngine callbacks are available while the saved execution mode remains `paper`. The Agent console returns HTTP 200. Temporary runs verified the live feedback route, persisted verdict/reviewer, daily reviewed count, and a final `human_feedback` timeline event, then were deleted without submitting an order. AlphaSift status reports four complete snapshot-source health entries and `dynamic_health` routing; with no current degradation it retains `sina,efinance,akshare_em,em_datacenter`. Portfolio allocation remains disabled by default and its persisted method remains `score_weighted`; the OpenAPI settings enum exposes `target_tracking_min_variance_20d`, with a default covariance penalty of 0.25 ready for opt-in use. The minimum data-quality score and cross-run quality gate are both disabled by default. No auto-trade run or order was triggered during verification.
...
- The Agent workflow is usable but not a complete autonomous research loop: run-level human acceptance and context reuse are complete, while cross-market dynamic objectives and multi-model/long-horizon review-quality evaluation still need more work.
