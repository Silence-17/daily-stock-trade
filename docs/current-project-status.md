...
The latest post-change runtime uses listener PID `13704` and build id `local-alphasift-llm-circuit-final-v2`; Python 3.13.14, the vn.py runtime, built-in connected `DSA_SIM` gateway, MainEngine bridge, and all four EventEngine callbacks are available while the saved execution mode remains `paper`. Automatic trading remains enabled and its next run is aligned to `2026-07-17T09:30:00` Asia/Shanghai. The read-only LLM policy check reports an open circuit after five persisted ranking failures, deterministic `screen_score` fallback, a 60-minute cooldown, a 10-second recovery probe, and a 300-second probe lease. The paper-trading page and Agent console both return HTTP 200. Agent run count remains eight and the latest run remains `ss-agent-20260716081103-27d104bb`; the final restart and validation created no Agent run or order.
