...
- Automated buys now have optional persisted-snapshot market-breadth and hotspot-retreat gates. Breadth compares the latest `MarketLightSnapshot` breadth score with a configured floor; hotspot retreat compares the latest two limit-dimension scores. Enabled rules fail closed when required evidence is missing, affect buys only, and persist the snapshots, thresholds, score delta, proxy definition, and reason in `diagnostics.market_context_risk` plus the Agent timeline. They do not claim intraday breadth.
...
The latest post-change runtime uses listener PID `14952` and build id `local-market-context-gates-final`; Python 3.13.14, the vn.py runtime, built-in connected `DSA_SIM` gateway, MainEngine bridge, and all four EventEngine callbacks are available while the saved execution mode remains `paper`. Automatic trading remains enabled and its next run is aligned to `2026-07-17T09:30:00` Asia/Shanghai. The new market-breadth and hotspot-retreat gates are loaded with safe defaults of disabled, breadth floor 35, and retreat drop 25. The paper-trading page and Agent console both return HTTP 200. Agent run count remains eight and the latest run remains `ss-agent-20260716081103-27d104bb`; the final restart and validation created no Agent run or order.
...
- `python -m pytest tests/test_vnpy_paper_trading_service.py tests/test_vnpy_paper_trading_api.py -q`
  - 173 passed and 1 optional test skipped, including market-breadth blocking, hotspot-retreat two-snapshot comparison, missing-evidence fail-closed behavior, persisted settings, decision/trade-plan reasons, timeline diagnostics, and existing paper-trading behavior.
