...
- `python -m pytest tests/test_stock_selection_historical_universe_service.py tests/test_tushare_fetcher_get_stock_list.py tests/test_stock_selection_factor_ingestion_service.py tests/test_stock_selection_full_market_ingestion_service.py tests/test_stock_selection_portfolio_backtest_service.py tests/test_stock_selection_strategy_replay_service.py tests/test_stock_selection_agent_backtest_service.py tests/test_alphasift_replay_api.py -q -p no:cacheprovider`
  - 58 passed, adding per-date universe isolation, persistent batch checkpoints, failed-batch resume, force-takeover lease protection, recent-job recovery, cash/whole-lot accounting, explicit target weights, residual cash, configured-but-unselected audit, drift rebalancing, blocked final liquidation, and full-market/API contracts to the prior replay coverage.
...
- Final live replay/backtest smoke on `http://127.0.0.1:8000`
  - OpenAPI exposes replay paths for historical-universe resolution, persistent full-market ingestion create/list/detail/resume, bounded ingestion status, and portfolio backtest. The portfolio request schema includes `target_weights` and defaults `accounting_mode` to `cash_ledger` and `enforce_tradeability` to `true`. `/agent-console` returned HTTP 200; the isolated Python 3.13 vn.py runtime, simulated gateway connection, event bridge, and scheduler loop remained available under server PID `14196`. Execution mode remained `paper`; recovery reported zero attempted/submitted orders and no order was triggered by the smoke.
...
- Persisted Agent candidates have strict forward multi-horizon evaluation; Tushare lifecycle metadata reconstructs each dated A-share universe and feeds lease-protected resumable factor ingestion, while replay now executes equal or explicit per-symbol targets with cash and whole-lot quantities. A successful live full-market run still requires external Tushare permission and stable long-running upstream access; corporate actions and market-specific fee minima/taxes remain incomplete.
