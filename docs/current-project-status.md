...
- `python -m pytest tests/test_stock_selection_full_market_ingestion_service.py tests/test_stock_selection_factor_ingestion_service.py tests/test_stock_selection_corporate_action_repo.py tests/test_stock_selection_portfolio_backtest_service.py tests/test_alphasift_replay_api.py tests/test_tushare_fetcher_get_stock_list.py -q`
  - 54 passed after adding explicit fractional-share cash-in-lieu settlement, missing-price rejection, invalid-field rejection, aggregate cash audit, and API forwarding to the broader ingestion/replay suite.
...
- Persisted Agent candidates have strict forward multi-horizon evaluation; Tushare lifecycle metadata reconstructs each dated A-share universe and feeds lease-protected resumable factor ingestion, while implemented Tushare dividend/share-adjustment events are stored alongside ingestion and consumed by replay by default. Explicit request events can override stored events and provide an actual fractional-share cash-in-lieu settlement price. Broker schedules, historical tax regimes, and settlement prices are not inferred automatically. A successful live full-market and corporate-action run still requires external Tushare permission and stable long-running upstream access.
