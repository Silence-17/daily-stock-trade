...
The latest post-change runtime uses listener PID `10220` and build id `local-default-quality-gate-final`; Python 3.13.14, the vn.py runtime, built-in connected `DSA_SIM` gateway, MainEngine bridge, and all four EventEngine callbacks are available while the saved execution mode remains `paper`. The paper-trading page and Agent console both return HTTP 200, and the previously absent saved quality threshold now resolves to the safe default `60.0`. The initial read-only smoke created no run, plan, or order. At the configured 2026-07-16 13:00 Asia/Shanghai window, the published context-health build then completed run `ss-agent-20260716050026-2b3951b2` with three candidates, one simulated fill, and two skips. It observed quote/news health at 100% and fund-flow health at 0%, producing a 46.16 partial quality score. The filled candidate explicitly lacked `trading_status`; replaying that persisted candidate through the subsequently loaded field-gate build returns `candidate_trading_status_unavailable`, and the 46.16 round is also below the new default quality floor, so both paths now fail closed. Current database counts remain five Agent runs, nine trade plans, and twelve Portfolio trades after the final restart.
